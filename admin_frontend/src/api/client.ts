...
client.interceptors.request.use(
  (config: InternalAxiosRequestConfig) => {
    ...
    // If we're sending FormData (multipart), Axios should handle the Content-Type with boundary.
...
function transformField(name: string, field: Record<string, unknown>): ModelField {
  ...
  const fieldName = (field.name as string) || name
  return {
    name: fieldName,
    ...
    max_digits: (config.max_digits as number) || undefined,
    default_currency: (config.default_currency as string) || undefined,
    currency_field_name: (config.currency_field_name as string) || undefined,
    allow_negative: (config.allow_negative as boolean) || false,
  }
}
...
export const modelsApi = {
  ...
  async createModelInstance(
    ...
    if (hasFiles) {
      ...
      for (const [key, value] of Object.entries(data)) {
        ...
        // Skip undefined/null for create - field defaults will apply
      }
      payload = formData
    }
    ...
    if (hasFiles) {
      ...
      for (const [key, value] of Object.entries(data)) {
        if (value instanceof File || value instanceof Blob) {
          ...
          // Send empty string to indicate field should be cleared
          formData.append(key, '')
  ...
  async getSingleInstance(
    appLabel: string,
    modelName: string
  ): Promise<ModelInstance> {
    const response = await client.get<{ instance: ModelInstance }>(
      `/models/${appLabel}/${modelName}/single/`
    )
    return response.data.instance
  },
