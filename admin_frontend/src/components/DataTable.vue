...
import { countryFlag } from '@/utils/display'
...
function isCurrencyField(fieldName: string): boolean {
  const field = props.model.fields.find((f) => f.name === fieldName)
  return field?.type === 'CurrencyField' || field?.component === 'currency'
}

function getCurrencyCodeForField(fieldName: string, instance: Record<string, unknown>): string {
  const field = props.model.fields.find((f) => f.name === fieldName)
  const siblingName = field?.currency_field_name || `${fieldName}_currency`
  return String(instance[siblingName] ?? field?.default_currency ?? '')
}

const urlFieldNames = computed(() => {
  const names = new Set<string>()
  for (const field of props.model.fields) {
    if (field.type === 'URLField') names.add(field.name)
  }
  return names
})
</script>
  ...
  <div class="overflow-x-auto">
    <table class="table">
      ...
      <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
        <!-- Loading state -->
        <tr v-if="loading">
          <td :colspan="displayColumns.length + (selectable ? 1 : 0)" class="text-center py-8">
        ...
        >
          ...
          <td v-for="column in displayColumns" :key="column">
            ...
            <template v-else-if="isCurrencyField(column) && instance[column] != null">
              <span class="inline-flex items-center gap-1 font-mono text-xs">
                <span class="text-gray-500 dark:text-gray-400 uppercase">{{ getCurrencyCodeForField(column, instance) }}</span>
                <span class="text-gray-900 dark:text-gray-100">{{ formatValue(instance[column]) }}</span>
              </span>
            </template>
            <template v-else>
              {{ formatValue(instance[column]) }}
