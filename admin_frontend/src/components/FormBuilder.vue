...
function getCurrencySiblingName(field: ModelField): string {
  return field.currency_field_name || `${field.name}_currency`
}

function handleCurrencyAmountChange(fieldName: string, value: string | null) {
  updateField(fieldName, value)
}

function handleCurrencyCodeChange(field: ModelField, code: string | null) {
  const siblingName = getCurrencySiblingName(field)
  updateField(siblingName, code)
}
  ...
  <div class="space-y-6">
        ...
        <div class="space-y-4">
          ...
          >
            <template v-if="fieldsetFieldMap[fieldName]">
              ...
              <!-- CurrencyField -->
              <CurrencyFieldInput
                v-else-if="fieldComponentMap[fieldName] === 'currency'"
                :model-value="formData[fieldName] as string | number | null"
                :choices="fieldsetFieldMap[fieldName].choices ?? []"
                :default-currency="fieldsetFieldMap[fieldName].default_currency"
                :currency-field-name="fieldsetFieldMap[fieldName].currency_field_name"
                :disabled="disabled || isReadonly(fieldsetFieldMap[fieldName])"
                :required="fieldsetFieldMap[fieldName].required"
                :decimal-places="fieldsetFieldMap[fieldName].decimal_places"
                :allow-negative="fieldsetFieldMap[fieldName].allow_negative"
                :currency-value="(formData[getCurrencySiblingName(fieldsetFieldMap[fieldName])] as string | null) ?? null"
                @update:model-value="handleCurrencyAmountChange(fieldName, $event)"
                @update:currency="handleCurrencyCodeChange(fieldsetFieldMap[fieldName], $event)"
              />
    ...
    <template v-else>
      ...
      >
        ...
        <!-- CurrencyField -->
        <CurrencyFieldInput
          v-else-if="fieldComponentMap[field.name] === 'currency'"
          :model-value="formData[field.name] as string | number | null"
          :choices="field.choices ?? []"
          :default-currency="field.default_currency"
          :currency-field-name="field.currency_field_name"
          :disabled="disabled || isReadonly(field)"
          :required="field.required"
          :decimal-places="field.decimal_places"
          :allow-negative="field.allow_negative"
          :currency-value="(formData[getCurrencySiblingName(field)] as string | null) ?? null"
          @update:model-value="handleCurrencyAmountChange(field.name, $event)"
          @update:currency="handleCurrencyCodeChange(field, $event)"
        />
