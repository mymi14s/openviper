...
type LeafletDivIcon = InstanceType<typeof import('leaflet').default>['DivIcon']
let PIN_ICON: LeafletDivIcon | null = null

function ensurePinIcon() {
  if (!L || PIN_ICON) return
  PIN_ICON = L.divIcon({
...
let leafletMap: InstanceType<typeof import('leaflet').default> | null = null
let marker: InstanceType<typeof import('leaflet').default.Marker> | null = null
...
function placeMarker(lat: number, lng: number) {
  if (!leafletMap || !L || !PIN_ICON) return
  if (marker) {
    ...
    marker = L.marker([lat, lng], { icon: PIN_ICON, draggable: !props.disabled })
      ...
      .on('dragend', (e: unknown) => {
        const ll = (e as { target: { getLatLng: () => { lng: number; lat: number } } }).target.getLatLng()
        point.value = {
...
onMounted(async () => {
  ...
  if (!props.disabled) {
    leafletMap.on('click', (e: { latlng: { lng: number; lat: number } }) => {
