  ...
  async function createInstance(
    ...
    try {
      ...
      if (errorMessages && typeof errorMessages === 'object') {
        const allMsg = errorMessages.__all__
        if (allMsg) {
          error.value = allMsg
        } else {
          error.value = Object.values(errorMessages).join('; ')
        }
      } else {
        error.value = errorData?.error || 'Failed to create instance'
      }
      return null
    }
  }
  ...
  async function updateInstance(
    ...
    try {
      ...
      if (errorMessages && typeof errorMessages === 'object') {
        ...
        error.value = errorData?.error || 'Failed to update instance'
      }
