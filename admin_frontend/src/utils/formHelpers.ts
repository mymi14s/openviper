...
/** Filter editable fields from a model config (excludes auto-generated id, created_at, updated_at, and currency sibling fields). */
export function getEditableFields(model: ModelConfig): ModelField[] {
  const currencySiblingNames = new Set<string>()
  for (const field of model.fields) {
    if (field.component === 'currency' || field.type === 'CurrencyField') {
      const siblingName = field.currency_field_name || `${field.name}_currency`
      currencySiblingNames.add(siblingName)
    }
  }
  return model.fields.filter((field) => {
    if (field.name === 'id' && field.readonly) return false
    if (field.name === 'created_at' || field.name === 'updated_at') return false
    if (currencySiblingNames.has(field.name)) return false
    return true
  })
}
