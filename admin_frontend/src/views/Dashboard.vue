...
import { formatShortDate, getActionIcon, getActionBadgeClass } from '@/utils/display'
...
async function loadChartJs() {
  ...
  const [
    { Chart: ChartJS, Title, Tooltip, Legend, BarElement, CategoryScale, LinearScale, ArcElement },
  ] = await Promise.all([
    import('chart.js'),
  ])
            ...
            >
              ...
              <div class="flex items-center gap-2 flex-shrink-0">
                ...
                <span class="text-xs text-gray-400 hidden sm:block">
                  {{ activity.change_time ? formatShortDate(activity.change_time) : '' }}
                </span>
              </div>
            </div>
