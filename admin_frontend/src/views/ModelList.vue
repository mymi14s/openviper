...
async function loadFilterOptions() {
  try {
    await adminStore.fetchFilterOptions(props.appLabel, props.modelName)
    filterOptions.value = adminStore.filterOptions
  } catch {
    filterOptions.value = []
  }
}
...
async function handleExport(format: 'csv' | 'json') {
  const blob = await adminStore.exportData(
    props.appLabel,
    props.modelName,
    format,
    selectedIds.value.length > 0 ? { ids: selectedIds.value } : {}
  )
...
<template>
  ...
  <Teleport to="body">
    <div v-if="showBulkConfirm" role="dialog" aria-modal="true" class="fixed inset-0 z-50 flex items-center justify-center">
