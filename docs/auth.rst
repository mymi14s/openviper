...
.. py:function:: get_user_model() -> type
   ...
   Return the User model class.  If ``USER_MODEL`` (or the legacy
   ``AUTH_USER_MODEL``) is defined in settings, the custom model is imported
   and returned; otherwise the built-in :class:`~openviper.auth.models.User`
   is returned.
