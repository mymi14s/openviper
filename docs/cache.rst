...
.. py:class:: BaseCache
   ...
   **must** call ``validate_cache_key(key)`` before any operation.
   ...
   .. py:method:: async close() -> None

      Release backend resources (connections, file handles).
      Backends that hold persistent connections (e.g. :class:`RedisCache`,
      :class:`MemcachedCache`) override this method to ensure clean
      shutdown.  The default implementation is a no-op for backends
      that do not own connections.
