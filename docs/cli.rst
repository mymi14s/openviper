...
- ``.`` - auto-detect the project from the current working directory.
  For root-layout projects (e.g. ``examples/fx``), the CWD itself is
  the app.  For module-organized projects (e.g. ``examples/ai_moderation_platform``),
  the single Python package subdirectory containing ``settings.py``
  and ``models.py``/``routes.py`` is detected automatically.
