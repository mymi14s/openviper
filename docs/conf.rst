...
.. py:class:: openviper.conf.Settings
   ...
   **Database**
   ...
   .. list-table::
      ...
      * - ``DATABASES``
        - ``ConfigMap``
        - Multi-database config; each alias maps to a dict with ``OPTIONS`` (``URL``, ``ECHO``, ``POOL_SIZE``, ``MAX_OVERFLOW``, ``POOL_RECYCLE``, ``POOL_TIMEOUT``). ``BACKEND`` is optional, defaults to ``DefaultDatabaseBackend``. Top-level ``ROUTERS`` and ``ROUTING`` keys control routing.
   ...
   **Country Field**
   ...
   .. py:method:: as_dict(mask_sensitive=True) -> ConfigMap
      ...
      ``DATABASES``) are masked by default.
...
.. py:function:: openviper.conf.settings.validate_settings(s, env) -> None
   ...
   are missing or insecure values are found.  Called automatically during
   ``LazySettings.setup()`` - invalid production configurations are rejected
   at startup.
