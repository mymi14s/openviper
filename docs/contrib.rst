...
``openviper.contrib.fields.currencies``
    ISO 4217 ``CurrencyField`` for storing monetary amounts with per-row
    currency codes.  Stores amount as ``NUMERIC`` (enabling native SQL
    ``SUM``/``AVG``) paired with a ``CHAR(3)`` currency code column.
    Requires ``py-moneyed`` (installed via ``pip install openviper[currencies]``).
...
.. py:class:: DefaultLandingMiddleware(app, *, debug=False, version=__version__, has_custom_root=False)
   ...
   .. py:attribute:: landing_response

      Pre-built :class:`~openviper.http.response.HTMLResponse` for the
      welcome page, or ``None`` in production mode.  Set during
      ``__init__`` when ``debug=True`` and ``has_custom_root=False``.

.. py:data:: NOT_FOUND_RESPONSE

   Module-level :class:`~openviper.http.response.HTMLResponse` with
   status 404, used when the landing middleware is active but debug
   mode is off.
