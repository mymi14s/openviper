...
- ``RemoveColumn`` - soft-remove a column (nullable + tracked in
  ``openviper_soft_removed_columns``).  Pass ``drop=True`` to permanently
  drop the column.
- ``RestoreColumn`` - restore a previously soft-removed column back into
  active use by removing it from the tracking table.
- ``RenameColumn`` - rename a column.
- ``RenameTable`` - rename an existing table.
- ``AlterColumn`` - change column type, nullability, default, or uniqueness.
  Supports a ``using`` parameter for PostgreSQL type conversions.
...
- ``AddConstraint`` - add a ``CHECK`` or ``UNIQUE`` constraint.  ``CHECK``
  constraints accept a raw SQL ``check`` expression.  ``UNIQUE`` constraints
  accept a ``columns`` list and an optional ``condition`` for partial
  indexes (PostgreSQL/SQLite).  All SQL expressions are validated against
...
.. py:function:: validate_on_delete(action, context) -> str
   ...
   ``"SET_DEFAULT"``, ``"NO_ACTION"``, ``"SET_NULL"``, ``"SET_DEFAULT"``,
   and ``"DO_NOTHING"``.  Returns the uppercased action.  Raises
