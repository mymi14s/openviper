...
- Database connection URLs are **never** stored inside backup archives.
- All subprocess calls use ``asyncio.create_subprocess_exec`` — shell
  interpolation is disabled.
- Archive extraction validates every member path to prevent tar-slip
  (path traversal) attacks.
- Backup paths are validated to reject directory traversal sequences.
- MySQL/MariaDB passwords are written to a temporary ``0o600`` options
  file rather than passed as a command-line argument, preventing exposure
  via ``ps`` listings.
