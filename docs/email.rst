...
This means **zero configuration** is needed - if OpenViper can enqueue the
message to a configured task broker, emails are queued automatically.  If the
broker is unavailable while auto-detection is in use, OpenViper falls back to
inline delivery instead of dropping the message.
