...
.. py:data:: BACKEND_REGISTRY
   ...
   ``dict[str, BaseGeoBackend]`` mapping lowercase dialect strings to
   singleton backend instances.  ``get_backend()`` looks up this
   registry and falls back to ``"generic"``.
...
.. py:function:: register_postgis_types() -> None

   Register ``geometry``, ``geography``, ``point``, ``polygon``, and
   ``linestring`` with ``PGDialect.ischema_names`` so SQLAlchemy
   introspection recognises PostGIS columns.

.. py:function:: is_postgresql() -> bool

   Return ``True`` if the configured database engine targets
   PostgreSQL.  Checks ``db_connection._engine.url`` first, then
   falls back to ``settings.DATABASE_URL``.

.. py:class:: GeoType

   ``sqlalchemy.types.UserDefinedType`` placeholder for PostGIS
   ``GEOMETRY`` columns during SQLAlchemy reflection.

.. py:class:: AdaptiveGeometryType

   ``sqlalchemy.types.TypeDecorator`` that wraps EWKT for PostGIS at
   runtime.  Uses ``ST_GeomFromEWKT`` on bind and ``ST_AsEWKT`` on
   fetch when connected to PostgreSQL; passes through unchanged on
   other databases.

.. py:class:: PostGISGeometryType

   Legacy ``sqlalchemy.types.UserDefinedType`` for PostGIS
   ``GEOMETRY`` columns.  Kept for backwards compatibility.
