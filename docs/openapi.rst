...
.. py:function:: read_openapi_settings() -> dict[str, object]
   ...
   Read and normalise the ``OPENAPI`` config dict from settings.  Maps
   legacy flat attributes (``OPENAPI_ENABLED``, ``OPENAPI_EXCLUDE``, etc.)
   into the dict for backward compatibility, then fills defaults for any
   missing keys.
...
A dict consolidating all OpenAPI configuration.  Each key maps to a
previously separate ``OPENAPI_*`` flat setting; the flat names are still
accepted for backward compatibility via ``read_openapi_settings()``.
