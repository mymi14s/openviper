...
.. py:class:: openviper.serializers.Serializer
   ...
   **Internal helpers** (used by ``serialize_many`` / ``paginate``):
   ...
   .. py:staticmethod:: serialize_value(value) -> bool | int | float | str | list | dict | None
      ...
      ``list`` / ``dict`` containers recursively.  Contrib field value
      types (``Country``, ``Money``, ``Point``) are delegated to
      registered contrib serializers via
      :func:`serialize_contrib_value`.
...
.. py:function:: openviper.serializers.register_contrib_serializer(type_name, fn)

   Register a serializer function for a contrib field value type.
   Called at import time by each contrib package's ``__init__.py``.

.. py:function:: openviper.serializers.serialize_contrib_value(value)

   Check registered contrib serializers for a matching value type.
   Returns ``_CONTRIB_SENTINEL`` when no match is found, signalling the
   caller to fall through to the default ``return value``.
...
.. list-table::
   ...
   * - Package
     ...
     - JSON output
     - Serializer module
   * - ``contrib.fields.countries``
     - ``Country``
     - ``"US"`` (string)
     - ``countries/serializer.py``
   * - ``contrib.fields.currencies``
     - ``Money``
     - ``{"amount": 100.5, "currency": "USD"}``
     - ``currencies/serializer.py``
   * - ``contrib.fields.geolocation``
     - ``Point``
     - ``{"type": "Point", "coordinates": [lon, lat]}``
     - ``geolocation/serializer.py``
