...
.. list-table::
   ...
   * - ``database_url``
     - Explicit test database URL.  When omitted, a dedicated ``test_``-prefixed
       database is derived from ``settings.DATABASES``, falling back to in-memory
       SQLite.
   * - ``database_isolation``
...
* ``resolve_test_database_url``, ``assert_safe_database_url`` (``openviper.testing.database``)
