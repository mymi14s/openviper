...
task system is enabled (non-empty ``TASKS`` dict in settings).
...
class EventModel(Protocol):
    """Model surface required by example event handlers."""
...
def cleanup_comments(post: EventModel, event: str | None = None) -> None:
...
def handle_post_update(post: EventModel, event: str | None = None) -> None:
...
def moderate_post(post: EventModel, event: str | None = None) -> None:
