...
class Post(Model):
    ...
    comments_count = IntegerField(default=0, null=True, blank=True)
...
class CommentLike(Model):
    ...
    comment = ForeignKey("posts.models.Comment", on_delete="CASCADE")
    user = ForeignKey(UserModel, on_delete="CASCADE")
    created_at = DateTimeField(auto_now_add=True)
...
class PostReport(Model):
    ...
    reported_by = ForeignKey(UserModel, on_delete="CASCADE")
    reason = TextField()
    created_at = DateTimeField(auto_now_add=True)
