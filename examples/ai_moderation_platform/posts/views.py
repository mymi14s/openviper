...
def serialize_post(
    post: Post, preview: bool = False, user_id: int | None = None
) -> dict[str, object]:
    data = PostResponseSerializer.from_orm(post).serialize()
    if preview and len(data["content"]) > 300:
        data["content"] = data["content"][:300] + "..."

    if user_id is not None:
        data["user_liked"] = hasattr(post, "_user_liked") and post._user_liked
    else:
...
class PostListCreateView(View):
    ...
    async def get(self, request: Request) -> Response:
        ...
        if search_query:
            ...
            # Apply pagination
            posts = [p async for p in qs.order_by("id").offset(offset).limit(page_size)]
        ...
        for post in posts:
            ...
            serialized_posts.append(serialize_post(post, preview=True, user_id=user_id))
...
class PostDetailView(View):
    ...
    async def get(self, request: Request, post_id: int) -> Response:
        ...
        return JSONResponse(serialize_post(post, user_id=user_id))
...
class BlogListAPIView(View):
    ...
    async def get(self, request: Request) -> Response:
        ...
        # Build base queryset with stable ordering for cursor pagination.
        qs = Post.objects.filter(is_hidden=False).order_by("-created_at", "id")
        ...
        if search_query:
            ...
            # Build pagination URLs manually for in-Python filtering
            base_url = "/posts/blog/"
            next_url = None
            previous_url = None
        ...
        # Use serializer pagination for DB-level filtering (fast, cursor-aware).
        result = await PostResponseSerializer.paginate(
            qs,
            page=page,
            page_size=page_size,
            base_url="/posts/blog",
