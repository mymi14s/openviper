...
async def register_page(request: Request) -> Response:
    ...
    if request.method == "POST":
        ...
        if not username or not email or not password:
            ...
            existing = await UserModel.objects.filter(username=username).first()
            if existing:
                error = "Username already taken."
            else:
                existing_email = await UserModel.objects.filter(email=email).first()
                if existing_email:
        ...
        if error:
            return HTMLResponse(
                ...
                context={"title": "Register", "error": error, "form": dict(form.items())},
            )
