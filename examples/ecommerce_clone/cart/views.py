...
def item_to_dict(item: object) -> dict[str, object]:
...
async def enrich_items(items: list[object]) -> list[object]:
...
async def get_or_create_cart(user_id: int) -> object:
...
class CartView(View):
    ...
    async def get(self, request: Request) -> Response:
        ...
        cart = await get_or_create_cart(request.user.id)
        items = await CartItem.objects.filter(cart_id=cart.id).all()
        items = await enrich_items(list(items))
        return JSONResponse(
            {
                "id": str(cart.id),
...
class CartAddView(View):
    ...
    async def post(self, request: Request) -> Response:
        ...
        return JSONResponse(item_to_dict(item), status_code=201)
