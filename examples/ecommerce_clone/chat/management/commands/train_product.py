...
from chat.ai_service import get_router
...
class Command(BaseCommand):
    ...
    def handle(self, *args: object, **kwargs: object) -> None:
        asyncio.run(self.run_training(clear=bool(kwargs.get("clear", False))))

    async def run_training(self, clear: bool = False) -> None:
        await init_db()
        ...
        async for batch in Product.objects.all().batch(size=200):
            for product in batch:
                ...
                summary_text = build_product_context(product)
...
def build_product_context(product: object) -> str:
    category = str(product.category_id) if product.category_id else "Uncategorised"
    return (
