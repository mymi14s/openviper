...
def fake_price(category: str) -> Decimal:
    price_ranges = {
...
async def clear_data(stdout: object) -> None:
...
async def seed_data(
    ...
    while created < num_products:
        ...
        name = fake_product_name(cat_name, existing_names)
        description = fake_description(cat_name, name)
        price = fake_price(cat_name)
    ...
    for product in random.sample(products, min(len(products), max(1, len(products) * 2 // 3))):
        ...
        for user in reviewers:
            existing = await Review.objects.filter(product_id=product.id, user_id=user.id).first()
            if existing:
                continue
            review = Review(
                user_id=user.id,
    ...
    cart_count = 0
    for user in users:
        existing_cart = await Cart.objects.filter(user_id=user.id).first()
        if existing_cart:
            continue
...
class Command(BaseCommand):
    help = (
        "Populate the database with fake ecommerce data "
