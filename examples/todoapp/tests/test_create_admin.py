...
async def test_create_new_superuser(capsys):
    ...
    user = await user_model.objects.get_or_none(username="newadmin")
...
async def test_empty_password_exits_early(capsys):
    ...
    user_model = get_user_model()
    assert await user_model.objects.get_or_none(username="nopwduser") is None


async def test_existing_user_update_yes(capsys):
    """main() promotes an existing user when the operator answers 'y'."""
    user_model = get_user_model()
    existing = user_model(username="existingadmin", email="ex@example.com")
    await existing.set_password("old_pass")
    await existing.save()
    ...
    updated = await user_model.objects.get_or_none(username="existingadmin")
...
async def test_existing_user_update_no(capsys):
    ...
    user_model = get_user_model()
    existing = user_model(username="keepadmin", email="keep@example.com", is_superuser=False)
    await existing.set_password("old_pass")
    await existing.save()
...
def test_main_entrypoint_calls_asyncio_run():
    ...
    with patch.object(asyncio, "run", side_effect=close_coro) as mock_run:
