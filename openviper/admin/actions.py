...
action_registry: dict[str, type[AdminAction]] = {
...
def register_action(action_class: type[AdminAction]) -> type[AdminAction]:
    ...
    action_registry[instance.name] = action_class
    return action_class
...
def get_action(name: str) -> AdminAction | None:
    ...
    """
    action_class = action_registry.get(name)
    if action_class:
        return action_class()
    return None
...
def get_available_actions(request: Request) -> list[AdminAction]:
    ...
    """
    actions = []
    for action_class in action_registry.values():
        action = action_class()
