...
async def _serialize_instance_with_children(
    request: Request, model_admin: ModelAdmin, model_class: type[Model], instance: Model
) -> dict[str, Any]:
    """Helper to serialize an instance with its child table data."""
    sensitive_fields = ["password"] if _is_auth_user_model(model_class) else []
...
def get_admin_router() -> Router:
    ...
    async def admin_refresh_token(request: Request) -> JSONResponse:
        ...
        user_cls = User
        user = await user_cls.objects.get_or_none(id=user_id)
        if not user:
    ...
    async def admin_change_password(request: Request) -> JSONResponse:
        ...
        user_cls = User
        user = await user_cls.objects.get_or_none(id=request.user.id)
        if not user:
    ...
    @router.post("/auth/change-user-password/{user_id:int}/")
    async def admin_change_user_password(request: Request, user_id: int) -> JSONResponse:
    ...
    async def admin_dashboard(request: Request) -> JSONResponse:
        ...
        # Collect stats for each registered model
        stats = {}
        for model_class, _ in admin.get_all_models():
            try:
                count = await model_class.objects.count()
                stats[model_class.__name__] = count
            except Exception:
                stats[model_class.__name__] = 0
    ...
    async def list_instances_by_app(
        ...
        # Exclude only password field for auth user models
        sensitive_fields = ["password"] if _is_auth_user_model(model_class) else []
            ...
            async with engine.begin() as _:
                ...
                for inline_class in model_admin.child_tables or model_admin.inlines:
                    ...
                    for row in incoming_rows:
                        ...
                        # Apply extra filters as fixed values
                        if hasattr(inline, "extra_filters") and inline.extra_filters:
    ...
    @router.get("/models/{app_label}/{model_name}/{obj_id:int}/")
    ...
    @router.put("/models/{app_label}/{model_name}/{obj_id:int}/")
    ...
    @router.delete("/models/{app_label}/{model_name}/{obj_id:int}/")
    ...
    @router.get("/models/{app_label}/{model_name}/{obj_id:int}/history/")
    ...
    async def list_instances(request: Request, model_name: str) -> JSONResponse:
        ...
        # Apply pagination
        page = int(request.query_params.get("page", 1))
        page_size = int(request.query_params.get("page_size", model_admin.list_per_page))
        offset = (page - 1) * page_size
    ...
    @router.get("/models/{model_name}/{obj_id:int}/")
    async def get_instance(request: Request, model_name: str, obj_id: int) -> JSONResponse:
        ...
        instance = await model_class.objects.get_or_none(id=obj_id)
        if not instance:
            raise NotFound(f"{model_name} with id {id} not found.")
    ...
    @router.patch("/models/{model_name}/{obj_id:int}/")
    async def update_instance(request: Request, model_name: str, obj_id: int) -> JSONResponse:
    ...
    @router.delete("/models/{model_name}/{obj_id:int}/")
    async def delete_instance(request: Request, model_name: str, obj_id: int) -> JSONResponse:
        """Delete a model instance."""
        if not check_admin_access(request):
            raise PermissionDenied("Admin access required.")
    ...
    async def bulk_delete(request: Request, model_name: str) -> JSONResponse:
        ...
        # Delete instances
        count = await model_class.objects.filter(id__in=ids).delete()
    ...
    @router.get("/models/{model_name}/{obj_id:int}/history/")
    async def get_instance_history(request: Request, model_name: str, obj_id: int) -> JSONResponse:
        """Get change history for a model instance."""
        if not check_admin_access(request):
            raise PermissionDenied("Admin access required.")
        ...
        # Get history
        history_records = await get_change_history(model_name, id)
