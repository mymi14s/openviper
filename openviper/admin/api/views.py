...
from openviper.admin.constants import (
    MIN_PASSWORD_LENGTH,
...
from openviper.auth.constants import JWT_GRANT_TYPE_ACCESS, JWT_GRANT_TYPE_REFRESH, MAX_TOKEN_LENGTH
from openviper.auth.jwt import decode_refresh_token_checked, decode_token_unverified
...
from openviper.db.fields import ManyToManyField, PasswordField, SensitiveField
...
def resolve_user_model() -> type:
    """Lazily resolve and cache the user model on first access."""
    global User
    if User is None:
        User = get_user_model()
    return User
...
def is_auth_user_model(model_class: type) -> bool:
    ...
    Returns:
        True if the model is the configured USER_MODEL (or extends it).
    """
    try:
...
def parse_validation_errors(message: str, field_names: set[str]) -> dict[str, str]:
    """Parse an aggregate validation error message into per-field errors.

    The ORM's ``Model.validate()`` raises a single ``ValueError`` joining
    all field-level errors with ``"; "``.  This function splits them back
    out and maps each to its originating field name so the admin API can
    return field-keyed errors instead of a generic ``__all__`` blob.
    """
    if not field_names:
        return {}
...
def get_admin_router() -> Router:
        ...
        return JSONResponse(
            {
                ...
                "is_custom_user": getattr(settings, "USER_MODEL", None) is not None,
            }
        )
    ...
    async def admin_login(request: Request) -> JSONResponse:
        ...
        try:
            creds = LoginCredentials.from_request_body(data_raw)
        except ValueError as exc:
            raise ValidationError({"detail": str(exc)}) from exc

        try:
            user = await authenticate(username=creds.username, password=creds.password)
        ...
        if auth_header.startswith("Bearer "):
            ...
            if jti:
                ...
                with contextlib.suppress(Exception):
                    await revoke_token(
                        jti=jti,
                        token_type=JWT_GRANT_TYPE_ACCESS,
    ...
    async def admin_refresh_token(request: Request) -> JSONResponse:
        ...
        if len(refresh_token) > MAX_TOKEN_LENGTH:
            raise ValidationError({"detail": "Refresh token exceeds maximum allowed length."})

        try:
            payload = await decode_refresh_token_checked(refresh_token)
        ...
        if jti:
            try:
                expires_at = (
                    datetime.datetime.fromtimestamp(float(exp), tz=datetime.UTC)
                    if exp
                    else timezone.now() + datetime.timedelta(days=7)
                )
                await revoke_token(jti, JWT_GRANT_TYPE_REFRESH, user_id, expires_at)
    ...
    async def admin_change_password(request: Request) -> JSONResponse:
        ...
        if new_password != confirm_password:
            raise ValidationError({"detail": PASSWORD_MISMATCH_MSG})

        if len(new_password) < MIN_PASSWORD_LENGTH:
            raise ValidationError({"detail": MIN_PASSWORD_LENGTH_MSG})
    ...
    async def update_single_instance(
        ...
        for field_name, value in data.items():
            ...
            try:
                if field_name in fields:
                    ...
                    _is_pw = isinstance(fields[field_name], (PasswordField, SensitiveField))
                    if _is_pw and coerced_value is None:
                        continue
                    setattr(instance, field_name, coerced_value)
                    new_data[field_name] = coerced_value
                    fields[field_name].validate(getattr(instance, field_name, None))
                else:
                    continue
            except ValueError as exc:
        ...
        try:
            ...
            parsed = parse_validation_errors(str(exc), set(fields.keys()))
            if parsed:
                return JSONResponse({"errors": parsed}, status_code=422)
            return JSONResponse({"errors": {"__all__": str(exc)}}, status_code=422)
        except Exception as exc:
            logger.error(
                "Singleton update failed for %s.%s: %s",
                app_label,
                model_name,
                str(exc),
                exc_info=True,
    ...
    async def create_instance_by_app(
        ...
        for field_name, value in data.items():
            ...
            try:
                ...
                _is_pw = isinstance(field, (PasswordField, SensitiveField))
                if _is_pw and coerced_data[field_name] is None:
                    del coerced_data[field_name]
                    continue
                validate_value = coerced_data[field_name]
                if getattr(field, "_is_currency_sibling", False) is not True:
                    sibling_name = getattr(field, "currency_field_name_override", None)
                    if sibling_name is None and hasattr(field, "resolve_currency_field_name"):
                        sibling_name = field.resolve_currency_field_name(field_name)
                    if sibling_name and sibling_name in coerced_data:
                        validate_value = (coerced_data[field_name], coerced_data[sibling_name])
                field.validate(validate_value)
            except ValueError as exc:
                logger.warning(
                    "Field validation failed for %s.%s.%s: %s",
                    app_label,
                    model_name,
                    field_name,
                    exc,
        ...
        try:
            ...
            parsed_errors = parse_validation_errors(str(exc), set(fields.keys()))
            if parsed_errors:
                return JSONResponse({"errors": parsed_errors}, status_code=422)
            return JSONResponse({"errors": {"__all__": str(exc)}}, status_code=422)
        ...
        for field_name, value in data.items():
            ...
            try:
                if field_name in fields:
                    ...
                    _is_pw = isinstance(field_obj, (PasswordField, SensitiveField))
                    if _is_pw and coerced_data[field_name] is None:
                        del coerced_data[field_name]
                        continue
                else:
