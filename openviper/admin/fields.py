...
try:
    from openviper.contrib.fields.currencies.utils import (
        get_currency_choices as currency_choices_fn,
    )
except ImportError:
    currency_choices_fn: t.Callable | None = None
...
def get_filter_choices(field: Field) -> list[dict[str, str]]:
    """Return filter choices for a field, including lazy-loaded CountryField/CurrencyField data."""
    ...
    if field.__class__.__name__ == "CurrencyField" and currency_choices_fn is not None:
        extra = getattr(field, "extra_currencies", ())
        return [{"value": code, "label": name} for code, name in currency_choices_fn(extra)]
...
def get_field_widget_config(field: Field) -> dict[str, t.Any]:
    ...
    if field_class_name == "SensitiveField":
        config["max_length"] = getattr(field, "_plaintext_max", 128)
        config["password_field"] = True

    if field_class_name == "PasswordField":
        config["max_length"] = getattr(field, "_plaintext_max", 128)
        config["password_field"] = True
        config["hashed"] = True
    ...
    if field_class_name == "CurrencyField" and currency_choices_fn is not None:
        extra = getattr(field, "extra_currencies", ())
        choices = currency_choices_fn(extra)
        config["choices"] = [{"value": code, "label": name} for code, name in choices]
        config["searchable"] = True
        config["currency_field"] = True
        config["default_currency"] = getattr(field, "default_currency", "USD")
        config["decimal_places"] = getattr(field, "decimal_places", 4)
        config["max_digits"] = getattr(field, "max_digits", 19)
        config["allow_negative"] = getattr(field, "allow_negative", False)
        currency_field_name = getattr(field, "currency_field_name_override", None)
        if currency_field_name is None:
            currency_field_name = f"{getattr(field, 'name', '')}_currency"
        config["currency_field_name"] = currency_field_name
...
def coerce_field_value(field: Field, value: t.Any) -> t.Any:
    ...
    try:
        ...
        if field_class_name == "CurrencyField":
            return Decimal(str(value))
        ...
        if field_class_name == "CurrencyCodeField":
    ...
    if field_class_name in ("PasswordField", "SensitiveField"):
        # The masked sentinel is sent back when the user leaves the field
        # unchanged.  Returning None signals the save path to skip this field.
        if value == "****":
            return None
        return str(value) if value else None

    if field_class_name == "ArrayField":
        if isinstance(value, str):
            try:
                return json.loads(value)
    ...
    if field_class_name == "DurationField":
        if isinstance(value, (int, float)):
            return int(value)
        if isinstance(value, str):
            # Parse ISO 8601 duration or microseconds
            try:
                return int(value)
            except ValueError:
                return None
    ...
    if field_class_name == "BinaryField":
        if isinstance(value, str):
            return value.encode()
        return value
