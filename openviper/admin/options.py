...
class ModelAdmin:
    ...
    list_select_related: list[str] | bool = []  # Fields to eager load in list view
    ...
    # Fields that should never be serialized/exposed in API responses
    ...
    def get_list_select_related(self, request: Request | None = None) -> list[str]:
        ...
        if isinstance(self.list_select_related, bool):
            if self.list_select_related:
                # Auto-detect foreign keys from list_display
                fk_fields = []
    ...
    def get_model_info(self, request: Any = None) -> dict[str, Any]:
        ...
        """
        # Return cached version if available (ignoring request for caching purposes)
        if self._cached_model_info is not None:
            # Override permissions for each request
            info = self._cached_model_info.copy()
            info["permissions"] = {
        ...
        # Add permissions for the current request
        info = self._cached_model_info.copy()
        info["permissions"] = {
            "add": bool(self.has_add_permission(request)),
            "change": bool(self.has_change_permission(request)),
