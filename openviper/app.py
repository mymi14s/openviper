...
class OpenViper:
    ...
    def __init__(
        ...
        if self.debug and os.environ.get(
            "OPENVIPER_ENV", os.environ.get("ENVIRONMENT", ""),
        ) in ("production", "prod"):
            logger.warning(
        ...
        self.on_shutdown(self.cleanup_database_connections)
    ...
    async def cleanup_database_connections(self) -> None:
        """Dispose all database engines before the event loop closes."""
    ...
    def add_static_file_serving(self, app: ASGIApp) -> ASGIApp:
        ...
        Both ``self.debug`` and the ``OPENVIPER_ENV`` (or legacy ``ENVIRONMENT``)
        variable are checked so that static serving is never accidentally
        enabled in production.
        """
        env = os.environ.get("OPENVIPER_ENV", os.environ.get("ENVIRONMENT", "")).lower()
