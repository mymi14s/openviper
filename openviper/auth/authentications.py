...
class JWTAuthentication(BaseAuthentication):
    ...
    async def authenticate(self, request: Request) -> tuple[Authenticable, AuthPayload] | None:
        ...
        # Reject oversized tokens before attempting any decode work.
        ...
        try:
            ...
            logger.debug("JWT authentication failed for request to %s: %s", request.path, exc)
...
class TokenAuthentication(BaseAuthentication):
    ...
    async def authenticate(self, request: Request) -> tuple[Authenticable, AuthPayload] | None:
        ...
        raw = auth_header[6:]
        # Reject oversized tokens before hashing.
        ...
        try:
            ...
            if row.expires_at is not None:
                now_utc = timezone.now()
                exp = row.expires_at
                # Normalise timezone awareness on both sides to avoid TypeError
                # when one is tz-aware and the other is naive.
                exp_aware = getattr(exp, "tzinfo", None) is not None
                now_aware = getattr(now_utc, "tzinfo", None) is not None
                if exp_aware and not now_aware:
                    now_compare: datetime.datetime = now_utc.replace(
                        tzinfo=datetime.UTC
                    )
                elif not exp_aware and now_aware:
...
class OAuth2Authentication(BaseAuthentication):
    ...
    async def trigger_event(self, event_name: str, payload: AuthPayload) -> None:
        ...
        """
        if event_name not in OAUTH2_EVENT_NAMES:
