...
class AbstractUser(Model):
    ...
    async def get_roles(self) -> list[Role]:
        ...
        if self.role_profile:
            ...
            if not details:
                return []
            # Partition into already-prefetched Role objects and bare IDs.
            # Batch-fetch the bare IDs in a single query instead of N individual ones.
            roles: list[Role] = []
            ids_to_fetch: list[int] = []
            for detail in details:
                role_id = detail.role  # type: ignore[attr-defined]
                if isinstance(role_id, Role):
                    roles.append(role_id)
                else:
                    ids_to_fetch.append(role_id)
            if ids_to_fetch:
                fetched = await Role.objects.filter(id__in=ids_to_fetch).all()
                roles.extend(cast("list[Role]", fetched))
            return roles

        # Fetch user roles with select_related optimization
        ...
        for ur in user_roles:
            role_id = ur.role  # type: ignore[attr-defined]
            if isinstance(role_id, Role):
                roles.append(role_id)
            else:
                ids_to_fetch.append(role_id)
    ...
    async def get_permissions(self) -> set[str]:
        ...
        # Optimized: fetch role permissions with select_related to get permissions in fewer queries
        role_perms = (
        ...
        # Extract permission codenames, handling both prefetched and ID-only relationships
        result_set: set[str] = set()
        perm_ids_to_fetch = []

        for rp in role_perms:
            perm = rp.permission  # type: ignore[attr-defined]
            if isinstance(perm, Permission):
                # Permission was prefetched
                result_set.add(str(perm.codename))
            else:
                # Still just an ID, need to fetch
                perm_ids_to_fetch.append(perm)

        # Fetch any remaining permissions that weren't prefetched
        if perm_ids_to_fetch:
            perms = await Permission.objects.filter(id__in=perm_ids_to_fetch).all()
            result_set.update(p.codename for p in perms)  # type: ignore[attr-defined]

        self._cached_perms = result_set
        return result_set
    ...
    async def has_model_perm(self, model_label: str, action: str) -> bool:
        ...
        # 1. Check global roles (cached by get_roles)
        roles = await self.get_roles()
        for role in roles:
        ...
        role_ids = [r.pk for r in roles]
        # Optimized: fetch content type permissions for all roles at once
