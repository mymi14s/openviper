...
ModelEventFunc = TypeVar("ModelEventFunc", bound=Callable[..., object])


def model_event_trigger(event_name: str) -> Callable[[ModelEventFunc], ModelEventFunc]:
    """Return a typed model event trigger decorator."""
    return cast("Callable[[ModelEventFunc], ModelEventFunc]", model_event.trigger(event_name))


def model_event_on_update(model: type[object]) -> Callable[[ModelEventFunc], ModelEventFunc]:
    """Return a typed model update decorator."""
    return cast("Callable[[ModelEventFunc], ModelEventFunc]", model_event.on_update(model))
...
class AbstractUser(Model):
    ...
    async def set_password(self, raw_password: str) -> None:
        ...
        object.__setattr__(self, "password", await make_password(raw_password))
    ...
    async def get_roles(self) -> list[Role]:
        ...
        if self.role_profile:
            ...
            for detail in details:
                role_obj = detail.role
                if isinstance(role_obj, Role):
                    roles.append(role_obj)
                else:
                    ids_to_fetch.append(cast("int", role_obj))
            if ids_to_fetch:
        ...
        for ur in user_roles:
            ...
            if isinstance(role_obj, Role):
                ...
                role_ids_to_fetch.append(cast("int", role_obj))
...
async def reset_user_permission_cache(user_id: int) -> None:
...
async def reset_all_superusers() -> None:
    """Reset caches for all superusers (they implicitly have all permissions)."""
    try:
        superusers = await get_user_model_lazy().objects.filter(is_superuser=True).all()
        for user in superusers:
            await reset_user_permission_cache(user.pk)
        logger.debug("Cleared permission caches for all superusers")
    except (LookupError, RuntimeError) as e:
        logger.warning("Failed to reset superuser caches: %s", e)


async def reset_users_by_role(role_id: int) -> None:
    ...
    for ur in user_roles:
        user_obj = ur.user
        uid = getattr(user_obj, "pk", user_obj)
        await reset_user_permission_cache(cast("int", uid))


async def reset_users_by_permission(permission_id: int) -> None:
    ...
    for rp in role_perms:
        role_obj = rp.role
        role_id = getattr(role_obj, "pk", role_obj)
        await reset_users_by_role(cast("int", role_id))


async def reset_users_by_content_type_permission(role_id: int) -> None:
    """Reset caches for all users assigned a role with ContentTypePermission."""
    await reset_users_by_role(role_id)


def get_user_model_class() -> type[object]:
    """Dynamically resolve the active User model."""
    user_model = getattr(settings, "USER_MODEL", "openviper.auth.models.User")
    if user_model != "openviper.auth.models.User":
        try:
            ...
            try:
                ...
                logger.warning(
                    "Failed to import USER_MODEL %r - falling back to built-in User.",
                    user_model,
                )
    return User


def get_user_model_lazy() -> type[object]:
    """Lazily resolve and cache the active User model on first access."""
    global UserModel
    if UserModel is None:
        UserModel = get_user_model_class()
    return UserModel


UserModel: type[object] | None = None


@model_event_trigger("openviper.auth.models.UserRole.after_insert")
@model_event_trigger("openviper.auth.models.UserRole.on_change")
@model_event_trigger("openviper.auth.models.UserRole.after_delete")
def on_user_role_change(instance: object, event: str | None = None, **kwargs: object) -> None:
    """Reset permission caches for the user affected by a UserRole change."""
    user_id = getattr(instance, "user", None)
...
@model_event_trigger("openviper.auth.models.RolePermission.after_insert")
@model_event_trigger("openviper.auth.models.RolePermission.after_delete")
def on_role_permission_change(instance: object, event: str | None = None, **kwargs: object) -> None:
    ...
    if role_id:
        task1 = asyncio.create_task(reset_users_by_role(role_id))
        _background_tasks.add(task1)
        task1.add_done_callback(_background_tasks.discard)

    task2 = asyncio.create_task(reset_all_superusers())
    _background_tasks.add(task2)
    task2.add_done_callback(_background_tasks.discard)


@model_event_trigger("openviper.auth.models.ContentTypePermission.after_insert")
@model_event_trigger("openviper.auth.models.ContentTypePermission.after_delete")
@model_event_trigger("openviper.auth.models.ContentTypePermission.on_update")
def on_content_type_permission_change(
    instance: object, event: str | None = None, **kwargs: object
) -> None:
...
@model_event_trigger("openviper.auth.models.Permission.after_insert")
@model_event_trigger("openviper.auth.models.Permission.after_delete")
@model_event_trigger("openviper.auth.models.Permission.on_update")
def on_permission_change(instance: object, event: str | None = None, **kwargs: object) -> None:
    ...
    if permission_id:
        task1 = asyncio.create_task(reset_users_by_permission(permission_id))
        _background_tasks.add(task1)
        task1.add_done_callback(_background_tasks.discard)
...
@model_event_trigger("openviper.auth.models.Role.after_delete")
def on_role_delete(instance: object, event: str | None = None, **kwargs: object) -> None:
...
@model_event.on_update("auth.models.User")
async def on_user_update(instance: object, **kwargs: object) -> None:
