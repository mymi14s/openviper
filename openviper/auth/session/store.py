...
SESSION_USER_CACHE_PREFIX = "session_user:"
...
class DatabaseSessionStore(BaseSessionStore):
    ...
    async def get_user(self, session_key: str) -> Authenticable | None:
        ...
        # Intent: Normalize backend datetimes before expiry comparison.
    ...
    async def rotate(
        ...
        Creates the new session first so that a failure during creation does
        not leave the user without a valid session (state isolation).
        """
        new_session = await self.create(user_id, data)
