...
async def create_session(user_id: int | str, data: Mapping[str, object] | None = None) -> str:
...
async def get_user_from_session(cookie_header: str) -> Authenticable | None:
...
async def clear_session_cache() -> None:
    ...
    """
    cache = get_cache()
    for prefix in (SESSION_CACHE_PREFIX, SESSION_USER_CACHE_PREFIX):
