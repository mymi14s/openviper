...
def read_email_config() -> dict[str, object]:
...
class EmailSettings:
    ...
    def __repr__(self) -> str:
        """Redact password from repr to prevent credential leakage in logs."""
        return (
            f"EmailSettings(host={self.host!r}, port={self.port!r}, "
            f"username={self.username!r}, password='***', "
            f"use_tls={self.use_tls!r}, use_ssl={self.use_ssl!r}, "
            f"default_sender={self.default_sender!r}, timeout={self.timeout!r})"
        )
    ...
    def from_settings(cls) -> EmailSettings:
        ...
        return cls(
            ...
            port=int(config.get("port") or 587),  # type: ignore[call-overload]
...
class SMTPBackend:
    ...
    async def send(self, data: EmailMessageData) -> None:
        ...
        await asyncio.to_thread(send_smtp_message, message, recipients, self.email_settings)
...
def get_backend(name: str | None = None) -> EmailBackend:
    ...
    backend_class: type[EmailBackend] | None = mapping.get(resolved)
...
def send_smtp_message(
    message: EmailMessage, recipients: list[str], email_settings: EmailSettings
) -> None:
