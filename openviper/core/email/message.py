...
def build_message(data: EmailMessageData) -> EmailMessage:
    ...
    message["Subject"] = CRLF_RE.sub("", data.subject)
    message["From"] = CRLF_RE.sub("", data.sender)
    message["To"] = ", ".join(CRLF_RE.sub("", addr) for addr in data.recipients)
    if data.cc:
        message["Cc"] = ", ".join(CRLF_RE.sub("", addr) for addr in data.cc)

    plain_text = data.text
    if data.html and plain_text is None:
        plain_text = html_to_text(data.html)
    ...
    for attachment in data.attachments:
        maintype, subtype = split_mimetype(attachment.mimetype)
