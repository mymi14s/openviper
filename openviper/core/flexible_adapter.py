...
from openviper.conf.settings import settings as lazy_settings
...
def bootstrap_and_run(
    *,
    ...
    if resolved.is_root:
        prepare_root_layout(cwd)
    else:
        ensure_sys_path(cwd)

    if settings_module is None:
        settings_module = synthesize_settings_module()
        click.echo(
            "Warning: No settings.py found; using default settings.",
            err=True,
        )

    # Root-layout settings must be qualified so the importer resolves
    # them via the parent directory now on sys.path.
    ...
    inject_app_into_settings(resolved.app_label)
    ensure_models_imported(resolved)
    ...
    sys.exit(0)  # pragma: no cover  # pylint: disable=unreachable
...
def prepare_root_layout(cwd: Path) -> None:
    ...
    ensure_sys_path(cwd.parent)

    os.chdir(cwd.parent)
    logger.debug("Changed working directory to %s", cwd.parent)


def inject_app_into_settings(app_label: str) -> None:
...
def ensure_models_imported(resolved: ResolvedModule) -> None:
...
def synthesize_settings_module() -> str:
