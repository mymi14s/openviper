...
from openviper.db.migrations.writer import diff_states, normalize_state
from openviper.db.schemas.introspect import introspect_db_schema
from openviper.db.schemas.json_reader import discover_json_schemas
...
class Command(BaseCommand):
    ...
    def handle(self, **options) -> None:  # type: ignore[override]
        ...
        try:
            ...
            logger.warning("Background migration check failed", exc_info=True)
    ...
    def check_pending_migrations(self) -> None:
        ...
        try:
            ...
            async def get_pending() -> list[str]:
                desired_state = discover_json_schemas(resolved_apps=resolved_apps)
                if not desired_state:
                    return []
                table_names = list(desired_state.keys())
                actual_state = await introspect_db_schema(table_names=table_names)
                ops = diff_states(
                    normalize_state(desired_state),
                    normalize_state(actual_state),
                )
                if ops:
                    return [op.table_name if hasattr(op, "table_name") else str(op) for op in ops]
                return []
            ...
            logger.debug("Migration status check skipped", exc_info=True)
