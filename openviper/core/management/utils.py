...
from openviper.core.management.base import BaseCommand, CommandError, NoModelsModule
...
def import_models_module(app_name: str, app_path: str) -> object:
    """Import an app's models module, surfacing real import errors.

    Swallows only the case where the app has no ``models`` submodule.
    Any other failure (e.g. a broken import inside ``models.py``) is
    re-raised as a ``CommandError`` so silent model drops cannot happen.

    Args:
        app_name: Dotted app name (e.g. ``"core"`` or ``"myproj.core"``).
        app_path: Filesystem path to the app package, used as a fallback
            sys.path entry when the dotted import fails to resolve.

    Returns:
        The imported models module.

    Raises:
        NoModelsModule: If the app has no ``models`` submodule.
        CommandError: If the models module exists but raises during import.
    """
    models_dotted = f"{app_name}.models"
    try:
        return importlib.import_module(models_dotted)
    except ModuleNotFoundError as exc:
        if exc.name is not None and exc.name.startswith(models_dotted):
            try:
                sys.path.insert(0, app_path)
                return importlib.import_module(models_dotted)
            except ModuleNotFoundError as retry_exc:
                if retry_exc.name is not None and retry_exc.name.startswith(models_dotted):
                    raise NoModelsModule(app_name) from None
                raise CommandError(
                    f"Failed to import models for app '{app_name}': {retry_exc}"
                ) from retry_exc
            finally:
                if app_path in sys.path:
                    sys.path.remove(app_path)
        raise CommandError(
            f"Failed to import models for app '{app_name}': {exc}"
        ) from exc
...
def prompt_password(
    ...
    the confirmation prompt and meets the minimum length requirement.
    ...
    try:
        while True:
            ...
            if len(password) < MIN_CLI_PASSWORD_LENGTH:
                cmd.stderr(cmd.style_error(
                    f"Password must be at least {MIN_CLI_PASSWORD_LENGTH} characters."
                ))
                continue
