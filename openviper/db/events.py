...
---------------------
All thirteen ``Model`` lifecycle hooks are supported.
...
``pre_bulk_create``
    Fired before a bulk CREATE operation.
``post_bulk_create``
    Fired after a successful bulk CREATE operation.
``pre_bulk_update``
    Fired before a bulk UPDATE operation.
``post_bulk_update``
    Fired after a successful bulk UPDATE operation.
...
from openviper.db.utils import get_running_loop_or_none
        ...
        if event_map:
            ...
            if handlers:
                for handler in handlers:
                    try:
                        call_handler(handler, instance, event_name, **kwargs)
...
def is_safe_module_path(module_path: str) -> bool:
...
def resolve_dotted(path: str | Callable[..., Any]) -> Any | None:
...
def call_handler(handler: Any, instance: Any, event_name: str, **kwargs: Any) -> None:
...
def dispatch_decorator_handlers(
    model_path: str,
    event_name: str,
    instance: Any,
    **kwargs: Any,
