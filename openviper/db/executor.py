...
from openviper.db.dialects import get_dialect, resolve_dialect_by_vendor
...
def validate_regex_pattern(pattern: str) -> None:
    ...
    if len(pattern) > MAX_REGEX_LENGTH:
        raise FieldError(f"Regex pattern exceeds maximum length of {MAX_REGEX_LENGTH} characters.")
    if REDoS_PATTERN.search(pattern):
        raise FieldError(
            f"Regex pattern contains potentially catastrophic nested quantifiers: {pattern!r}"
        )
    if any(char in UNSAFE_REGEX_CHARS for char in pattern):
...
def redact_filters(
    ...
    for f in filters:
        ...
        for key, value in f.items():
            base = key.split("__")[0]
            entry[key] = "[REDACTED]" if base in SENSITIVE_FIELD_NAMES else repr(value)
...
def redact_values(values: dict[str, object]) -> dict[str, str]:
    ...
    for key, value in values.items():
        result[key] = "[REDACTED]" if key in SENSITIVE_FIELD_NAMES else repr(value)
    return result
...
def check_table_missing(exc: Exception, model_cls: type) -> None:
    ...
    """
    if isinstance(exc, (SAOperationalError, SAProgrammingError)) and TABLE_MISSING_RE.search(
        str(exc)
    ):
...
def extract_field_names(error_msg: str) -> list[str]:
    ...
    fields = []
    for match in FIELD_NAME_EXTRACT_RE.finditer(error_msg):
...
def check_field_missing(exc: Exception, model_cls: type) -> None:
    ...
    """
    if isinstance(exc, SADBAPIError) and FIELD_MISSING_RE.search(str(exc)):
...
async def get_real_columns(conn: Any, table_name: str) -> frozenset[str] | None:
    ...
    Delegates to the dialect-specific introspection module to generate the
    correct SQL for each database.  Results are cached for performance.
    """
    if table_name in _REAL_COLUMNS_CACHE:
        return _REAL_COLUMNS_CACHE[table_name]

    dialect = str(conn.engine.url.drivername).lower()
    dialect_key = dialect.split("+")[0]
    ...
    async with lock:
        ...
        dialect_obj = resolve_dialect_by_vendor(dialect_key)
        sql, params = dialect_obj.get_real_columns_sql(table_name)
        ...
        try:
            ...
            cols = {
                row[1] if dialect_key == "sqlite" else row[0]
...
def parse_traversal_cached(key: str, model_cls: type) -> Any:
    """Internal cached parser that returns TraversalLookup or TRAVERSAL_FAILURE.
...
def cached_traversal_lookup(key: str, model_cls: type) -> Any:
    ...
    *(key, model_cls)* pair.
    ...
    if result is TRAVERSAL_FAILURE:
        available = ", ".join(sorted(model_cls._fields))
        raise FieldError(
...
async def resolve_engine_for_alias(alias: str, *, write: bool = False) -> AsyncEngine:
    """Return the engine for *alias*, preserving default setup."""
    try:
...
def resolve_fk_table_name(field: ForeignKey | OneToOneField, model_cls: type) -> str:
    ...
    if target_model_cls and hasattr(target_model_cls, "_table_name"):
        meta = getattr(target_model_cls, "_meta", None)
        if meta is not None and getattr(meta, "abstract", False):
            target_str_check = field.to if isinstance(field.to, str) else ""
            if "auth" in target_str_check and "User" in target_str_check:
    ...
    if "." in target_str:
        ...
        return AUTH_TABLE_PLURALIZATION.get(related, related)
...
def build_table(table_name: str, model_cls: type) -> sa.Table:
    ...
    Keyed by ``(table_name, model_cls)``; the LRU cache ensures each
    ``(name, cls)`` pair is only ever built once.  Building the same
    table twice would raise a SQLAlchemy ``InvalidRequestError`` (table
    already in metadata).
    """
    ...
    for _name, field in cast("Any", model_cls)._fields.items():
        ...
        if isinstance(field, (ForeignKey, OneToOneField)):
            ...
            if related_table:
                fk_ref = f"{related_table}.id"
                ondelete = field.on_delete.replace("_", " ") if field.on_delete else None
                args.append(sa.ForeignKey(fk_ref, ondelete=ondelete))
        ...
        columns.append(col)
        added_columns.add(field.column_name)
...
model_registry.table_cache_callbacks.append(build_table.cache_clear)


def rebuild_all_tables_from_registry() -> None:
    """Remove all SQLAlchemy tables from metadata and rebuild from the registry.
    """
    metadata = get_metadata()
    ...
    for model_cls in model_registry.registry.values():
        ...
        if meta is not None and getattr(meta, "abstract", False):
            continue
        get_table(cast("type[Model]", model_cls))
...
def apply_lookup(
    ...
    if is_f_like(value):
        ...
        try:
            if lookup in ("in", "not_in", "range") and isinstance(value, (list, tuple)):
...
def compile_single_filter(
    ...
    if remaining:
        ...
        if remaining[-1] in OPERATORS:
            lookup = remaining.pop()
        elif remaining[-1] not in DATETIME_TRANSFORMS:
    ...
    for t in transform_parts:
        if t not in DATETIME_TRANSFORMS:
    ...
    async with connect(qs._db_alias, model_cls) as conn:
        ...
        for field_name, related_table in related_info:
            ...
            if real_cols is not None:
                real_cols = frozenset(c.lower() for c in real_cols)

            prefix = f"{field_name}__"
            for col in related_table.c:
                if real_cols is not None and col.name.lower() not in real_cols:
                    continue

                label = f"{prefix}{col.name}"
                if (
                    (only_fields and label not in only_fields and col.name != "id")
                    or (defer_fields and label in defer_fields)
                ):
                    continue
        ...
        # Oracle uppercases all identifiers, so introspection returns
        # UPPERCASE column names while the model defines lowercase.
        if base_real_cols is not None:
            base_real_cols = frozenset(c.lower() for c in base_real_cols)

        if only_fields:
            wanted: set[str] = {
                model_cls._fields[f].column_name
                if f in model_cls._fields
                else f
                for f in only_fields
            }
            wanted.add(
                model_cls._fields["id"].column_name
                if "id" in model_cls._fields
                else "id"
            )
            base_cols: list[Any] = [
                col
                ...
                and (base_real_cols is None or col.name.lower() in base_real_cols)
            ]
        elif defer_fields:
            deferred: set[str] = {
            ...
            base_cols = [
                ...
                if base_real_cols is None or col.name.lower() in base_real_cols
            ]
...
async def execute_save(
    ...
    # Combine the explicit flag with the ContextVar bypass.
...
async def execute_explain(qs: QuerySet) -> str:
    ...
    async with connect(qs._db_alias, model_cls) as conn:
        ...
        explain_stmt = sa.text(dialect_obj.explain_sql(str(compiled))[0])
