...
class CharField(Field):
    ...
    @property
    def column_type(self) -> str:
        """Return the SQL column type with length suffix for this field."""
        return f"{self._column_type}({self.max_length})"

    @column_type.setter
    def column_type(self, value: str) -> None:
        """Allow subclasses to override the base type name."""
        self._column_type = value
...
class ManyToManyManager:
    ...
    async def all(self) -> list[Any]:
        ...
        # Use prefetch cache to avoid a redundant DB round-trip.
        cached = self.instance.get_related(self.field.name)
...
class PasswordField(CharField):
    """One-way hashed password storage field (e.g. user passwords).

    Stores only the hashed representation in the database.  The original
    plaintext cannot be recovered.  Callers must hash via
    ``model.set_password()`` before assigning.

    The column length defaults to 255, sufficient for Argon2id and bcrypt
    hashes produced by :mod:`openviper.auth.hashers`.

    Args:
        min_length: Minimum plaintext password length (default: 4).
        max_length: Maximum plaintext password length (default: 128).
    """

    # Minimum length guards against trivially guessable passwords.
    _DEFAULT_MIN_LENGTH: int = 4
    # Maximum length prevents DoS via bcrypt which has a 72-byte limit,
    # and caps Argon2 memory cost for excessively long inputs.
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._plaintext_max = self._DEFAULT_MAX_LENGTH

    def to_python(self, value: Any) -> str | None:
        return str(value) if value is not None else None

    def validate(self, value: Any) -> None:
        # Hashed values already stored in the DB bypass plaintext checks.
        # A value is treated as hashed when it starts with a recognised
        # algorithm prefix written by openviper.auth.hashers.
        if value is not None and isinstance(value, str) and not (
            value.startswith("argon2$")
            or value.startswith("bcrypt$")
            or value.startswith("plain$")
        ):
            if len(value) < self.min_length:
                raise ValueError(
                    f"Field '{self.name}': password must be at least "
                    f"{self.min_length} characters."
                )
            if len(value) > self._plaintext_max:
                raise ValueError(
                    f"Field '{self.name}': password must not exceed "
                    f"{self._plaintext_max} characters."
                )
        super().validate(value)


class SensitiveField(CharField):
    """Encrypted secret storage field (e.g. API keys, tokens).

    Stores values encrypted with Fernet symmetric encryption using the
    project's ``SECRET_KEY``.  Unlike :class:`PasswordField`, the original
    plaintext can be retrieved via ``model.get_sensitive()``.

    The column length defaults to 512 to accommodate the Fernet token
    (which is larger than the plaintext due to padding and HMAC).

    Args:
        max_length: Maximum column length in the database.
    """

    _column_type = "VARCHAR"
    _ENCRYPTED_PREFIX: str = "enc$"
    _DEFAULT_MAX_LENGTH: int = 512

    def __init__(self, max_length: int = _DEFAULT_MAX_LENGTH, **kwargs: Any) -> None:
        kwargs.setdefault("null", True)
        super().__init__(max_length=max_length, **kwargs)

    @staticmethod
    def get_fernet_key() -> bytes:
        """Derive a 32-byte Fernet key from the project SECRET_KEY."""
        secret = getattr(settings, "SECRET_KEY", "")
        if not secret:
            raise RuntimeError(
                "SensitiveField requires settings.SECRET_KEY to be configured."
            )
        return base64.urlsafe_b64encode(hashlib.sha256(secret.encode()).digest())

    def encrypt(self, plaintext: str) -> str:
        """Encrypt *plaintext* and return a prefixed Fernet token string."""
        token = Fernet(self.get_fernet_key()).encrypt(plaintext.encode())
        return f"{self._ENCRYPTED_PREFIX}{token.decode()}"

    def decrypt(self, stored_value: str) -> str:
        """Decrypt a stored Fernet token and return the original plaintext."""
        if not stored_value.startswith(self._ENCRYPTED_PREFIX):
            raise ValueError(
                f"Field '{self.name}': stored value is not an encrypted token."
            )
        token = stored_value[len(self._ENCRYPTED_PREFIX):]
        try:
            return Fernet(self.get_fernet_key()).decrypt(token.encode()).decode()
        except InvalidToken as exc:
            raise ValueError(
                f"Field '{self.name}': failed to decrypt - invalid key or corrupted data."
            ) from exc
    ...
    def to_db(self, value: Any) -> str | None:
        ...
        if str_value.startswith(self._ENCRYPTED_PREFIX):
            return str_value
...
_LENGTH_SUFFIX_PATTERN: re.Pattern[str] = re.compile(r"\(\d+\)$")


def strip_length_suffix(column_type: str) -> str:
    """Strip a length suffix for use as a PostgreSQL array element type."""
    return _LENGTH_SUFFIX_PATTERN.sub("", column_type.strip())
...
class PostgresArrayBackend(BaseArrayBackend):
    ...
    def column_ddl(self, field: ArrayField) -> str:
        base_type = strip_length_suffix(field.base_column_type)
        return f"{base_type}[]"
    ...
    def get_sa_type(self, field: ArrayField) -> sa.types.TypeEngine[Any]:
        base_type = strip_length_suffix(field.base_column_type).upper()
...
_cached_array_backend: BaseArrayBackend | None = None


def get_array_backend() -> BaseArrayBackend:
    """Return the appropriate array backend based on the configured database."""
    global _cached_array_backend
    if _cached_array_backend is not None:
        return _cached_array_backend

    _cached_array_backend = (
        PostgresArrayBackend() if is_postgresql() else FallbackJsonBackend()
    )
...
class ArrayField(Field):
    ...
    Args:
        base_field: A :class:`Field` instance describing the element type
            (e.g. ``IntegerField()``, ``CharField(max_length=50)``).
