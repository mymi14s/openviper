...
class AlterColumn(Operation):
    ...
    def backward_sql(self) -> list[str]:
        ...
        if self.old_type and self.old_type != self.column_type:
            stmts.append(
                f"ALTER TABLE {quoted_table} ALTER COLUMN {quoted_column} TYPE {self.old_type}"
            )
...
def sort_migrations(migrations: list[MigrationRecord]) -> list[MigrationRecord]:
    ...
    queue = deque(
        sorted(
            ...
            key=lambda n: migrations.index(lookup[n]),
        )
    )
    ...
    while queue:
        ...
        neighbors = sorted(adj[curr], key=lambda n: migrations.index(lookup[n]))
    ...
    async def migrate(
        ...
        *,
        ...
        if verbose:
            ...
            print(f"DEBUG: Discovered migrations: {[m.name for m in all_migrations]}")
            print(f"DEBUG: Applied migrations: {applied}")
