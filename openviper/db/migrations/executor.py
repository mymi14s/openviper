...
def is_unconfigured_mock(value: object) -> bool:
...
# Use centralized implementations from openviper.db.utils.sql_validation
...
class CreateTable(Operation):
    ...
    def forward_sql(self) -> list[Any]:
        ...
        for col in self.columns:
            ...
            quoted_name = quote_identifier(col["name"], dialect)
            ...
            if col.get("target_table"):
                ...
                on_delete = validate_on_delete(
                    ...
                    f"CreateTable.{self.table_name}.{col['name']}",
                )
                if dialect == "sqlite":
                    # SQLite doesn't support ALTER TABLE ADD CONSTRAINT FOREIGN KEY,
                    definition += (
                        f" REFERENCES {quote_identifier(target, dialect)}(id) ON DELETE {on_delete}"
                    )
        ...
        for fk_col in fk_columns:
            ...
            quoted_idx = quote_identifier(idx_name, dialect)
            quoted_col = quote_identifier(fk_col, dialect)
    ...
    def deferred_fk_stmts(self) -> list[str]:
        ...
        for col in self.columns:
            ...
            quoted_col = quote_identifier(col_name, dialect)
            quoted_target = quote_identifier(target, dialect)
            quoted_constraint = quote_identifier(constraint_name, dialect)
            ...
            if dialect == "mssql":
                ...
                if dialect == "postgresql":
                    # Escape single quotes in the constraint name to prevent
                    # PL/pgSQL string-literal injection via crafted identifiers.
                    constraint_pg_escaped = constraint_name.replace("'", "''")
                    stmts.append(
                        f"DO $$ BEGIN\n"
                        f"  IF NOT EXISTS (\n"
                        f"    SELECT 1 FROM pg_constraint"
                        f" WHERE conname = '{constraint_pg_escaped}'\n"
                        f"  ) THEN\n"
...
class AddColumn(Operation):
    ...
    def forward_sql(self) -> list[str]:
        ...
        if self.default is not None:
            sql += f" DEFAULT {sql_literal(self.default)}"
        return [sql]
...
class CreateIndex(Operation):
    ...
    def forward_sql(self) -> list[str]:
        dialect = get_dialect()
        quoted_table = quote_identifier(self.table_name, dialect)
        quoted_index = quote_identifier(self.index_name, dialect)
        quoted_cols = ", ".join(quote_identifier(c, dialect) for c in self.columns)
...
def get_existing_columns_sync(connection: Any, table_name: str) -> set[str]:
    ...
    database backends.  Unrecognized column types (e.g. PostGIS
    ``geometry``) are tolerated - only the column names are needed.
    """
    try:
...
class MigrationExecutor:
    ...
    async def get_engine_for_alias(self, db_alias: str = "default") -> Any:
        ...
        Falls back to the legacy ``get_engine()`` for the default
        alias when the connections manager is not configured.
        """
        if db_alias == "default":
            try:
    ...
    async def migrate(
        ...
        *,
        ...
        Args:
            ...
            database: Database alias to run migrations on.  Defaults to
                ``'default'``.  Routers can control which aliases receive
                migrations via ``allow_migrate``.
