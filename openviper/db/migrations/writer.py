    ...
    *,
    ...
    for model_cls in sorted_models:
        ...
        if meta and getattr(meta, "abstract", False):
            continue
        # Virtual models are not backed by database tables.
...
def get_keyword_str(node: ast.Call, key: str) -> str | None:
...
def parse_operation(node: ast.AST, state: dict[str, dict[str, Any]]) -> None:
    ...
    if op_name == "CreateTable":
        parse_create_table(node, state)
    elif op_name == "CreateIndex":
        parse_create_index(node, state)
    elif op_name == "DropTable":
        table_name = get_keyword_str(node, "table_name")
        if table_name:
            state.pop(table_name, None)
    elif op_name == "AddColumn":
        parse_add_column(node, state)
    elif op_name == "RemoveColumn":
        parse_remove_column(node, state)
    elif op_name == "AlterColumn":
        parse_alter_column(node, state)
    elif op_name == "RemoveIndex":
        parse_remove_index(node, state)
    elif op_name == "RenameColumn":
        parse_rename_column(node, state)
    elif op_name == "RestoreColumn":
        parse_restore_column(node, state)
...
def parse_remove_index(node: ast.Call, state: dict[str, dict[str, Any]]) -> None:
    ...
    table_name = get_keyword_str(node, "table_name")
    index_name = get_keyword_str(node, "index_name")
...
def parse_add_column(node: ast.Call, state: dict[str, dict[str, Any]]) -> None:
    ...
    table_name = get_keyword_str(node, "table_name")
    column_name = get_keyword_str(node, "column_name")
...
def parse_rename_column(node: ast.Call, state: dict[str, dict[str, Any]]) -> None:
    ...
    table_name = get_keyword_str(node, "table_name")
    old_name = get_keyword_str(node, "old_name")
    new_name = get_keyword_str(node, "new_name")
    ...
    for table_name in sorted(current.keys() & existing.keys()):
        ...
        for col_name in sorted(added_col_names):
            ...
            if removed_name in old_cols:
                ...
                if not types_compatible(old_col.get("type", "TEXT"), new_col.get("type", "TEXT")):
        ...
        for col_name in sorted(added_col_names - restored):
            ...
            if was_soft:
                new_col = cur_cols[col_name]
                if not types_compatible(was_soft.get("type", "TEXT"), new_col.get("type", "TEXT")):
...
def write_migration(
    ...
    *,
    ...
    ops_str = "\n".join(format_operation(op) for op in operations)
    deps_str = repr(dependencies or [])
    timestamp = datetime.now(UTC).strftime("%Y-%m-%d %H:%M:%S")
