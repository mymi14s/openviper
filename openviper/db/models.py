...
class ModelMeta(type):
    ...
    def __new__(
        ...
        meta_index_together: list[list[str]] = []
        ...
        if meta:
            ...
            if hasattr(meta, "index_together"):
                it = meta.index_together
                if isinstance(it, (list, tuple)):
                    if it and isinstance(it[0], (list, tuple)):
                        meta_index_together = [list(item) for item in it]
                    else:
                        meta_index_together = [list(it)]
        ...
        for it_fields in meta_index_together:
            for f_name in it_fields:
                if f_name not in fields:
                    raise FieldError(f"index_together field '{f_name}' not found on {name}")
        ...
        namespace["_meta_index_together"] = meta_index_together
        ...
        for field_name, field_obj in list(fields.items()):
            if hasattr(field_obj, "contribute_to_class"):
...
class QuerySet:
    ...
    def raw_sql(self) -> str:
        ...
        only_fields: set[str] = set(self._only_fields)
        defer_fields: set[str] = set(self._defer_fields)
        ...
        if only_fields:
            ...
            } | {model_cls._fields["id"].column_name if "id" in model_cls._fields else "id"}
            cols = [col for col in table.c if col.name in wanted]
        elif defer_fields:
            deferred: set[str] = {
                model_cls._fields[f].column_name
                if f in model_cls._fields
            ...
            cols = [col for col in table.c if col.name not in deferred]
        else:
            cols = [table]

        stmt = sa.select(*cols) if cols else sa.select(table)

        q_filters = getattr(self, "_q_filters", [])
        where = build_where_clause(table, self._filters, self._excludes, q_filters)
        if where is not None:
            stmt = stmt.where(where)

        stmt = apply_order_limit_offset(stmt, self, table)
...
class Model(metaclass=ModelMeta):
    ...
    **Delete flow**::
    ...
    def get_sensitive(self, field_name: str) -> str | None:
        """Return the decrypted plaintext for a SensitiveField column.

        :class:`SensitiveField` stores values encrypted with Fernet symmetric
        encryption.  This method decrypts and returns the original plaintext,
        making it suitable for retrieving secrets like API keys or tokens.

        For :class:`PasswordField` (user passwords), the original plaintext
        cannot be recovered; use ``check_password()`` instead.

        Args:
            field_name: Name of the SensitiveField column.

        Returns:
            The decrypted plaintext string, or ``None`` when the field is empty.

        Raises:
            FieldError: *field_name* is not a SensitiveField on this model.
        """
        field = self._fields.get(field_name)
        if field is None:
            raise FieldError(f"{self.__class__.__name__} has no field '{field_name}'.")
        if not isinstance(field, SensitiveField):
            raise FieldError(
                f"Field '{field_name}' is not a SensitiveField; "
                f"use normal attribute access instead."
            )
        stored = self.__dict__.get(field_name)
        if stored is None:
            return None
        # In-memory instances may hold plaintext before save; only decrypt
        # values that carry the encrypted prefix written by to_db().
        if isinstance(stored, str) and stored.startswith(field._ENCRYPTED_PREFIX):
            return field.decrypt(stored)
        return str(stored)
