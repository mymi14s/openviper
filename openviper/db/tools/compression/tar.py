...
def create_tar_gz_sync(archive_path: Path, source_files: list[Path]) -> None:
...
async def extract_tar_gz(archive_path: Path, dest_dir: Path) -> list[Path]:
    ...
    *dest_dir* (prevents "tar-slip" / "zip-slip" attacks).
    ...
    return extract_tar_gz_sync(archive_path, dest_dir)


def extract_tar_gz_sync(archive_path: Path, dest_dir: Path) -> list[Path]:
