...
MISSING_SENTINEL = object()  # sentinel for getattr default

# Intent: Match "namespace:route_name" patterns to distinguish them from
...
def json_encode(
    ...
    *,
    ...
    if indent == 2:
        ...
        # Intent: orjson only supports indent=2; fall back to stdlib for other values.
...
class Response:
    ...
    def set_cookie(
        ...
        # Intent: Prevent CR/LF injection in cookie names and values.
        if "\r" in key or "\n" in key:
        ...
        if expires is not None:
            # Intent: Convert integer timestamps to RFC 2822 HTTP date format.
        ...
        # Intent: SameSite=None without Secure violates RFC 6265bis.
...
class JSONResponse(Response):
    ...
    def _default_encoder(obj: object) -> object:
        ...
        if isinstance(obj, uuid.UUID):
            return str(obj)
        # Intent: Delegate to model_dump() for JSON-safe serialisation of Pydantic models.
        if isinstance(obj, BaseModel):
            return obj.model_dump()
        # Intent: LazyFK wraps a raw FK id - serialise the underlying id value.
...
class HTMLResponse(Response):
    ...
    def _render_template(
        ...
        # Intent: Block path traversal attempts in template names.
        if ".." in template or template.startswith("/") or template.startswith("\\"):
            raise ValueError(f"Invalid template name: {template!r}")
        # Intent: Block Windows-style absolute paths (e.g. C:\ or \\?\).
        if len(template) >= 2 and template[1] == ":" and template[0].isalpha():
            raise ValueError(f"Invalid template name: {template!r}")
        # Intent: Fall back to settings.TEMPLATES_DIR when the default sentinel is used.
        ...
        # Intent: Auto-inject the current request into template context when absent.
...
class RedirectResponse(Response):
    ...
    def __init__(
        ...
        **path_params: str,
        ...
        # Intent: Block protocol-relative URLs that could redirect to external sites.
        stripped = url.lstrip()
        if stripped.startswith("//"):
            raise ValueError(f"Protocol-relative redirect URLs are not allowed: {url!r}")
        # Intent: Block path-traversal sequences in redirect targets.
        if ".." in url:
            raise ValueError(f"Redirect URL must not contain path traversal sequences: {url!r}")
        # Intent: Only allow http and https schemes in absolute redirect URLs.
...
class FileResponse(Response):
    ...
    async def __call__(self, scope: ASGIScope, receive: ASGIReceive, send: ASGISend) -> None:
        ...
        if self._filename:
            # Intent: Sanitise filename to prevent content-disposition header injection.
        ...
        # Intent: Conditional request - If-None-Match (ETag).
        ...
        # Intent: Conditional request - If-Modified-Since.
        ...
        # Intent: Range request - partial content (RFC 7233).
        ...
        if range_header and range_header.startswith("bytes="):
            ...
            try:
                ...
                if raw_start == "":
                    # Intent: Suffix range - bytes=-N means last N bytes.
                    suffix = int(raw_end)
                ...
                if range_start > range_end or range_start >= file_size:
                    # Intent: Range Not Satisfiable - respond with 416.
        ...
        async with await anyio.open_file(self.file_path, "rb") as f:
            ...
            if remaining > 0:
                # Intent: Send a final empty frame to close the response stream cleanly
...
class GZipResponse(Response):
    ...
    # Intent: Content types that are already compressed and should not be re-compressed.
    ...
    async def __call__(self, scope: ASGIScope, receive: ASGIReceive, send: ASGISend) -> None:
        ...
        # Intent: Skip compression for already-compressed content types or small bodies.
        ...
        if isinstance(inner, (StreamingResponse, FileResponse)):
            # Intent: Collect the full streamed body by intercepting ASGI events.
        ...
        if should_skip or len(body) < self._minimum_size:
            # Intent: Send uncompressed when content is already compressed or body is too small.
