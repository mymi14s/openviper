...
async def _get_user_cached(user_id: int) -> Any:
...
def _headers_as_dict(raw: list[tuple[bytes, bytes]]) -> dict[bytes, bytes]:
    """Build an O(1) bytes → bytes header map from the raw ASGI header list."""
...
class AuthenticationMiddleware(BaseMiddleware):
    ...
    async def _authenticate(self, scope: dict[str, Any]) -> tuple[Any, Any]:
        ...
        """
        # Build header map once — O(1) lookups for the rest of this method.
        headers = _headers_as_dict(scope.get("headers", []))
        ...
        if auth_str.startswith("Bearer "):
            ...
            try:
                ...
                if jti and await is_token_revoked(jti):
                    ...
                    if user_id:
                        user = await _get_user_cached(int(user_id))
