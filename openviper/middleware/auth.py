...
class AuthenticationMiddleware(BaseMiddleware):
    ...
    def __init__(self, app: ASGIApp) -> None:
        ...
        for auth_path in getattr(settings, "DEFAULT_AUTHENTICATION_CLASSES", []):
            try:
                auth_cls = cast("type[BaseAuthentication]", import_string(auth_path))
    ...
    async def __call__(self, scope: ASGIScope, receive: ASGIReceive, send: ASGISend) -> None:
        ...
        user, auth_info = await self.authenticate(request, authenticators)
    ...
    ) -> tuple[Authenticable, AuthPayload]:
