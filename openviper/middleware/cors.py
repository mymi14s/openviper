...
class CORSMiddleware(BaseMiddleware):
    ...
    def __init__(
        ...
        if max_age is None:
            max_age = int(getattr(settings, "CORS_MAX_AGE", 600))
        if allowed_origins is None:
            cfg_origins = getattr(settings, "CORS_ALLOWED_ORIGINS", None)
            if cfg_origins:
                allowed_origins = list(cfg_origins)
        if allowed_methods is None:
            cfg_methods = getattr(settings, "CORS_ALLOWED_METHODS", None)
            if cfg_methods:
                allowed_methods = list(cfg_methods)
        if allowed_headers is None:
            cfg_headers = getattr(settings, "CORS_ALLOWED_HEADERS", None)
            if cfg_headers:
                allowed_headers = list(cfg_headers)
        ...
        # Wildcard + credentials leaks private responses to any origin.
    ...
    async def __call__(self, scope: ASGIScope, receive: ASGIReceive, send: ASGISend) -> None:
        ...
        is_preflight = method.upper() == "OPTIONS"
        ...
        # Vary: Origin is required when the response depends on the request
        # Origin.  Without it, shared caches (CDNs) may serve a response with
        # wrong CORS headers to a different origin - a cache-poisoning vector.
