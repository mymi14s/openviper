...
def escape_html_attr(value: str) -> str:
    """Escape a value for safe inclusion in an HTML attribute (both quote styles)."""
...
def get_swagger_html(title: str, openapi_url: str) -> str:
    ...
    """
    safe_title = escape_html_attr(title)
    safe_url = escape_html_attr(openapi_url)
      ...
      requestInterceptor: function(request) {{
        ...
        for (var i = 0; i < cookies.length; i++) {{
          ...
          if (c.indexOf('csrftoken=') === 0) {{
            request.headers['X-CSRFToken'] = decodeURIComponent(c.substring(10));
            break;
          }}
        }}
        return request;
