...
class Serializer(BaseModel):
    ...
    def serialize_value(
        ...
        contrib_result = serialize_contrib_value(value)
        if contrib_result is not _CONTRIB_SENTINEL:
...
# Contrib field serializers register here so the core serializer can
# delegate value conversion without importing optional dependencies.
_CONTRIB_SERIALIZERS: dict[str, Callable[[Any], Any]] = {}


def register_contrib_serializer(type_name: str, fn: Callable[[Any], Any]) -> None:
    """Register a serializer function for a contrib field value type."""
    _CONTRIB_SERIALIZERS[type_name] = fn


def serialize_contrib_value(value: Any) -> Any | None:
    """Check registered contrib serializers for a matching value type."""
    for type_name, fn in _CONTRIB_SERIALIZERS.items():
        if type(value).__name__ == type_name:
            return fn(value)
    return _CONTRIB_SENTINEL


_CONTRIB_SENTINEL: object = object()
