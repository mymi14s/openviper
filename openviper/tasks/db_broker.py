...
class DbapiCursor(Protocol):
    """DB-API cursor operations used for SQLite setup."""
...
class DbapiConnection(Protocol):
    """DB-API connection operation used for SQLite setup."""
...
class DatabaseBroker(Broker):
    ...
    def __init__(self, *, middleware: list[object] | None = None) -> None:
    ...
    def _get_sync_engine(self) -> sa.engine.Engine:
        ...
        if sync_url.startswith("sqlite") and ":memory:" not in sync_url:
            ...
            def set_sqlite_wal(dbapi_conn: object, connection_record: object) -> None:
