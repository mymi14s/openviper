...
conf_module: types.ModuleType | None = None
if importlib.util.find_spec("openviper.conf") is not None:
    import openviper.conf as _conf_module  # noqa: F811

    conf_module = _conf_module
...
def configure_worker_logging(
    ...
    if log_to_file:
        tasks_logger.info(
            "Worker logging → %s  (level=%s, format=%s)",
            resolved,
            log_level,
            log_format,
        )
    else:
        tasks_logger.info("Worker logging → console only  (level=%s, log_to_file=False)", log_level)
    return resolved
...
def configure_worker_logging_from_settings() -> Path:
    ...
    try:
        loaded_settings = getattr(conf_module, "settings", None)
        task_settings: dict[str, object] = getattr(loaded_settings, "TASKS", {}) or {}
        raw_log_level = task_settings.get(
            "log_level",
            task_settings.get("LOG_LEVEL", getattr(loaded_settings, "LOG_LEVEL", "INFO")),
        )
        raw_log_format = task_settings.get(
            "log_format",
            task_settings.get("LOG_FORMAT", getattr(loaded_settings, "LOG_FORMAT", "text")),
        )
        log_level = str(raw_log_level)
        log_format = str(raw_log_format)
        raw_log_dir = task_settings.get("log_dir") or task_settings.get("LOG_DIR")
        log_dir = str(raw_log_dir) if raw_log_dir is not None else None
        log_to_file = bool(task_settings.get("log_to_file", False))
    except Exception as exc:
        logging.getLogger("openviper.tasks").warning(
            "Could not read task logging settings: %s", exc
        )
