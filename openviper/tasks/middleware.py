...
class StateObservationMiddleware(dramatiq_middleware.Middleware):
    ...
    def upsert_task_result(
        ...
        *,
        ...
        try:
            message_id = getattr(message, "message_id", uuid.uuid4())
            ...
            try:
                ...
                # Running in a worker thread (no running loop).  Use the
                # Dramatiq EventLoopThread so the DB engine shares the same
                # loop as the task coroutines, avoiding cross-loop errors.
                elt = get_event_loop_thread()
                if elt is not None and elt.loop.is_running():
                    future = asyncio.run_coroutine_threadsafe(persist(), elt.loop)
                    future.result(timeout=5.0)
                else:
                    # Fallback: create a throwaway loop only when no
                    # EventLoopThread is available (e.g. tests).
