...
if TYPE_CHECKING:
    ...
    from openviper.tasks.types import ActorProtocol, TaskValue
...
class ScheduleEntry:
    ...
    args: tuple[TaskValue, ...] = field(default_factory=tuple)
    #: Keyword arguments forwarded to ``actor.send()``.
    kwargs: dict[str, TaskValue] = field(default_factory=dict)
