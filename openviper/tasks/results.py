...
if TYPE_CHECKING:
    from openviper.tasks.types import TaskFields, TaskResultRow, TaskValue, UpsertFunction
...
_upsert_fn: UpsertFunction | None = None
_engine_lock = threading.Lock()
# Thread pool for async ops - sized for typical ASGI concurrency
_async_executor = ThreadPoolExecutor(
    max_workers=min(32, (os.cpu_count() or 1) * 4), thread_name_prefix="task_results_async"
)
...
def get_engine() -> sa.engine.Engine:
    ...
    with _engine_lock:
        ...
        engine_kwargs: dict[str, TaskValue] = {"pool_pre_ping": True}
        ...
        _engine = engine
        _upsert_fn = build_upsert_fn(engine.dialect.name)
...
def reset_engine() -> None:
    ...
    with _engine_lock:
        if _engine is not None:
            ...
            _engine = None
        _upsert_fn = None
    # Note: _async_executor is not shut down here as it's a module-level singleton
    # that may be used across multiple test cases. Call shutdown_async_executor()
    # explicitly in test teardown if needed.
...
def build_upsert_fn(dialect_name: str) -> UpsertFunction:
    ...
    if dialect_name == "postgresql":
        ...
        def pg_upsert(conn: sa.engine.Connection, message_id: str, fields: TaskFields) -> None:
            data = {
                "actor_name": "unknown",
                "queue_name": "unknown",
                "status": "pending",
            ...
            stmt = pg_insert(_table).values(**data)
            stmt = (
    ...
    if dialect_name == "sqlite":
        ...
        def sqlite_upsert(conn: sa.engine.Connection, message_id: str, fields: TaskFields) -> None:
            data = {
                "actor_name": "unknown",
                "queue_name": "unknown",
                "status": "pending",
            ...
            update_data = {k: v for k, v in fields.items() if k != "message_id"}
            stmt = sqlite_insert(_table).values(**data)
            stmt = (
                stmt.on_conflict_do_update(index_elements=["message_id"], set_=update_data)
                if update_data
                else stmt.on_conflict_do_nothing()
    ...
    if dialect_name in ("mysql", "mariadb"):
        ...
        def mysql_upsert(conn: sa.engine.Connection, message_id: str, fields: TaskFields) -> None:
            data = {
                "actor_name": "unknown",
                "queue_name": "unknown",
                "status": "pending",
            ...
            update_data = {k: v for k, v in fields.items() if k != "message_id"}
            stmt = mysql_insert(_table).values(**data)
            if update_data:
    ...
    def generic_upsert(conn: sa.engine.Connection, message_id: str, fields: TaskFields) -> None:
        row = conn.execute(
...
def upsert_result(message_id: str, **fields: TaskValue) -> None:
    ...
    try:
        with engine.begin() as conn:
            ...
            if upsert_fn is None:
                raise RuntimeError("Task result upsert function is not initialised.")
            upsert_fn(conn, message_id, fields)
    except Exception as exc:
        logger.warning("upsert_result failed for %s: %s", message_id, exc)


def batch_upsert_results(events: list[tuple[str, TaskFields]]) -> None:
    ...
    intended for use by :class:`~openviper.tasks.middleware.EventBuffer`.
    ...
    prepared: list[tuple[str, TaskFields]] = []
    for message_id, raw_fields in events:
        fields: TaskFields = dict(raw_fields)
        for key in ("args", "kwargs"):
            if key in fields and not isinstance(fields[key], str):
                try:
                    fields[key] = json.dumps(fields[key])
...
def get_task_result_sync(message_id: str) -> TaskResultRow | None:
...
async def get_task_result(message_id: str) -> TaskResultRow | None:
...
async def get_task_stats() -> dict[str, int]:
    ...
    await asyncio.sleep(0)
    with engine.connect() as conn:
        counts = {str(status): int(count) for status, count in conn.execute(stmt).fetchall()}
...
def row_to_dict(row: object) -> TaskResultRow:
    mapping = cast("Mapping[str, object]", row._mapping)
    d: TaskResultRow = dict(mapping)
    for key in ("args", "kwargs"):
        val = d.get(key)
        if isinstance(val, str):
            try:
                ...
                logger.debug("row_to_dict: could not parse %r column: %s", key, exc)
...
def setup_cleanup_task() -> None:
    ...
    This is called automatically when the worker starts if
    TASKS["cleanup_enabled"] is set to True.

    The task runs daily at 3 AM UTC and cleans results older than
    TASKS["cleanup_days"] (default: 7 days).
    """
    try:
        task_cfg: dict[str, object] = getattr(settings, "TASKS", {}) or {}
        cleanup_days_val = task_cfg.get("cleanup_days", 7)
        cleanup_days = (
            int(cleanup_days_val) if isinstance(cleanup_days_val, (int, float, str)) else 7
        )

        @periodic(cron="0 3 * * *", name="cleanup_old_task_results")
        ...
        logger.info(
            "Automatic task result cleanup enabled: daily at 3 AM UTC, "
            "removing results older than %d days",
            cleanup_days,
        )
    except Exception as exc:
