...
class CroniterInstance(Protocol):
    """Croniter object operation used by ``CronSchedule``."""

    def get_next(self, ret_type: type[datetime]) -> datetime: ...


CroniterFactory = Callable[[str, datetime], CroniterInstance]
...
def expand_cron_field(token: str, lo: int, hi: int) -> set[int]:
...
class CronSchedule(Schedule):
    ...
    def _parse(expr: str) -> dict[str, set[int]]:
        ...
        return {
            name: expand_cron_field(token, *_CRON_FIELD_RANGES[name])
    ...
    def _croniter_is_due(self, last_run_at: datetime | None, now: datetime) -> bool:
        try:
            ...
            if last_run_at.tzinfo is None:
                last_run_at = last_run_at.replace(tzinfo=UTC)
            croniter_factory = cast("CroniterFactory", croniter_lib.croniter)
            it = croniter_factory(self.expr, last_run_at)
...
try:
    croniter_lib: types.ModuleType | None = importlib.import_module("croniter")
    _HAS_CRONITER = True
except ImportError:
    croniter_lib = None
    _HAS_CRONITER = False
