...
:func:`run_worker` is the single entry-point called by the ``start-worker``
...
def discover_tasks(extra_modules: list[str] | None = None) -> list[str]:
    ...
    Uses parallel imports via ThreadPoolExecutor to speed up discovery.
    ...
    def import_module_result(module_path: str) -> tuple[str | None, str | None]:
        try:
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {executor.submit(import_module_result, path): path for path in module_paths}

        for future in as_completed(futures):
            module_path, _error = future.result()
            if module_path:
    ...
    imported.sort()
    logger.info(
        "Task discovery complete - %d modules imported, %d failed (parallelized with %d workers).",
        len(imported),
        failed,
        max_workers,
    )
...
def run_worker(
    ...
    # A non-empty TASKS dict means the task system is enabled.
    if not task_cfg:
        logger.warning(
            "Task worker is disabled. Add a TASKS dict to your settings to start the worker."
        )
        return
    ...
    scheduler_enabled: bool = bool(task_cfg.get("scheduler", True))
    ...
    if scheduler_enabled:
        ...
        logger.info("Periodic scheduler disabled (set TASKS['scheduler'] = False to disable).")
