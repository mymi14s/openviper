...
def render_to_string(template_name: str, context: dict[str, object] | None = None) -> str:
    """Render a template to a string.

    Resolves search paths from ``settings.TEMPLATES_DIR`` and
    ``templates/`` folders in ``settings.INSTALLED_APPS``.
