...
async def body_receive(body: bytes = b"") -> dict[str, object]:
    """Return a single http.request message with the given body."""
    return {"type": "http.request", "body": body, "more_body": False}


def make_receive(body: bytes = b"") -> object:
...
class MockQuerySet:
    ...
    def exclude(self, **kwargs: object) -> MockQuerySet:
        return self
    ...
    async def _aiter_items(self) -> AsyncIterator[object]:
        for item in self._items:
            yield item

    async def batch(self, size: int = 25) -> AsyncIterator[list[object]]:
...
def echo_app(status: int = 200, body: bytes = b"OK") -> object:
