...
from openviper.core.context import current_request, request_perms_cache
from openviper.db.connection import close_db, get_metadata
from openviper.db.events import _background_tasks
from openviper.http.request import Request
from openviper.http.response import JSONResponse
from tests.factories.db import create_test_engine
from tests.integration._credentials import ADMIN_PASSWORD, USER_PASSWORD
...
async def admin_user(test_database) -> User:
    ...
    await user.set_password(ADMIN_PASSWORD)
    await user.save()
    return user
...
async def user_with_role(test_database, test_cache) -> tuple[User, Role]:
    ...
    await drain_tasks()  # drain Permission.after_insert background tasks before add()
    ...
    await drain_tasks()  # drain RolePermission.after_insert background tasks
...
@pytest.fixture
async def sample_test_data(test_database, test_cache) -> dict[str, object]:
    ...
    for codename in ["add_user", "change_user", "delete_user", "view_user"]:
        ...
        await perm.save()
        await drain_tasks()  # drain Permission.after_insert background tasks
