...
class TestPeriodicIntegration:
    ...
    def test_periodic_registers_cron_job(self) -> None:
        ...
        registry = Registry()
        assert cron_health.periodic_config["name"] in registry.periodic_jobs
        entry = registry.periodic_jobs[cron_health.periodic_config["name"]]
        assert entry["cron"] == "*/5 * * * *"
    ...
    def test_periodic_registers_interval_job(self) -> None:
        ...
        registry = Registry()
        assert interval_health.periodic_config["name"] in registry.periodic_jobs
        entry = registry.periodic_jobs[interval_health.periodic_config["name"]]
    ...
    def test_periodic_dedup_is_automatic(self) -> None:
        ...
        assert "singleton" not in auto_dedup_job.periodic_config
