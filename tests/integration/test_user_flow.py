...
from openviper.auth.models import Permission, Role, User, request_perms_cache
...
from tests.integration.conftest import drain_tasks
...
class TestUserCreationFlow:
    ...
    async def test_create_user_with_password(self, test_database):
        ...
        await user.set_password(USER_PASSWORD)
        await user.save()
...
class TestUserAuthenticationFlow:
    ...
    async def test_password_verification(self, test_database, admin_user: User):
        ...
        is_valid = await check_password(ADMIN_PASSWORD, admin_user.password)
        assert is_valid is True

        is_valid = await check_password(WRONG_PASSWORD, admin_user.password)
...
class TestUserPermissions:
    ...
    async def test_user_permissions_from_roles(
        ...
        suffix = uuid.uuid4().hex[:8]
        perm1 = Permission(codename=f"view_content_{suffix}", name="Can view content")
        await perm1.save()
        await drain_tasks()

        perm2 = Permission(codename=f"edit_content_{suffix}", name="Can edit content")
        await perm2.save()
        await drain_tasks()

        role = Role(name=f"content_editor_{suffix}", description="Content editor")
        await role.save()
        await drain_tasks()
        ...
        user = User(username=f"contentuser_{suffix}", email=f"content_{suffix}@example.com")
        await user.save()
        await drain_tasks()
        ...
        assert f"view_content_{suffix}" in user_permissions
        assert f"edit_content_{suffix}" in user_permissions
    ...
    async def test_permission_caching(
        ...
        suffix = uuid.uuid4().hex[:8]
        perm = Permission(codename=f"cached_perm_{suffix}", name="Cached permission")
        await perm.save()
        await drain_tasks()

        role = Role(name=f"cached_role_{suffix}", description="Cached role")
        await role.save()
        await drain_tasks()

        await role.permissions.add(perm)

        user = User(username=f"cacheuser_{suffix}", email=f"cache_{suffix}@example.com")
        await user.save()
        await drain_tasks()
    ...
    async def test_permissions_from_multiple_roles(
        ...
        suffix = uuid.uuid4().hex[:8]
        perm1 = Permission(codename=f"read_data_{suffix}", name="Can read data")
        await perm1.save()
        await drain_tasks()

        perm2 = Permission(codename=f"write_data_{suffix}", name="Can write data")
        await perm2.save()
        await drain_tasks()

        perm3 = Permission(codename=f"delete_data_{suffix}", name="Can delete data")
        await perm3.save()
        await drain_tasks()

        reader_role = Role(name=f"reader_{suffix}", description="Reader")
        await reader_role.save()
        await drain_tasks()
        await reader_role.permissions.add(perm1)

        writer_role = Role(name=f"writer_{suffix}", description="Writer")
        await writer_role.save()
        await drain_tasks()
        await writer_role.permissions.add(perm1)
        await writer_role.permissions.add(perm2)

        admin_role = Role(name=f"data_admin_{suffix}", description="Data admin")
        ...
        user = User(username=f"multiperms_{suffix}", email=f"multi_{suffix}@example.com")
        await user.save()
        await drain_tasks()
        ...
        assert f"read_data_{suffix}" in permissions
        assert f"write_data_{suffix}" in permissions
        assert f"delete_data_{suffix}" not in permissions

        await user.roles.add(admin_role)
        # bulk_create does not fire after_insert events, so caches must be
        # cleared manually before re-fetching permissions.
        await test_cache.delete(f"user_perms:{user.pk}")
        req_cache = dict(request_perms_cache.get() or {})
        req_cache.pop(user.pk, None)
        request_perms_cache.set(req_cache)
        permissions = await user.get_permissions()
        assert f"delete_data_{suffix}" in permissions
