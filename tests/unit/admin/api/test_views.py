...
from openviper.admin.api.views import (
    ...
    get_admin_router,
    is_auth_user_model,
    serialize_instance_with_children,
)
...
class TestAdminAPIViews:
    ...
    @pytest.mark.asyncio
    async def testbatch_load_children(self):
        ma = MagicMock()
        inline = MagicMock()
        ...
        with patch.object(MockModel.objects, "filter") as mock_filter:
            ...
            res = await batch_load_children(ma, MockModel, [inst])
            assert 1 in res
...
def make_request(method="GET", path="/", user=None, body=None):
...
def make_user(username="testuser", is_staff=True, is_superuser=False):
    """Create a mock user."""
    user = MagicMock()
...
def make_model_class(name="TestModel", app_name="test"):
...
class TestIsAuthUserModel:
    """Test is_auth_user_model helper function."""
    ...
    def test_returns_true_for_user_model(self):
        ...
        with patch("openviper.admin.api.views.User") as mock_user:
            ...
            result = is_auth_user_model(mock_model)
    ...
    def testis_auth_user_model_handles_user_and_non_user_models(self):
        """Test is_auth_user_model handles user and non-user models correctly."""
        ...
        with patch("openviper.admin.api.views.get_user_model", mock_get_user_model):
            ...
            # Assuming is_auth_user_model returns True for user models and False otherwise
            assert is_auth_user_model(mock_user_model) is True
            assert is_auth_user_model(mock_non_user_model) is False
...
class TestViewHelperFunctions:
    ...
    def testis_auth_user_model_with_different_models(self):
        """Test is_auth_user_model with various models."""
        ...
        assert isinstance(is_auth_user_model(model1), bool)
        assert isinstance(is_auth_user_model(model2), bool)
...
@pytest.mark.asyncio
async def testserialize_instance_with_children_with_preloaded():
    request = MagicMock()
    ...
    result = await serialize_instance_with_children(
...
@pytest.mark.asyncio
async def testserialize_instance_with_children_without_preloaded():
    request = MagicMock()
    model_admin = MagicMock()
    model_class = MagicMock()
    instance = MagicMock(id=1)
    ...
    result = await serialize_instance_with_children(request, model_admin, model_class, instance)
...
class TestBatchLoadChildren:
    """Test batch_load_children helper function."""
    ...
    async def test_no_instances(self):
        ...
        result = await batch_load_children(model_admin, model_class, instances)
        assert result == {}
    ...
    async def test_with_child_tables(self):
        ...
        # Use a real class with _fields so getattr returns a real dict.
        child_fields_dict = {"id": MagicMock(), "name": MagicMock()}

        class ChildModel:
            _fields = child_fields_dict
            __name__ = "ChildModel"

        ChildModel.objects = MagicMock()
        inline_mock.model = ChildModel
...
class TestSerializeInstanceWithChildren:
    """Test serialize_instance_with_children helper function."""
    ...
    async def test_isoformat_value_serialized(self):
        ...
        with patch("openviper.admin.api.views.is_auth_user_model", return_value=False):
            result = await serialize_instance_with_children(
...
def find_handler(router, path, method=None):
...
class TestBatchLoadChildrenEdgeCases:
    """Edge cases for batch_load_children."""
    ...
    async def test_auto_fk_discovery_via_field_introspection(self):
        ...
        fk_field.__class__.__name__ = "ForeignKey"
        fk_field.resolve_target.return_value = model_class
        fk_field.column_name = "parent_id"
        ...
        # Use a real class with _fields so getattr returns a real dict.
        child_fields_dict = {"parent": fk_field, "value": MagicMock()}
        ...
        result = await batch_load_children(model_admin, model_class, [instance])
        assert 1 in result
    ...
    async def test_no_fk_found_skips_inline(self):
        ...
        class ChildModelNoFK:
            _fields = {}
            __name__ = "ChildModelNoFK"

        inline_mock.model = ChildModelNoFK
    ...
    async def test_child_isoformat_serialized(self):
        ...
        child_fields_dict = {"created_at": MagicMock()}
    ...
    async def test_child_non_primitive_coerced_to_str(self):
        ...
        child_fields_dict = {"status": MagicMock()}

        class ChildModelStatus:
            _fields = child_fields_dict

        ChildModelStatus.objects = MagicMock()
        inline_mock.model = ChildModelStatus
        ...
        result = await batch_load_children(model_admin, model_class, [instance])
        assert result[1]["childmodelstatus_set"][0]["status"] == "ok"
...
class TestLoginErrors:
    ...
    async def test_missing_credentials_raises(self, router, mock_request):
        handler = find_handler(router, "/auth/login/", "POST")
...
class TestLogoutEdgeCases:
    ...
    async def test_logout_body_parse_failure_still_succeeds(self, router, mock_request):
        handler = find_handler(router, "/auth/logout/", "POST")
...
class TestRefreshTokenErrors:
    ...
    async def test_missing_refresh_token_raises(self, router, mock_request):
        handler = find_handler(router, "/auth/refresh/", "POST")
        mock_request.json.return_value = {}
        with pytest.raises(ValidationError):
            await handler(mock_request)
...
class TestChangePassword:
    ...
    def handler(self, router):
        return find_handler(router, "/auth/change-password/", "POST")
...
class TestDashboardCoverage:
    ...
    async def test_dashboard_with_activity_records(self, router, mock_request):
        handler = find_handler(router, "/dashboard/", "GET")
        activity = MagicMock()
        activity.id = 1
...
class TestListModelsCoverage:
    ...
    async def test_model_filtered_by_permission(self, router, mock_request):
        handler = find_handler(router, "/models/", "GET")
        with (
            patch("openviper.admin.api.views.admin") as mock_admin,
            patch("openviper.admin.api.views.check_model_permission", return_value=False),
        ):
...
class TestGetModelConfigErrors:
    ...
    async def test_not_registered_raises_not_found(self, router, mock_request):
        handler = find_handler(router, "/models/{app_label}/{model_name}/", "GET")
        with patch("openviper.admin.api.views.admin") as mock_admin:
...
class TestListInstancesByAppCoverage:
    ...
    async def test_search_applies_filter(self, list_app_handler, mock_request):
        ...
        with (
            patch("openviper.admin.api.views.admin") as mock_admin,
            patch("openviper.admin.api.views.check_model_permission", return_value=True),
            patch("openviper.admin.api.views.is_auth_user_model", return_value=False),
        ):
            ma = MagicMock()
            ma.list_per_page = 10
    ...
    async def test_ordering_param_applied(self, list_app_handler, mock_request):
        ...
        mc = MagicMock()
        mc.__name__ = "MyModel"
        mc._fields = {"created_at": make_field_mock()}
        mc.objects = MagicMock()
        mc.objects.all.return_value = qs
...
class TestCreateInstanceByAppErrors:
    @pytest.fixture
    def handler(self, router):
        return find_handler(router, "/models/{app_label}/{model_name}/", "POST")
...
class TestLegacyListInstances:
    @pytest.fixture
    def handler(self, router):
        return find_handler(router, "/models/{model_name}/", "GET")
...
class TestLegacyCreateInstance:
    @pytest.fixture
    def handler(self, router):
        return find_handler(router, "/models/{model_name}/", "POST")
...
class TestLegacyBulkDelete:
    @pytest.fixture
    def handler(self, router):
        return find_handler(router, "/models/{model_name}/bulk-delete/", "POST")
...
class TestLegacyBulkAction:
    @pytest.fixture
    def handler(self, router):
        return find_handler(router, "/models/{model_name}/bulk-action/", "POST")
...
class TestSearchInstances:
    @pytest.fixture
    def handler(self, router):
        return find_handler(router, "/models/{model_name}/search/", "GET")
...
class TestGetFilterOptionsCoverage:
    @pytest.fixture
    def handler(self, router):
        return find_handler(router, "/models/{model_name}/filters/", "GET")
...
class TestLegacyExportInstances:
    @pytest.fixture
    def handler(self, router):
        return find_handler(router, "/models/{model_name}/export/", "POST")
...
class TestLegacyGetInstanceHistory:
    @pytest.fixture
    def handler(self, router):
        return find_handler(router, "/models/{model_name}/{obj_id}/history/", "GET")
...
class TestFkSearchFallbackPaths:
    ...
    async def test_fallback_to_importlib(self, handler, mock_request):
        """When model is found via admin registry, fk_search returns items."""
        mock_request.query_params = {"q": "test"}
        mc = MagicMock()
    ...
    async def test_fallback_to_all_registered_models(self, handler, mock_request):
        """fk_search returns items when model is found in registry."""
...
class TestGlobalSearchEdgeCases:
    @pytest.fixture
    def handler(self, router):
        return find_handler(router, "/search/", "GET")
...
class TestInlineCreateAndUpdate:
    ...
    async def test_create_instance_inline_parsing(self, router, mock_request, mock_engine):
        handler = find_handler(router, "/models/{app_label}/{model_name}/", "POST")
    ...
    async def test_update_instance_inline_parsing(self, router, mock_request, mock_engine):
        handler = find_handler(router, "/models/{app_label}/{model_name}/{obj_id}/", "PUT")
...
class TestOtherExceptionEdges:
    ...
    async def test_list_instances_by_app_not_registered(self, router, mock_request):
        handler = find_handler(router, "/models/{app_label}/{model_name}/list/", "GET")
        with patch("openviper.admin.api.views.admin") as mock_admin:
    ...
    async def test_legacy_create_instance_value_error(self, router, mock_request, mock_engine):
        handler = find_handler(router, "/models/{model_name}/", "POST")
    ...
    async def test_legacy_update_instance_value_error(self, router, mock_request, mock_engine):
        handler = find_handler(router, "/models/{model_name}/{obj_id}/", "PATCH")
        mock_request.json.return_value = {"bad_field": "val"}
        mc = MagicMock()
        mc.__name__ = "MC"
        mc._fields = {"bad_field": MagicMock()}
    ...
    async def test_legacy_bulk_delete_errors(self, router, mock_request):
        handler = find_handler(router, "/models/{model_name}/bulk-delete/", "POST")
    ...
    async def test_legacy_bulk_action_errors(self, router, mock_request):
        handler = find_handler(router, "/models/{model_name}/bulk-action/", "POST")
    ...
    async def test_legacy_export_instances_not_registered(self, router, mock_request):
        handler = find_handler(router, "/models/{model_name}/export/", "POST")
        with patch("openviper.admin.api.views.admin") as mock_admin:
            mock_admin.get_model_admin_by_name.side_effect = NotRegistered()
            with pytest.raises(NotFound):
                await handler(mock_request, model_name="x")
    ...
    async def test_legacy_get_filter_options_not_registered(self, router, mock_request):
        handler = find_handler(router, "/models/{model_name}/filters/", "GET")
        with patch("openviper.admin.api.views.admin") as mock_admin:
            mock_admin.get_model_admin_by_name.side_effect = NotRegistered()
            with pytest.raises(NotFound):
                await handler(mock_request, model_name="x")
    ...
    async def test_legacy_get_instance_history_no_permission(self, router, mock_request):
        handler = find_handler(router, "/models/{model_name}/{obj_id}/history/", "GET")
        with patch("openviper.admin.api.views.admin") as mock_admin:
...
class TestAdminSecurityFixes:
    ...
    async def test_serialize_uses_model_admin_sensitive_fields(self):
        """serialize_instance_with_children uses model_admin.get_sensitive_fields(),
        ...
        request = MagicMock()
        result = await serialize_instance_with_children(request, ma, model_class, instance)
    ...
    async def test_list_filter_rejects_unknown_field(self, router, mock_request):
        ...
        handler = find_handler_by_path_method(
    ...
    async def test_bulk_delete_rejects_more_than_1000_ids(self, router, mock_request):
        ...
        handler = find_handler_by_path_method(router, "/models/{model_name}/bulk-delete/", "POST")
        assert handler is not None
    ...
    async def test_delete_not_found_uses_obj_id(self, router, mock_request):
        ...
        handler = find_handler_by_path_method(router, "/models/{model_name}/{obj_id}/", "DELETE")
        assert handler is not None
...
class TestAdminAccessPermissionBranches:
    ...
    async def test_create_instance_by_app_requires_admin_access(self, router, mock_request):
        ...
        handler = find_handler_by_path_method(router, "/models/{app_label}/{model_name}/", "POST")
        assert handler is not None
