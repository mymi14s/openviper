...
async def test_admin_refresh_token(monkeypatch):
    monkeypatch.setattr(views, "decode_refresh_token_checked",
        AsyncMock(return_value={"sub": 1, "jti": "j"}))
...
async def test_admin_current_user(monkeypatch):
    monkeypatch.setattr("openviper.admin.api.permissions.require_admin_access", allow_admin_access)
    req = MagicMock()
...
async def test_list_by_app_with_filters(monkeypatch):
    ...
    """
    monkeypatch.setattr("openviper.admin.api.permissions.require_admin_access",
...
async def test_create_by_app_readonly_field_skip(monkeypatch):
    ...
    monkeypatch.setattr(
        views, "serialize_instance_with_children", AsyncMock(return_value={"id": 1})
    )
