...
class TestGetFieldWidgetConfig:
    ...
    def test_required_field(self):
        field = make_field("CharField", null=False, blank=False)
        config = get_field_widget_config(field)
        assert config["required"] is True

    def test_optional_field(self):
        field = make_field("CharField", null=True, blank=True)
        config = get_field_widget_config(field)
        assert config["required"] is False

    def test_auto_field_readonly(self):
        field = make_field("AutoField")
        config = get_field_widget_config(field)
        assert config["readonly"] is True

    def test_help_text(self):
        field = make_field("CharField", help_text="Enter your name")
        config = get_field_widget_config(field)
        assert config["help_text"] == "Enter your name"

    def test_char_field_max_length(self):
        field = make_field("CharField", max_length=255)
        config = get_field_widget_config(field)
        assert config["max_length"] == 255

    def test_choices_configuration(self):
        choices = [("A", "Option A"), ("B", "Option B")]
        field = make_field("CharField", choices=choices)
        config = get_field_widget_config(field)
...
class TestGetFieldSchema:
    ...
    def test_foreign_key_field_with_string_reference(self):
        field = make_field("ForeignKey", to="auth.User")
        schema = get_field_schema(field)
    ...
    def test_foreign_key_field_with_model_class(self):
        ...
        field = make_field("ForeignKey", to=mock_model)
        schema = get_field_schema(field)
    ...
    def test_foreign_key_with_module_string(self):
        ...
        field = make_field("ForeignKey", to="auth.models.TestModel")
    ...
    def test_foreign_key_with_module_string_fallback(self):
        ...
        field = make_field("ForeignKey", to="auth.sub.TestModel")
...
class TestCoerceFieldValue:
    ...
    def test_foreign_key_coercion_empty_str(self):
        field = make_field("ForeignKey", null=False)  # Not nullable but empty string
...
class TestFieldSchemaEdgeCases:
    ...
    def test_many_to_many_field_component(self):
        field = make_field("ManyToManyField")
        component = get_field_component_type(field)
        assert component == "multiselect"

    def testserialize_default_none_direct(self):
        assert serialize_default(None) is None

    def testserialize_default_unserializable_dict(self):
        class Unserializable:
            pass

        assert "Unserializable" in serialize_default({"k": Unserializable()})

    def testserialize_default_unsupported_object(self):
        class CustomObj:
            def __str__(self):
                return "custom_str"

        assert serialize_default(CustomObj()) == "custom_str"
