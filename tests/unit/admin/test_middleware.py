...
class TestCheckAdminAccess:
    ...
    def test_staff_user_has_access(self):
        ...
        request = MagicMock()
        request.user = make_user(is_authenticated=True, is_staff=True)
        assert check_admin_access(request) is True

    def test_superuser_has_access(self):
        """Test that superusers have access."""
        request = MagicMock()
        request.user = make_user(is_authenticated=True, is_superuser=True)
        assert check_admin_access(request) is True

    def test_regular_user_no_access(self):
        """Test that regular users don't have access."""
        request = MagicMock()
        request.user = make_user(is_authenticated=True, is_staff=False)
...
class TestCheckModelPermission:
    ...
    def test_regular_user_no_permissions(self):
        ...
        request = MagicMock()
        request.user = make_user(is_authenticated=True, is_staff=False, has_perm=False)
        model_class = MagicMock()
