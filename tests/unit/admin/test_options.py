...
def make_user(is_authenticated=True, is_staff=False, is_superuser=False):
    """Create a mock user."""
    user = MagicMock()
...
def make_model_class(name="TestModel", app_name="test", fields=None):
...
def make_field(name, field_type="CharField"):
    """Create a mock field."""
    field = MagicMock()
    field.name = name
...
class TestListViewMethods:
    ...
    def test_get_list_display_default(self):
        ...
        fields = {"name": make_field("name"), "email": make_field("email")}
        model = make_model_class(fields=fields)
        admin = ModelAdmin(model)
    ...
    def test_get_list_select_related_boolean_true(self):
        ...
        fk_field = make_field("author", "ForeignKey")
        fields = {"author": fk_field, "title": make_field("title")}
        model = make_model_class(fields=fields)
        admin = ModelAdmin(model)
...
class TestFormViewMethods:
    ...
    def test_get_fields_default(self):
        ...
        fields_dict = {"name": make_field("name"), "email": make_field("email")}
        model = make_model_class(fields=fields_dict)
        admin = ModelAdmin(model)
    ...
    def test_get_sensitive_fields_default_logic(self):
        ...
        fields_dict = {
            ...
            "user_password": make_field("user_password"),
            "auth_token": make_field("auth_token"),
            "api_key": make_field("api_key"),
        }
        model = make_model_class("User", fields=fields_dict)
        admin = ModelAdmin(model)
    ...
    def test_get_fieldsets_default(self):
        ...
        fields_dict = {"name": make_field("name")}
        model = make_model_class(fields=fields_dict)
        admin = ModelAdmin(model)
...
class TestActionMethods:
    ...
    async def testaction_delete_selected(self):
        ...
        request = make_request()
        count = await admin.action_delete_selected(request, queryset)
...
class TestModelInfoMethod:
    ...
    def test_get_model_info_structure(self):
        ...
        fields_dict = {"name": make_field("name")}
        model = make_model_class("User", "auth", fields_dict)
        admin = ModelAdmin(model)
    ...
    def test_get_model_info_sensitive_fields_excluded(self):
        ...
        fields_dict = {"name": make_field("name"), "password": make_field("password")}
        model = make_model_class(fields=fields_dict)
        admin = ModelAdmin(model)
...
class TestInlineModelAdmin:
    ...
    def test_initialization(self):
        ...
        parent_model = make_model_class("Parent")
        child_model = make_model_class("Child")
...
class TestChildTable:
    ...
    def test_get_child_tables_info(self):
        parent_model = make_model_class()
        child_model = make_model_class("Child")
        child_model._fields = {"id": make_field("id"), "name": make_field("name")}
