...
def make_field(name, field_type="CharField", **kwargs):
    field = MagicMock()
    field.name = name
...
def make_model_class(fields=None):
    model = MagicMock()
...
async def test_save_model_allows_readonly_in_create():
    ...
    fields = {
        "id": make_field("id", "IntegerField", primary_key=True, auto_increment=True),
        "slug": make_field("slug", "CharField"),
        "name": make_field("name", "CharField"),
    }
    model_cls = make_model_class(fields=fields)
...
async def test_save_model_always_blocks_intrinsic_readonly():
    ...
    fields = {
        "id": make_field("id", "IntegerField", primary_key=True, auto_increment=True),
        "created_at": make_field("created_at", "DateTimeField", auto_now_add=True),
        "name": make_field("name", "CharField"),
    }
    model_cls = make_model_class(fields=fields)
...
def test_get_fields_excludes_non_editable_on_create():
    ...
    fields = {
        "id": make_field("id", "IntegerField", primary_key=True, auto_increment=True),
        "name": make_field("name", "CharField"),
        "internal_code": make_field("internal_code", "CharField", editable=False),
    }
    model_cls = make_model_class(fields=fields)
    admin = ModelAdmin(model_cls)
