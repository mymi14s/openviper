...
class TestModerate:
    ...
    async def test_unparseable_response_returns_safe_fallback(self):
        ...
        # Fail-closed: unparseable AI responses must not be marked safe
        assert result["is_safe"] is False
    ...
    @pytest.mark.asyncio
    async def test_low_confidence_hate_is_not_safe(self):
        """Low-confidence hate classification must not be marked safe."""
        raw = json.dumps(
            {
                "classification": "hate",
                "confidence": 0.3,
        ...
        p = ModProvider()
        result = await p.moderate("test")
        # Fail-closed: hate classification is never safe regardless of confidence
        assert result["is_safe"] is False
