...
class TestRegistryLoadFromSettings:
    ...
    def test_load_from_settings_infers_provider_type(self):
        ...
        with patch("openviper.ai.registry.settings", mock_settings):
            with patch("openviper.ai.registry.resolve_provider_class") as mock_resolve:
                mock_cls = MagicMock()
...
class TestResolveProviderClass:
    ...
    def test_resolve_from_cache(self):
        """Test resolve_provider_class uses cache."""

        # First call populates cache
        cls = resolve_provider_class("openai")
        assert cls is not None

        # Second call should use cache
        cls2 = resolve_provider_class("openai")
        assert cls2 is cls

    def test_resolve_unknown_returns_none(self):
        """Test resolve_provider_class returns None for unknown."""

        result = resolve_provider_class("completely_unknown_provider")
        assert result is None

    def test_resolve_handles_import_error(self):
        """Test resolve_provider_class handles ImportError in fallback."""

        original_cache = registry.PROVIDER_CLASS_CACHE.copy()

        try:
            registry.PROVIDER_CLASS_CACHE.clear()

            with patch.dict("sys.modules", {"openviper.ai.providers.openai_provider": None}):
                with patch("importlib.import_module", side_effect=ImportError("no module")):
                    result = registry.resolve_provider_class("openai")
                    assert result is None
        finally:
...
class TestGeminiProviderBranches:
    ...
    def test_gemini_make_config(self):
        """Test GeminiProvider.make_config() with various params."""
    ...
    def test_gemini_build_contents_with_data(self):
        """Test build_contents with image data."""
        # Mock the module
        mock_types = MagicMock()
    ...
    def test_gemini_build_contents_with_url(self):
        """Test build_contents with image URL."""
        parts = []
        prompt = "describe this"
    ...
    def test_gemini_wrap_error_rate_limit(self):
        """Test wrap_error with rate limit error."""
    ...
    def test_gemini_wrap_error_auth(self):
        """Test wrap_error with auth error."""
