...
from openviper.app import OpenViper, get_handler_signature, resolve_middleware_entry
...
class TestGetHandlerSignature:
    """Tests for get_handler_signature function."""

    def test_caches_signature(self):
        def handler(request: Request) -> dict:
            return {}

        result1 = get_handler_signature(handler)
        result2 = get_handler_signature(handler)
...
class TestResolveMiddlewareEntry:
    """Tests for resolve_middleware_entry function."""

    def test_returns_class_as_is(self):
        class MyMiddleware:
            pass

        result = resolve_middleware_entry(MyMiddleware)
    ...
    def test_imports_string_reference(self, mock_import):
        ...
        result = resolve_middleware_entry("myapp.middleware.MyMiddleware")
    ...
    def test_raises_on_import_error(self, mock_import):
        with pytest.raises(ImportError, match="nonexistent.Middleware"):
            resolve_middleware_entry("nonexistent.Middleware")
...
class TestGetOpenAPISchema:
    """Tests for get_openapi_schema method."""
    ...
    def test_caches_schema(self):
        ...
        schema1 = app.get_openapi_schema()
        schema2 = app.get_openapi_schema()
...
class TestCoerceResponse:
    """Tests for coerce_response method."""

    def test_dict_to_json_response(self):
        app = OpenViper()

        response = app.coerce_response({"key": "value"})
    ...
    def test_list_to_json_response(self):
        ...
        response = app.coerce_response([1, 2, 3])
    ...
    def test_string_to_plain_text(self):
        ...
        response = app.coerce_response("Hello")
    ...
    def test_response_passthrough(self):
        ...
        response = app.coerce_response(original)
    ...
    def test_none_to_empty_response(self):
        ...
        response = app.coerce_response(None)
...
class TestCallHandler:
    ...
    async def test_calls_sync_handler(self):
        ...
        response = await app.call_handler(handler, request)
...
class TestCall:
    ...
    async def test_coerce_response_pydantic(self):
        ...
        response = app.coerce_response(MockPydantic())
        assert isinstance(response, JSONResponse)
        # orjson produces compact JSON
        assert response.body == b'{"a":1}'

    @pytest.mark.asyncio
    async def testcall_handler_path_params_injection(self):
        app = OpenViper()
        request = MagicMock(spec=Request)
