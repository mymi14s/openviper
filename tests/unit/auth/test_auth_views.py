...
from openviper.auth.views.jwt_refresh import JWTRefreshView
...
def make_user(pk: int = 1, username: str = "alice") -> MagicMock:
...
class TestBaseLoginViewAuthenticateUser:
    ...
    async def test_raises_unauthorized_when_username_missing(self) -> None:
        ...
        view = BaseLoginView()
        req = make_request(json_body={"password": "secret"})
    ...
    async def test_raises_unauthorized_when_password_missing(self) -> None:
        ...
        view = BaseLoginView()
        req = make_request(json_body={"username": "alice"})
        with pytest.raises(Unauthorized, match="required"):
            await view.authenticate_user(req)
    ...
    async def test_raises_unauthorized_when_both_fields_empty(self) -> None:
        ...
        view = BaseLoginView()
        req = make_request(json_body={"username": "", "password": ""})
        with pytest.raises(Unauthorized, match="required"):
            await view.authenticate_user(req)
    ...
    async def test_raises_unauthorized_when_authenticate_returns_none(self) -> None:
        ...
        view = BaseLoginView()
        req = make_request(json_body={"username": "bob", "password": "wrong"})
        with patch(
    ...
    async def test_raises_unauthorized_when_account_inactive(self) -> None:
        ...
        req = make_request(json_body={"username": "alice", "password": "correct"})
        with patch(
            "openviper.auth.views.base_login.authenticate",
            new_callable=AsyncMock,
...
class TestJWTLoginView:
    ...
    async def test_returns_access_and_refresh_tokens(self) -> None:
        ...
        view = JWTLoginView()
        user = make_user(pk=7)
        req = make_request(json_body={"username": "alice", "password": "pw"})
    ...
    async def test_propagates_unauthorized_from_authenticate_user(self) -> None:
        ...
        view = JWTLoginView()
        req = make_request(json_body={"username": "x", "password": "y"})
        with patch.object(
...
class TestJWTRefreshView:
    """Tests for JWTRefreshView.post."""

    @pytest.mark.asyncio
    async def test_rotates_refresh_token(self) -> None:
        """A successful refresh revokes the old token and returns a new pair."""
        view = JWTRefreshView()
        req = make_request(json_body={"refresh": "old.refresh.token"})
        claims = {"sub": "7", "jti": "old-jti", "exp": 32503680000}

        with (
            patch(
                "openviper.auth.views.jwt_refresh.decode_refresh_token_checked",
                new_callable=AsyncMock,
                return_value=claims,
            ),
            patch(
                "openviper.auth.views.jwt_refresh.revoke_token", new_callable=AsyncMock
            ) as m_revoke,
            patch(
                "openviper.auth.views.jwt_refresh.create_access_token", return_value="new.access"
            ) as m_acc,
            patch(
                "openviper.auth.views.jwt_refresh.create_refresh_token", return_value="new.refresh"
            ) as m_ref,
        ):
            result = await view.post(req)

        m_revoke.assert_awaited_once()
        m_acc.assert_called_once_with(user_id="7")
        m_ref.assert_called_once_with(user_id="7")
        assert result == {"access": "new.access", "refresh": "new.refresh"}

    @pytest.mark.asyncio
    async def test_missing_refresh_token_raises_unauthorized(self) -> None:
        """An empty refresh field raises Unauthorized."""
        view = JWTRefreshView()
...
class TestLogoutViewAuthTypeNone:
    ...
    async def test_raises_unauthorized_for_anonymous_request(self) -> None:
        ...
        view = LogoutView()
        req = make_request(auth={"type": "none"})
        with pytest.raises(Unauthorized):
            await view.post(req)
...
class TestLogoutViewJWT:
    ...
    async def test_revokes_jwt_with_valid_claims(self) -> None:
        ...
        view = LogoutView()
        req = make_request(
            auth={"type": "jwt"}, headers={"authorization": "Bearer header.payload.sig"}
        )
    ...
    async def test_skips_revocation_when_token_missing(self) -> None:
        """Missing token header skips JWT revocation silently."""
        view = LogoutView()
        req = make_request(auth={"type": "jwt"})
        with patch(
            "openviper.auth.views.logout.revoke_jwt_token", new_callable=AsyncMock
        ) as m_revoke:
            result = await view.post(req)
    ...
    async def test_skips_revocation_when_jti_missing(self) -> None:
        ...
        view = LogoutView()
        req = make_request(auth={"type": "jwt"}, headers={"authorization": "Bearer bad.token"})
        with (
    ...
    async def test_exp_as_datetime_object_is_accepted(self) -> None:
        ...
        view = LogoutView()
        req = make_request(auth={"type": "jwt"}, headers={"authorization": "Bearer tok"})
...
class TestLogoutViewOpaqueToken:
    ...
    async def test_revokes_opaque_token(self) -> None:
        ...
        view = LogoutView()
        req = make_request(
            auth={"type": "token"}, headers={"authorization": "Token raw-opaque-token"}
        )
        with patch(
    ...
    @pytest.mark.asyncio
    async def test_skips_revocation_when_token_missing(self) -> None:
        """Missing raw token header skips revocation silently."""
        view = LogoutView()
        req = make_request(auth={"type": "token"})
        with patch(
            "openviper.auth.views.logout.revoke_opaque_token", new_callable=AsyncMock
        ) as m_revoke:
            result = await view.post(req)
...
class TestLogoutViewSession:
    ...
    async def test_calls_session_manager_logout(self) -> None:
        """Session logout delegates to SessionManager.logout and clears the cookie."""
        view = LogoutView()
        req = make_request(auth={"type": "session"})
        ...
        assert mock_manager.logout.await_count == 1
        call_args = mock_manager.logout.await_args
...
class TestLogoutViewUnknownAuthType:
    ...
    async def test_unknown_auth_type_returns_detail(self) -> None:
        ...
        view = LogoutView()
        req = make_request(auth={"type": "custom_scheme"})
        result = await view.post(req)
        assert result == {"detail": "Logged out."}
...
class TestMeView:
    ...
    async def test_returns_user_profile_dict(self) -> None:
        ...
        view = MeView()
        user = make_user(pk=99, username="bob")
...
class TestJwtRoutes:
    ...
    def test_has_three_entries(self) -> None:
        """jwt_routes contains exactly three route entries."""
        assert len(jwt_routes) == 3
    ...
    def test_refresh_path_and_methods(self) -> None:
        """Second entry is the JWT refresh route."""
        path, _handler, methods = jwt_routes[1]
        assert path == "/jwt/refresh"
        assert methods == ["POST"]

    def test_logout_path_and_methods(self) -> None:
        """Third entry is the JWT logout route."""
        path, _handler, methods = jwt_routes[2]
