...
class TestLogout:
    ...
    async def test_deletes_session(self, mock_request):
        ...
        assert mock_request._session.key == ""

    @pytest.mark.asyncio
    async def test_clears_scope_session_on_logout(self, mock_request):
        old_session = MagicMock()
        old_session.key = "test-session-key"
        mock_request._session = old_session
        mock_request._scope = {"session": old_session}
        ...
        assert mock_request._scope["session"].key == ""
...
class TestUpdateLastLogin:
    ...
    async def test_handles_save_error_gracefully(self):
        ...
        user = MagicMock()
        user.pk = 1
        user.save = AsyncMock(side_effect=RuntimeError("DB error"))
