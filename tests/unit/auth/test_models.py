...
class TestOnUserUpdate:
    ...
    async def test_invalidate_user_cache_called_with_user_pk(self) -> None:
        """Event handler calls invalidate_user_cache with the instance pk."""
        instance = MagicMock()
        instance.pk = 42
        with patch(
            "openviper.auth.models.invalidate_user_cache", new=AsyncMock()
        ) as mock_invalidate:
            await on_user_update(instance, event="on_update")
            mock_invalidate.assert_called_once_with(42)

    async def test_no_call_when_pk_is_none(self) -> None:
    ...
    async def test_invalidate_user_cache_removes_entry(self) -> None:
        """invalidate_user_cache pops the user id from USER_CACHE."""
        USER_CACHE[99] = (MagicMock(), 9999999.0)
        assert 99 in USER_CACHE
        await invalidate_user_cache(99)
        assert 99 not in USER_CACHE

    async def test_invalidate_user_cache_noop_for_missing_id(self) -> None:
        """invalidate_user_cache does not raise when the id is absent."""
        USER_CACHE.pop(1000, None)
        await invalidate_user_cache(1000)  # must not raise
