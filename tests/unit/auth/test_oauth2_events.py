...
VALID_PATH_INITIAL = "myapp.events.oauth_initial"

SAMPLE_EVENTS: dict[str, str] = {
    "on_success": VALID_PATH,
    "on_fail": VALID_PATH_FAIL,
    "on_error": VALID_PATH_ERROR,
    "on_initial": VALID_PATH_INITIAL,
}
...
class TestLoadOAuth2Events:
    ...
    def test_returns_configured_events(self) -> None:
        ...
        mock_settings.OAUTH2_EVENTS = SAMPLE_EVENTS
...
class TestTriggerEventOnSuccess:
    ...
    async def test_calls_sync_handler(self) -> None:
        ...
        with (
            ...
            await auth.trigger_event("on_success", SAMPLE_PAYLOAD)
        handler.assert_called_once_with(SAMPLE_PAYLOAD)
    ...
    async def test_calls_async_handler(self) -> None:
        ...
        with (
            ...
            await auth.trigger_event("on_success", SAMPLE_PAYLOAD)
        handler.assert_awaited_once_with(SAMPLE_PAYLOAD)
    ...
    async def test_ignores_unknown_event_name(self) -> None:
        ...
        with patch("importlib.import_module") as mock_import:
            await auth.trigger_event("on_unknown", SAMPLE_PAYLOAD)
        mock_import.assert_not_called()
...
class TestTriggerEventOnFail:
    ...
    async def test_calls_on_fail_handler(self) -> None:
        ...
        with (
            ...
            await auth.trigger_event("on_fail", SAMPLE_PAYLOAD)
        handler.assert_called_once_with(SAMPLE_PAYLOAD)
    ...
    async def test_passes_payload_verbatim(self) -> None:
        ...
        def capture(p: dict[str, object]) -> None:
            received.append(p)
        ...
        with (
            ...
            await auth.trigger_event("on_fail", SAMPLE_PAYLOAD)
        assert received[0] is SAMPLE_PAYLOAD
...
class TestTriggerEventOnError:
    ...
    async def test_calls_on_error_handler(self) -> None:
        ...
        with (
            ...
            await auth.trigger_event("on_error", SAMPLE_PAYLOAD)
        handler.assert_awaited_once_with(SAMPLE_PAYLOAD)
...
class TestTriggerEventOnInitial:
    ...
    async def test_calls_on_initial_handler(self) -> None:
        ...
        with (
            ...
            await auth.trigger_event("on_initial", SAMPLE_PAYLOAD)
        handler.assert_called_once_with(SAMPLE_PAYLOAD)
