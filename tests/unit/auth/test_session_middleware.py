...
from openviper.auth.backends import logout
...
class TestSessionMiddleware:
    ...
    @pytest.mark.asyncio
    async def test_logout_does_not_reissue_deleted_session_cookie(self, mock_store):
        """Backend logout clears scope session before middleware writes cookies."""
        old_session = Session(key="old-session-key", data={"user_id": "42"}, store=mock_store)
        mock_store.load.return_value = old_session
        mock_store.delete = AsyncMock()

        async def app_that_logs_out(scope, receive, send):
            await logout(scope["request"], response=None)
            await send({"type": "http.response.start", "status": 200, "headers": []})

        async def app_with_request(scope, receive, send):
            scope["request"] = Request(scope, receive)
            await app_that_logs_out(scope, receive, send)

        middleware = SessionMiddleware(app_with_request, store=mock_store)
        scope = {
            "type": "http",
            "method": "POST",
            "path": "/logout",
            "headers": [(b"cookie", b"sessionid=old-session-key")],
        }
        receive = AsyncMock()
        send = AsyncMock()

        with patch("openviper.auth.backends.settings") as backend_settings:
            backend_settings.SESSION_COOKIE_NAME = "sessionid"
            await middleware(scope, receive, send)
