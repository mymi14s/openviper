...
class TestSessionStoreCache:
    ...
    async def test_load_caches_session_data(self, mock_cache, mock_engine) -> None:
        ...
        with patch("openviper.auth.session.store.ensure_session_table", new=AsyncMock()):
            with patch(
    ...
    @pytest.mark.asyncio
    async def test_load_returns_from_cache_on_hit(self, mock_cache, mock_engine) -> None:
    ...
    @pytest.mark.asyncio
    async def test_get_user_uses_cached_user_id(self, mock_cache, mock_engine) -> None:
        """get_user should use cached user_id when session verified in DB."""
        engine, mock_conn = mock_engine
        key = "f" * 64
