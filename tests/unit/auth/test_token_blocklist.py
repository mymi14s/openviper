...
from openviper.auth.token_blocklist import (
    clear_token_cache,
    evict_blocklist_cache_if_full,
    get_blocklist_cache_lock,
    is_token_revoked,
    revoke_token,
)
...
class TestGetCacheLock:
    """Tests for get_blocklist_cache_lock function."""

    def test_creates_lock_if_none(self):

        original = token_blocklist._CACHE_LOCK_REF[0]
        token_blocklist._CACHE_LOCK_REF[0] = None
        ...
        try:
            ...
            token_blocklist._CACHE_LOCK_REF[0] = original

    def test_returns_existing_lock(self):

        lock1 = get_blocklist_cache_lock()
        lock2 = get_blocklist_cache_lock()

        assert lock1 is lock2


class TestEvictIfFull:
    """Tests for evict_blocklist_cache_if_full function."""

    def test_does_nothing_when_under_limit(self):

        cache = {"key1": time.time() + 100, "key2": time.time() + 100}
        evict_blocklist_cache_if_full(cache, time.time())
    ...
    def test_evicts_expired_entries_first(self):
        ...
        try:
            ...
            evict_blocklist_cache_if_full(cache, now)
...
class TestRevokeToken:
    ...
    async def test_inserts_token_into_blocklist(self, mock_db_context):
        ...
        with patch("openviper.auth.token_blocklist.ensure_blocklist_table", new=AsyncMock()):
    ...
    async def test_adds_to_revoked_cache(self, mock_db_context):
        ...
        assert "cached-jti" in token_blocklist.JTI_REVOKED_CACHE
    ...
    async def test_removes_from_valid_cache(self, mock_db_context):
        ...
        token_blocklist.JTI_VALID_CACHE["to-revoke-jti"] = time.time() + 100
        ...
        assert "to-revoke-jti" not in token_blocklist.JTI_VALID_CACHE
        assert "to-revoke-jti" in token_blocklist.JTI_REVOKED_CACHE
...
class TestIsTokenRevoked:
    ...
    async def test_returns_true_for_cached_revoked_token(self):
        ...
        token_blocklist.JTI_REVOKED_CACHE["revoked-jti"] = time.time() + 3600
    ...
    async def test_returns_false_for_cached_valid_token(self):
        ...
        token_blocklist.JTI_VALID_CACHE["valid-jti"] = time.time() + 3600
    ...
    async def test_evicts_expired_revoked_token(self):
        ...
        token_blocklist.JTI_REVOKED_CACHE["expired-revoked-jti"] = time.time() - 3600
        ...
        assert "expired-revoked-jti" not in token_blocklist.JTI_REVOKED_CACHE
    ...
    async def test_caches_revoked_status_from_db(self):
        ...
        assert result is True
        assert "db-revoked-jti" in token_blocklist.JTI_REVOKED_CACHE
    ...
    async def test_caches_valid_status_from_db(self):
        ...
        assert result is False
        assert "db-valid-jti" in token_blocklist.JTI_VALID_CACHE
...
class TestClearTokenCache:
    ...
    def test_clears_both_caches(self):
        ...
        token_blocklist.JTI_REVOKED_CACHE["key1"] = time.time() + 100
        token_blocklist.JTI_VALID_CACHE["key2"] = time.time() + 100

        clear_token_cache()

        assert len(token_blocklist.JTI_REVOKED_CACHE) == 0
        assert len(token_blocklist.JTI_VALID_CACHE) == 0
