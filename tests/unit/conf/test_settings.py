...
class TestCastTuple:
    def test_comma_separated(self):
        assert cast_tuple("a,b,c") == ("a", "b", "c")

    def test_strips_whitespace(self):
        assert cast_tuple(" a , b ") == ("a", "b")
...
class TestCastTimedelta:
    def test_seconds_to_timedelta(self):
        assert cast_timedelta("3600") == timedelta(seconds=3600)

    def test_zero(self):
        assert cast_timedelta("0") == timedelta(0)


class TestCastEnvValue:
    def test_bool_cast(self):
        s = Settings()
        result = cast_env_value(s.DEBUG, "false")
    ...
    def test_str_cast(self):
        s = Settings()
        result = cast_env_value(s.TIME_ZONE, "America/New_York")
...
class TestAutoIncludeProjectApp:
    def test_prepends_project_app_when_absent(self):
        s = Settings()
        result = auto_include_project_app(s, "myproject.settings")
...
class TestValidateSettings:
    ...
    def test_openapi_enabled_in_production_fails(self):
        s = self._prod(OPENAPI={"enabled": True})
        with pytest.raises(SettingsValidationError, match="OPENAPI"):
            validate_settings(s, "production")
    ...
    def test_multiple_errors_raised_together(self):
        s = self._prod(DEBUG=True, OPENAPI={"enabled": True})
...
""")
        ...
        settings_mod.MODULE_CACHE.clear()
...
""")
    ...
    def test_setup_does_not_generate_secret_key_for_production(self, monkeypatch):
        """setup does not auto-generate SECRET_KEY for production and rejects insecure config."""
        monkeypatch.setenv("OPENVIPER_SETTINGS_MODULE", "")
...
class TestConfigureLogging:
    """Tests for configure_logging and JsonFormatter."""

    def setup_method(self) -> None:
        ov = logging.getLogger("openviper")
        ov.handlers = [h for h in ov.handlers if not isinstance(h, OVDefaultHandler)]
    ...
    def test_text_format_installs_handler_on_openviper_logger(self) -> None:
        ...
        configure_logging(s)
        ov = logging.getLogger("openviper")
        assert any(isinstance(h, OVDefaultHandler) for h in ov.handlers)
    ...
    def test_json_format_uses_json_formatter(self) -> None:
        ...
        ov = logging.getLogger("openviper")
        assert any(
            isinstance(h, OVDefaultHandler) and isinstance(h.formatter, JsonFormatter)
    ...
    def test_repeated_calls_do_not_duplicate_handlers(self) -> None:
        ...
        configure_logging(s)
        ov_default = [
            h for h in logging.getLogger("openviper").handlers if isinstance(h, OVDefaultHandler)
        ]
