...
def run_async_coro(coro: t.Coroutine[object, object, object]) -> object:
...
def reset_connections_state() -> cabc.Generator[None]:
    """Reset global singletons before each unit test to prevent cross-test pollution.

    ``configure_db()`` sets ``connections.initialized = True``, registers
    backends in ``connections.backends``, and caches a real async engine in
