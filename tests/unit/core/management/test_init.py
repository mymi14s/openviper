...
class TestExecuteFromCommandLine:
    ...
    @patch("openviper.core.management.find_command")
    def test_execute_from_command_line_runs_command(self, mockfind_command):
        mock_command = Mock()
        mockfind_command.return_value = mock_command

        with pytest.raises(SystemExit) as exc_info:
            execute_from_command_line(["viperctl.py", "testcmd"])

        assert exc_info.value.code == 0
        mockfind_command.assert_called_once_with("testcmd")
        mock_command.run_from_argv.assert_called_once_with(["viperctl.py", "testcmd"])

    @patch("openviper.core.management.find_command")
    def test_execute_from_command_line_handles_command_error(self, mockfind_command, capsys):
        mockfind_command.side_effect = CommandError("Unknown command", returncode=42)
...
class TestExtractSettingsFlag:
    """Tests for extract_settings_flag."""

    def test_equals_syntax(self) -> None:
        value, remaining = extract_settings_flag(
            ["viperctl.py", "--settings=project.settings.prod", "start-server"]
        )
        assert value == "project.settings.prod"
        assert remaining == ["viperctl.py", "start-server"]
    ...
    def test_not_present(self) -> None:
        value, remaining = extract_settings_flag(["viperctl.py", "start-server"])
        assert value is None
        assert remaining == ["viperctl.py", "start-server"]

    def test_flag_after_subcommand(self) -> None:
        value, remaining = extract_settings_flag(
            ["viperctl.py", "start-server", "--settings=project.settings.prod"]
        )
        assert value == "project.settings.prod"
        assert remaining == ["viperctl.py", "start-server"]
...
class TestAutoDiscoverSettings:
    """Tests for auto_discover_settings."""
    ...
    def test_discovers_package_settings(self, tmp_path) -> None:
        ...
        result = auto_discover_settings(str(tmp_path / "viperctl.py"))
...
class TestExecuteFromCommandLineSettings:
    ...
    def test_settings_flag_sets_env_and_forces_setup(
        self, mock_setup, mockfind_command, monkeypatch
    ) -> None:
    ...
    def test_settings_flag_stripped_from_argv(
        ...
        mock_command.run_from_argv.assert_called_once_with(
            ["viperctl.py", "start-server", "--port=9000"]
        )
...
class TestFindCommand:
    ...
    def testfind_command_settings_exception(self, mock_settings):
