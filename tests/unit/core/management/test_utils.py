...
class TestGetBannerNone:
    """Test get_banner with None cmd_obj."""

    def test_get_banner_with_cmd_obj(self, capsys):
        cmd_obj = BaseCommand()
        get_banner(cmd_obj, "127.0.0.1", 8000)
    ...
    def test_get_banner_without_cmd_obj(self, capsys):
        """Test that print_banner creates a BaseCommand if none provided."""
        get_banner(None, "localhost", 3000)
    ...
    def test_get_banner_custom_host_port(self, capsys):
        get_banner(None, "0.0.0.0", 5000)

        captured = capsys.readouterr()
        assert "http://0.0.0.0:5000/" in captured.out

    def test_get_banner_ipv6(self, capsys):
        """Test with IPv6 address."""
        get_banner(None, "::1", 8000)

        captured = capsys.readouterr()
        assert "http://::1:8000/" in captured.out

    def test_get_banner_high_port(self, capsys):
        """Test with high port number."""
        get_banner(None, "127.0.0.1", 65535)
...
class TestBannerIntegration:
    ...
    def test_get_banner_creates_basecommand_when_none(self):
        """Test that get_banner creates BaseCommand when cmd_obj is None."""
        with patch("openviper.core.management.utils.BaseCommand") as MockBaseCommand:
            ...
            get_banner(None, "127.0.0.1", 8000)
...
class TestEdgeCases:
    ...
    def test_get_banner_port_zero(self, capsys):
        """Test with port 0 (system-assigned port)."""
        get_banner(None, "127.0.0.1", 0)
