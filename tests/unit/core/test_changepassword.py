...
class TestHandleWithOptions:
    ...
    def test_handle_empty_username_raises_error(
        ...
        with pytest.raises(CommandError) as exc_info:
            command.handle(username=None, password="password123")
...
class TestUserLookup:
    ...
    def test_handle_user_not_found(self, mock_run_async, mock_get_user_model, command):
        ...
        with pytest.raises(CommandError, match="User 'nonexistent' not found"):
            command.handle(username="nonexistent", password="password123")
...
class TestPasswordValidation:
    ...
    @patch(
        "openviper.core.management.utils.getpass.getpass",
        side_effect=["pass1", "pass2", "password123", "password123"],
    )
    @patch(
        "openviper.core.management.commands.changepassword.run_async_command",
        side_effect=run_async_coro,
...
class TestSetPasswordAwaited:
    ...
    def test_set_password_is_awaited(
        ...
        mock_user.set_password.assert_awaited_once_with("newpass123")
...
class TestEdgeCases:
    ...
    def test_handle_with_empty_string_username(
        ...
        with pytest.raises(CommandError, match="Username is required"):
            command.handle(username="", password="password123")
