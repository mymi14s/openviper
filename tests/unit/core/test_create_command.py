...
class TestValidation:
    ...
    def test_handle_app_path_traversal_raises_error(self, command, temp_dir):
        with pytest.raises(CommandError) as exc_info:
            command.handle(command_name="safe", app_name="../escape", directory=temp_dir)

        assert "safe app path" in str(exc_info.value)

    def test_handle_absolute_app_path_raises_error(self, command, temp_dir):
        with pytest.raises(CommandError) as exc_info:
            command.handle(command_name="safe", app_name="/tmp/escape", directory=temp_dir)
