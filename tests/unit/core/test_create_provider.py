...
class TestHelperFunctions:
    ...
    def test_to_class_name_snake_case(self):
        assert to_class_name("my_provider") == "MyProviderProvider"

    def test_to_class_name_kebab_case(self):
        assert to_class_name("my-provider") == "MyProviderProvider"

    def test_to_class_name_single_word(self):
        assert to_class_name("openai") == "OpenaiProvider"

    def test_to_class_name_multiple_words(self):
        assert to_class_name("my_custom_ai") == "MyCustomAiProvider"

    def test_to_env_var_snake_case(self):
        assert to_env_var("my_provider") == "MY_PROVIDER_API_KEY"

    def test_to_env_var_kebab_case(self):
        assert to_env_var("my-provider") == "MY_PROVIDER_API_KEY"

    def test_to_env_var_single_word(self):
        assert to_env_var("openai") == "OPENAI_API_KEY"

    def test_render_simple_placeholder(self):
        template = "Hello {{ name }}"
        ctx = {"name": "World"}
        assert render_provider_template(template, ctx) == "Hello World"

    def test_render_multiple_placeholders(self):
        template = "{{ greeting }} {{ name }}!"
        ctx = {"greeting": "Hello", "name": "Alice"}
        assert render_provider_template(template, ctx) == "Hello Alice!"

    def test_render_no_placeholders(self):
        template = "No placeholders here"
        ctx = {}
        assert render_provider_template(template, ctx) == "No placeholders here"
