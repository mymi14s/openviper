...
class TestValidationFunctions:
    ...
    def testvalidate_username_valid(self):
        ...
        assert validate_username("test.user@domain") is None

    def testvalidate_username_empty(self):
        error = validate_username("")
        assert error is not None
        assert "cannot be blank" in error

    def testvalidate_username_invalid_chars(self):
        error = validate_username("test user!")
        assert error is not None
        assert "Invalid username" in error

    def testvalidate_username_too_long(self):
        error = validate_username("a" * 151)
        assert error is not None
        assert "Invalid username" in error

    def testvalidate_email_valid(self):
        assert validate_email("test@example.com") is None
        assert validate_email("user+tag@domain.co.uk") is None

    def testvalidate_email_empty(self):
        error = validate_email("")
        assert error is not None
        assert "cannot be blank" in error

    def testvalidate_email_invalid(self):
...
class TestPromptMethods:
    ...
    @patch(
        "openviper.core.management.utils.getpass.getpass",
        side_effect=["password1a", "password2b", "password123", "password123"],
    )
...
class TestEdgeCases:
    ...
    def testvalidate_username_regex_pattern(self):
        ...
        assert validate_username("user.name@domain") is None
        assert validate_username("user+tag") is None
        assert validate_username("user-name") is None

        # Invalid usernames
        assert validate_username("user name") is not None  # space
        assert validate_username("user!name") is not None  # invalid char

    def testvalidate_email_regex_pattern(self):
        """Test email validation regex."""
        # Valid emails
        assert validate_email("simple@example.com") is None
        assert validate_email("user+tag@domain.com") is None
        assert validate_email("user.name@sub.domain.com") is None
