...
def email_settings(tmp_path):
    ...
    stub = SimpleNamespace(
        ...
        INSTALLED_APPS=(),
        TASKS={"broker": "stub"},
        ATTACHMENT_ALLOWED_DIRS=[str(templates_dir), str(tmp_path)],
    )
...
class TestSendEmail:
    ...
    async def test_send_email_text_only(self, email_settings):
        with patch("openviper.core.email.sender.send_now", new_callable=AsyncMock) as mock_send:
    ...
    async def test_send_email_with_background_worker(self, email_settings):
        ...
        with (
            ...
            patch("openviper.core.email.sender.send_now", new_callable=AsyncMock) as mock_send,
        ):
            result = await send_email(
                recipients=["to@example.com"],
                subject="Queued",
    ...
    async def test_send_email_auto_mode_falls_back_when_enqueue_fails(self, email_settings):
        ...
        with (
            patch("openviper.core.email.sender.worker_available", return_value=True),
            patch(
                "openviper.core.email.sender.enqueue_email_job",
    ...
    async def test_fail_silently_behavior(self, email_settings):
        with (
            patch(
                "openviper.core.email.sender.send_now",
                new_callable=AsyncMock,
