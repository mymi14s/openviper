...
class TestTemplates:
    ...
    def test_markdown_template_returns_text_and_html(self) -> None:
        ...
        with (
            patch("openviper.http.response.compute_template_search_paths", return_value=["x"]),
...
class TestQueue:
    ...
    async def test_deliver_email_job_executes_send_now(self) -> None:
        ...
        with (
            ...
            message = queue.payload_to_message(payload)
            await queue.send_now(message)
    ...
    def test_payload_conversion_roundtrip_includes_attachments(self) -> None:
        ...
        payload = queue.message_to_payload(msg)
        restored = queue.payload_to_message(payload)
...
class TestBackends:
    ...
    def test_send_smtp_message_tls_and_login(self) -> None:
        ...
        with patch("openviper.core.email.backends.smtplib.SMTP", return_value=ctx) as mock_smtp:
            backends.send_smtp_message(MagicMock(), ["to@example.com"], email_settings)
