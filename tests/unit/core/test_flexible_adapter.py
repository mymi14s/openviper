...
from openviper.core.flexible_adapter import (
    ...
    ensure_sys_path,
    inject_app_into_settings,
    prepare_root_layout,
    synthesize_settings_module,
)
...
class TestPrepareRootLayout:
    """Test prepare_root_layout function."""
    ...
    def test_prepare_root_layout_creates_init_py(self, monkeypatch) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            prepare_root_layout(tmpdir_path)
...
class TestInjectAppIntoSettings:
    """Test inject_app_into_settings function."""
    ...
    def test_inject_app_into_settings_new_app(self) -> None:
        ...
        with patch("openviper.core.flexible_adapter.lazy_settings", mock_lazy):
            with patch("openviper.core.flexible_adapter.dataclasses.replace") as mock_replace:
                ...
                inject_app_into_settings("new_app")
    ...
    def test_inject_app_into_settings_existing_app(self) -> None:
        ...
        with patch("openviper.core.flexible_adapter.lazy_settings", mock_lazy):
            with patch("openviper.core.flexible_adapter.dataclasses.replace") as mock_replace:
                inject_app_into_settings("existing_app")
...
class TestEnsureModelsImported:
    """Test ensure_models_imported function."""

    def test_ensure_models_imported_success(self, mock_resolved) -> None:
        with patch("importlib.import_module") as mock_import:
            ensure_models_imported(mock_resolved)
    ...
    def test_ensure_models_imported_root_layout_file_exists(self, mock_root_resolved) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                with patch("importlib.import_module", side_effect=ImportError):
                    ensure_models_imported(mock_root_resolved)
...
class TestSynthesizeSettingsModule:
    ...
    def test_synthesize_settings_module_creates_module(self, monkeypatch) -> None:
        ...
        assert instance.DATABASES["default"]["OPTIONS"]["URL"] == "sqlite+aiosqlite:///db.sqlite3"
...
class TestBootstrapAndRun:
    ...
    @patch("openviper.core.flexible_adapter.inject_app_into_settings")
    @patch("openviper.core.flexible_adapter.ensure_models_imported")
    @patch("openviper.core.flexible_adapter.ensure_sys_path")
    ...
    @patch("openviper.core.flexible_adapter.inject_app_into_settings")
    @patch("openviper.core.flexible_adapter.ensure_models_imported")
    @patch("openviper.core.flexible_adapter.prepare_root_layout")
    ...
    @patch("openviper.core.flexible_adapter.ensure_models_imported")
    @patch("openviper.core.flexible_adapter.ensure_sys_path")
    @patch("openviper.core.flexible_adapter.synthesize_settings_module")
...
class TestEdgeCases:
    ...
    def test_ensure_sys_path_with_str_path(self, monkeypatch) -> None:
        """Test that ensure_sys_path handles string paths."""
        test_path = "/test/string/path"
        monkeypatch.setattr("sys.path", sys.path.copy())

        ensure_sys_path(Path(test_path))
