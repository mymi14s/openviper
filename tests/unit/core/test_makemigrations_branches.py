...
class TestMakemigrationsMoreBranches:
    ...
    def test_write_migration_includes_intra_and_inter_app_dependencies(
        ...
        with (
            ...
            patch(
                "openviper.core.management.commands.makemigrations.diff_states", return_value=ops
            ),
            patch(
                "openviper.core.management.commands.makemigrations.auto_migration_name",
                return_value="auto",
            ),
            patch(
    ...
    def test_drop_columns_sets_remove_column_drop_true(
        ...
        with (
            ...
            patch("openviper.core.management.commands.makemigrations.read_migrated_state"),
            patch(
                "openviper.core.management.commands.makemigrations.diff_states", return_value=[op]
            ),
            patch(
                "openviper.core.management.commands.makemigrations.write_migration"
            ) as mock_write_migration,
    ...
    def testdiff_states_empty_skips_write(self, command: Command, tmp_path: Path) -> None:
        app_dir = tmp_path / "app"
        app_dir.mkdir()
        migrations_dir = app_dir / "migrations"
        migrations_dir.mkdir()
        ...
        with (
            ...
            patch("openviper.core.management.commands.makemigrations.model_state_snapshot"),
            patch("openviper.core.management.commands.makemigrations.read_migrated_state"),
            patch("openviper.core.management.commands.makemigrations.diff_states", return_value=[]),
            patch(
                "openviper.core.management.commands.makemigrations.write_migration"
            ) as mock_write_migration,
            patch.object(command, "stdout") as mock_stdout,
