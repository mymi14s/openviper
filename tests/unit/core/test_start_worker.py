...
class TestDramatiqImport:
    ...
    def test_handle_missing_dramatiq_exits(self, command, capsys) -> None:
        ...
        with patch(
            "openviper.core.management.commands.start_worker.importlib.import_module",
...
class TestDatabaseBroker:
    ...
    def test_handle_database_broker_runs_in_process(self, command, capsys) -> None:
        ...
        with patch.dict(sys.modules, {"dramatiq": mock_dramatiq}):
            with patch("openviper.core.management.commands.start_worker.settings", mock_settings):
                with patch(
                    "openviper.core.management.commands.start_worker.run_worker",
                    mock_run_worker,
                ):
                    command.handle(modules=[], queues=None, threads=8, processes=1)
...
class TestRedisBroker:
    ...
    def test_handle_redis_broker_spawns_subprocess(self, command, capsys) -> None:
        ...
        with patch.dict(sys.modules, {"dramatiq": mock_dramatiq}):
            with patch("openviper.core.management.commands.start_worker.settings", mock_settings):
                with patch(
                    "openviper.core.management.commands.start_worker.AppResolver",
                    return_value=mock_resolver,
                ):
                    with patch(
                        "openviper.core.management.commands.start_worker.subprocess.Popen",
...
class TestModuleDiscovery:
        ...
        with patch.dict(sys.modules, {"dramatiq": mock_dramatiq}):
            with patch("openviper.core.management.commands.start_worker.settings", mock_settings):
                with patch(
                    ...
                    with patch(
                        "openviper.core.management.commands.start_worker.os.walk",
                        return_value=[],
                    ):
    ...
    @patch("openviper.core.management.commands.start_worker.subprocess.Popen")
        ...
        with patch.dict(sys.modules, {"dramatiq": mock_dramatiq}):
            with patch("openviper.core.management.commands.start_worker.settings", mock_settings):
                with patch(
                    ...
                    with patch("openviper.core.management.commands.start_worker.subprocess.Popen"):
