...
class TestGetEngineLock:
    def test_returns_lock(self):
        ...
        # When called outside an event loop, a threading.Lock is returned
        # to prevent cross-loop state bleeding.  Inside an async context,
        # an asyncio.Lock scoped to the running loop is returned.

        assert isinstance(lock, (asyncio.Lock, threading.Lock))

    async def test_returns_asyncio_lock_inside_loop(self):
        lock = get_engine_lock()
...
class TestValidatePoolConfig:
    def test_valid_value_returned(self):
        result = validate_pool_config(20, "POOL_SIZE", 1, 100, 20)
        assert result == 20

    def test_below_min_clamped(self):
        result = validate_pool_config(0, "POOL_SIZE", 1, 100, 20)
        assert result == 1

    def test_above_max_clamped(self):
        result = validate_pool_config(999, "POOL_SIZE", 1, 100, 20)
        assert result == 100

    def test_string_int_converted(self):
        result = validate_pool_config("15", "POOL_SIZE", 1, 100, 20)
        assert result == 15

    def test_invalid_string_raises_value_error(self):
        with pytest.raises(ValueError, match="invalid value"):
            validate_pool_config("not_a_number", "POOL_SIZE", 1, 100, 20)

    def test_none_returns_default(self):
        result = validate_pool_config(None, "POOL_SIZE", 1, 100, 20)
        assert result == 20

    def test_exact_min_boundary(self):
        result = validate_pool_config(1, "POOL_SIZE", 1, 100, 20)
        assert result == 1
...
class TestCreateEngine:
    ...
    async def test_mariadb_url_converted(self):
        engine = create_engine("mariadb://user:pass@localhost/db")
        assert "aiomysql" in str(engine.url)
        await engine.dispose()
...
class TestGetEngine:
    ...
    async def test_fast_path_returns_cached(self):
        """When engine is already set for the current loop, lock is not acquired."""

        sentinel = MagicMock()
        old = mod._engine
        old_per_loop = mod._engines_per_loop.copy()
        mod._engine = None
        try:
            loop = asyncio.get_running_loop()
            mod._engines_per_loop[id(loop)] = sentinel
            result = await get_engine()
            assert result is sentinel
        finally:
            mod._engines_per_loop.clear()
            mod._engines_per_loop.update(old_per_loop)
            mod._engine = old
