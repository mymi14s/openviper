...
from openviper.db.backends.features import (
    DatabaseFeatures,
    MariaDBFeatures,
    MSSQLFeatures,
    OracleFeatures,
...
class TestPerDialectFeatures:
    """Each dialect's feature class must declare its documented capabilities."""

    def test_postgresql_features(self) -> None:
        features = PostgreSQLFeatures()
        assert isinstance(features, DatabaseFeatures)
        assert features.supports_returning is True
        assert features.supports_partial_indexes is True
        assert features.supports_check_constraints is True
        assert features.supports_schema_comments is True

    def test_mariadb_features(self) -> None:
        features = MariaDBFeatures()
        assert isinstance(features, DatabaseFeatures)
        assert features.supports_returning is False
        assert features.supports_partial_indexes is False
        assert features.supports_check_constraints is True
        assert features.supports_schema_comments is False

    def test_sqlite_features(self) -> None:
    ...
    def test_mssql_features(self) -> None:
    ...
    def test_oracle_features(self) -> None:
...
class TestGetFeaturesForVendor:
    """Vendor dispatch must resolve to the correct dialect feature class."""
    ...
    def test_known_vendor_returns_dialect_features(self, vendor: str, expected_cls: type) -> None:
        features = get_features_for_vendor(vendor)
        assert isinstance(features, expected_cls)

    def test_unknown_vendor_falls_back_to_base_defaults(self) -> None:
        features = get_features_for_vendor("unknown")
        assert type(features) is DatabaseFeatures
        assert features.supports_returning is True
        assert features.supports_partial_indexes is True
        assert features.supports_schema_comments is False

    def test_empty_vendor_falls_back_to_base_defaults(self) -> None:
        features = get_features_for_vendor("")
        assert type(features) is DatabaseFeatures


class TestBackendFeaturesDispatch:
    """DefaultDatabaseBackend must wire vendor URL to the dialect feature set."""

    @pytest.mark.parametrize(
        ("url", "expected_cls"),
        [
            ("postgresql://u:p@h/d", PostgreSQLFeatures),
            ("mysql://u:p@h/d", MariaDBFeatures),
            ("mariadb://u:p@h/d", MariaDBFeatures),
            ("sqlite:///db.sqlite3", SQLiteFeatures),
            ("mssql://u:p@h/d", MSSQLFeatures),
            ("oracle://u:p@h/d", OracleFeatures),
        ],
    )
    def test_backend_features_match_dialect(self, url: str, expected_cls: type) -> None:
        backend = DefaultDatabaseBackend("default", {"URL": url})
        assert isinstance(backend.features, expected_cls)

    def test_backend_features_for_unknown_url_uses_base_defaults(self) -> None:
        backend = DefaultDatabaseBackend("default", {"URL": ""})
        assert type(backend.features) is DatabaseFeatures
