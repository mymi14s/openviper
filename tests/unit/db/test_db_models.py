...
async def setup_db():
    ...
    async with engine.begin() as conn:
        ...
        await conn.execute(
            sa.text("CREATE TABLE sample (id INTEGER PRIMARY KEY, name TEXT, age INTEGER)")
        )
    yield
    async with engine.begin() as conn:
...
async def test_model_ignore_permissions():
    ...
    with patch(
        ...
        with patch(
            "openviper.db.models.get_connection", new_callable=AsyncMock, return_value=mock_conn
        ):
            # bulk_create
            ...
            with patch(
                "openviper.db.models.execute_select", new_callable=AsyncMock, return_value=[]
            ):
                await qs.all()
            ...
            # update()
            with patch(
                "openviper.db.models.execute_update", new_callable=AsyncMock, return_value=0
            ):
            ...
            # delete()
            with patch(
                "openviper.db.models.execute_delete", new_callable=AsyncMock, return_value=0
            ):
                await qs.delete()
...
async def test_queryset_permission_error_returns_empty():
    ...
    with patch(
        "openviper.db.models.check_permission_for_model", side_effect=ModelPermissionError("denied")
    ):
        assert await qs.all() == []
