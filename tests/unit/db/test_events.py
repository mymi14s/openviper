...
class TestModelEventDispatcher:
    ...
    def test_dispatcher_repr(self):
        with patch("openviper.db.events.resolve_dotted", return_value=lambda x: x):
    ...
    def test_get_dispatcher_concurrency_mock(self):
        reset_dispatcher()
        with patch("openviper.db.events.build_dispatcher") as mock_build:
            mock_build.return_value = MagicMock()
            get_dispatcher()
            assert mock_build.called

    def testbuild_dispatcher_exception(self):
        with patch("openviper.db.events.ModelEventDispatcher", side_effect=Exception("ouch")):
            with patch("openviper.db.events.logger") as mock_logger:
                res = build_dispatcher()
                assert res is None
                assert mock_logger.warning.called

    def testresolve_dotted_invalid_path(self):
        with patch("openviper.db.events.logger") as mock_logger:
            assert resolve_dotted("no_dot") is None
            assert mock_logger.warning.called

    def testresolve_dotted_import_error(self):
        with patch("openviper.db.events.logger") as mock_logger:
            # "non" is not in INSTALLED_APPS or allowed prefixes,
            # so it gets blocked by the allowlist (logger.error, not warning)
            assert resolve_dotted("non.existent.module.attr") is None
            assert mock_logger.error.called

    @pytest.mark.asyncio
    async def testcall_handler_no_loop(self):
        async def mock_async_handler(inst, **kwargs):
            pass

        with patch("asyncio.get_running_loop", side_effect=RuntimeError("no loop")):
            with patch("openviper.db.events.logger") as mock_logger:
                call_handler(mock_async_handler, MagicMock(), "event")
    ...
    def test_dispatch_decorator_handler_exception(self, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="openviper.db"):
            dispatch_decorator_handlers(model_path, event, MagicMock())
...
class TestResolveDottedBlocked:
    ...
    def test_blocked_module_returns_none(self, path):
        with patch("openviper.db.events.logger") as mock_logger:
            assert resolve_dotted(path) is None
            mock_logger.error.assert_called_once()

    def test_safe_module_path_allowed(self):
        assert is_safe_module_path("openviper.auth.models") is True
        with patch("openviper.db.events.settings") as mock_settings:
            mock_settings.INSTALLED_APPS = ("myapp",)
            assert is_safe_module_path("myapp.events.handler") is True

    def test_safe_module_path_blocked(self):
        assert is_safe_module_path("os") is False
        assert is_safe_module_path("subprocess") is False
        assert is_safe_module_path("pickle") is False

    def test_resolve_callable_passthrough(self):
        def fn(x):
            return x

        assert resolve_dotted(fn) is fn
...
class TestCallHandlerAsyncWithLoop:
    ...
    async def test_async_handler_creates_task(self):
        ...
        call_handler(async_handler, MagicMock(), "after_insert")
    ...
    async def test_async_handler_tracked_in_background_tasks(self):
        ...
        call_handler(slow_handler, MagicMock(), "after_insert")
...
class TestGetDispatcherDoubleCheckInLock:
    ...
    def test_concurrent_double_check_returns_cached(self):
        ...
        with patch.object(events_mod, "build_dispatcher", return_value=None) as mock_build:
            ...
            t1.join()
            t2.join()
            # build_dispatcher should be called at most once
