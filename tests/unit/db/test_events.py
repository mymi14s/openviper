"""Unit tests for openviper.db.events — ModelEventDispatcher."""
...
import openviper.db.events as _events_module
...
class TestResolveDotted:
    def test_resolves_callable(self):
        fn = _resolve_dotted("openviper.db.events._resolve_dotted")
    ...
    def test_returns_none_on_missing_attr(self, caplog):
        result = _resolve_dotted("openviper.db.events.no_such_thing")
        assert result is None
...
class TestModelEventDispatcherInit:
    ...
    def test_resolves_valid_handler(self):
        cfg = {
            "myapp.models.Post": {
                "after_insert": ["openviper.db.events._resolve_dotted"],
            }
        }
    ...
    def test_bool_true_when_handlers_present(self):
        cfg = {"m.M": {"after_insert": ["openviper.db.events._resolve_dotted"]}}
        d = ModelEventDispatcher(cfg)
        assert bool(d)
...
class TestGetDispatcher:
    def test_returns_dispatcher_when_events_configured(self):
        ...
        with patch(
            "openviper.db.events._build_dispatcher",
...
class TestBuildDispatcher:
    def test_none_when_model_events_empty(self):
        ...
        # Returns an empty (falsy) dispatcher instance, not None
    ...
    def test_none_when_model_events_unresolvable(self):
        with patch(
            ...
            result = _build_dispatcher()
        # All handlers failed to resolve → dispatcher is falsy → but not None
        assert isinstance(result, ModelEventDispatcher)
        assert not bool(result)
    ...
    def test_creates_dispatcher_with_resolved_handlers(self):
        with patch(
            "openviper.db.events.settings",
            MODEL_EVENTS={"m.M": {"after_insert": ["openviper.db.events._resolve_dotted"]}},
            spec=False,
        ):
            result = _build_dispatcher()
...
class TestSupportedEvents:
    ...
    def test_unknown_event_only_logs_debug(self, caplog):
        ...
        cfg = {"m.M": {"unknown_event_xyz": ["openviper.db.events._resolve_dotted"]}}
        with caplog.at_level(logging.DEBUG, logger="openviper.db"):
            d = ModelEventDispatcher(cfg)
...
class TestModelTriggerEvent:
    ...
    def test_trigger_event_noop_when_dispatcher_is_empty(self):
        """_trigger_event must be a soft no-op when dispatcher is empty."""
        ...
        mock_dispatcher = ModelEventDispatcher({})
    ...
    def test_trigger_event_model_path_format(self):
        ...
        with patch("openviper.db.events.get_dispatcher", return_value=mock_disp):
...
class TestGetDispatcherDoubleCheckLock:
    ...
    def test_second_thread_sees_cached_value(self):
        ...
        def worker():
            with patch("openviper.db.events._build_dispatcher", side_effect=slow_build):
                barrier.wait()
