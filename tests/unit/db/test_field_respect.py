...
def test_apply_lookup_with_boolean_field():
    ...
    # True -> 1
    apply_lookup(col, "exact", True, field=field)
    col.__eq__.assert_called_with(1)

    # False -> 0
    apply_lookup(col, "exact", False, field=field)
...
def test_apply_lookup_with_datetime_field():
    ...
    apply_lookup(col, "exact", dt_str, field=field)
...
def test_apply_lookup_with_integer_field_in_lookup():
    ...
    vals = ["10", "20", 30]
    apply_lookup(col, "in", vals, field=field)
...
def test_apply_lookup_with_uuid_field():
    ...
    apply_lookup(col, "exact", val_str, field=field)
