...
from openviper.db.models import Model, ModelMeta, QuerySet
...
class TestForeignKey:
    ...
    def test_custom_on_delete(self):
        f = ForeignKey(to="X", on_delete="SET NULL")
        assert f.on_delete == "SET NULL"
    ...
    def test_resolve_target_name_index_conflict(self):
        ...
        ModelMeta.name_index["ConflictName"] = [MockCandidate1, MockCandidate2]
...
class TestFileField:
    ...
    async def test_pre_save_bytes(self):
        ...
        with patch("openviper.db.fields.Path") as mock_path:
            ...
            mock_path_instance.absolute.return_value.resolve.return_value.__truediv__ = MagicMock(
                return_value=mock_dir
            )
            mock_path_instance.__truediv__ = MagicMock(return_value=mock_dir)
            mock_path.return_value = mock_path_instance
            mock_path.side_effect = lambda *a, **kw: mock_path_instance

            with (
                patch("openviper.db.fields.aiofiles.open") as mock_open,
                patch("openviper.db.fields.asyncio.to_thread", new=AsyncMock()),
            ):
                mock_f = AsyncMock()
...
class TestLazyFK:
    ...
    async def test_lazy_fk_load_db(self):
        ...
        mock_model.objects.filter.return_value.all = AsyncMock(return_value=["LOADED_OBJ"])
...
class TestForeignKeyResolveTargetAppLabel:
    ...
    def test_resolve_via_app_label_prepend(self):
        ...
        AppAuthor._app_name = "apptest"
        full_key = "apptest.AppAuthor"
        ModelMeta.registry[full_key] = AppAuthor

        fk = ForeignKey("AppAuthor", on_delete="CASCADE")
        fk.model_class = AppAuthor
        result = fk.resolve_target()
        assert result is AppAuthor
...
class TestLazyFKLoadBranches:
    ...
    async def test_load_hydrates_and_caches(self):
        ...
        mock_model.objects.filter.return_value = mock_qs
        hydrated = MagicMock()
        mock_qs.all = AsyncMock(return_value=[hydrated])
        fk = ForeignKey("Target", on_delete="CASCADE")
        fk.name = "author"
        fk.resolve_target = MagicMock(return_value=mock_model)
        inst = MagicMock()
...
class TestSlugFieldValidation:
    """Tests for SlugField.validate() rejecting invalid slugs."""

    def test_valid_slug_passes(self):
        f = SlugField()
        f.name = "slug"
        f.validate("hello-world_123")
    ...
    def test_slug_none_passes_when_null_allowed(self):
        f = SlugField(null=True)
        f.name = "slug"
        f.validate(None)


class TestIPAddressFieldValidation:
    """Tests for IPAddressField.validate() rejecting invalid IPs."""
    ...
    def test_ip_none_passes_when_null_allowed(self):
        f = IPAddressField(null=True)
        f.name = "ip"
        f.validate(None)


class TestURLFieldValidation:
    """Tests for URLField.validate() rejecting invalid URLs."""
    ...
    def test_invalid_url_no_scheme_raises(self):
        ...
        with pytest.raises(ValueError, match="not a valid URL"):
            f.validate("example.com")

    def test_invalid_url_bad_scheme_raises(self):
        f = URLField()
        f.name = "url"
        with pytest.raises(ValueError, match="not a valid URL"):
            f.validate("ftp://example.com")

    def test_url_none_passes_when_null_allowed(self):
        f = URLField(null=True)
        f.name = "url"
        f.validate(None)


class TestUniqueConstraintConditionValidation:
    """Tests for UniqueConstraint condition SQL injection guard."""

    def test_valid_condition_accepted(self):
        uc = UniqueConstraint(fields=["name"], name="uc_name_active", condition="active = true")
        assert uc.condition == "active = true"

    def test_semicolon_in_condition_rejected(self):
        with pytest.raises(ValueError, match="disallowed SQL pattern"):
            UniqueConstraint(
                fields=["name"], name="uc_bad", condition="active = true; DROP TABLE users"
            )

    def test_sql_comment_in_condition_rejected(self):
        with pytest.raises(ValueError, match="disallowed SQL pattern"):
            UniqueConstraint(fields=["name"], name="uc_bad", condition="active = true -- comment")

    def test_drop_keyword_in_condition_rejected(self):
        with pytest.raises(ValueError, match="disallowed SQL pattern"):
            UniqueConstraint(fields=["name"], name="uc_bad", condition="DROP TABLE users")

    def test_none_condition_accepted(self):
        uc = UniqueConstraint(fields=["name"], name="uc_name")
        assert uc.condition is None


class TestManyToManyDescriptorSetRaises:
    """ManyToManyDescriptor.__set__ must raise AttributeError for non-None values."""

    def test_set_raises_attribute_error(self):
        mock_field = MagicMock()
        desc = ManyToManyDescriptor(field=mock_field)
        mock_instance = MagicMock()
        with pytest.raises(AttributeError, match="Cannot set ManyToMany field directly"):
            desc.__set__(mock_instance, ["tag1", "tag2"])

    def test_set_none_is_allowed(self):
        mock_field = MagicMock()
        desc = ManyToManyDescriptor(field=mock_field)
        mock_instance = MagicMock()
        desc.__set__(mock_instance, None)


# ---------------------------------------------------------------------------
# PasswordField (one-way hash, non-retrievable)
# ---------------------------------------------------------------------------
