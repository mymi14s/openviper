...
class TestMigrationWriterHelpers:
    def testsort_models_topologically(self):
        class Parent(Model):
            name = CharField()
        ...
        sorted_models = sort_models_topologically([Child, RealParent])
    ...
    def test_sort_models_circular(self):
        ...
        # Should not crash
        sorted_models = sort_models_topologically([M1, M2])
...
class TestMigrationDiff:
    def testdiff_states_new_table(self):
        current = {
            "users": {
                "columns": [
    ...
    def testdiff_states_add_column(self):
        existing = {
            "users": {
    ...
    def testdiff_states_drop_table(self):
        existing = {
            "users": {
                "columns": [{"name": "id", "type": "INTEGER"}],
                "indexes": [],
    ...
    def testdiff_states_restore_column_error(self):
        ...
        with pytest.raises(MigrationError):
            diff_states(current, existing)

        _soft_removed_columns.clear()

    def testformat_columns_with_fk(self):
        class Target(Model):
            class Meta:
                table_name = "target"
...
class TestFormatColumnsExtended:
    def test_unique_field(self):
        ...
        res = format_columns(UniqueModel)
    ...
    def test_default_value(self):
        ...
        res = format_columns(DefaultModel)
...
class TestValidateIdentifierBranch:
    def test_valid_identifier_passes(self):
        validate_identifier("my_table_123", "test context")

    def test_invalid_identifier_with_hyphen(self):
        with pytest.raises(ValueError, match="Invalid"):
            validate_identifier("my-table", "test context")

    def test_invalid_identifier_starts_with_number(self):
        with pytest.raises(ValueError, match="Invalid"):
            validate_identifier("123table", "test context")
...
class TestParseOperationBranches:
    ...
    def test_parse_rename_column(self, tmp_path):
        ...
        mig.write_text(
            ...
            'columns=[{"name": "old_col", "type": "TEXT"}]),\n'
    ...
    def test_parse_alter_column(self, tmp_path):
        ...
        mig.write_text(
            ...
            '    migrations.AlterColumn(table_name="t", column_name="val", '
            'column_type="VARCHAR(100)", nullable=False, default="hi"),\n'
            "]\n"
        )
        state = read_migrated_state(str(mig_dir))
        cols = state.get("t", {}).get("columns", [])
...
class TestFormatColumnsFK:
    def testformat_columns_includes_fk_target(self):
...
class TestParseRemoveColumnTracking:
    def test_remove_column_tracks_soft_removed(self, tmp_path):
        ...
        mig.write_text(
            ...
            '    migrations.RemoveColumn(table_name="t", column_name="col", '
            'column_type="INTEGER"),\n'
            "]\n"
        )
        _soft_removed_columns.clear()
