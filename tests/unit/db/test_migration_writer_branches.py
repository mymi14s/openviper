...
def test_validate_identifier_rejects_invalid_names():
    with pytest.raises(ValueError, match="Invalid app name"):
        validate_identifier("bad-name", "app name")

    with pytest.raises(ValueError, match="Invalid table name"):
        validate_identifier("123table", "table name")


def testdiff_states_restores_legacy_removed_column_name():
    existing = {
        "accounts": {
            "columns": [
...
# ── diff_states: circular dependency handling ────────────────────────────────


def testdiff_states_circular_dependency_does_not_crash():
...
# ── diff_states: type incompatibility error path ────────────────────────────


def testdiff_states_incompatible_type_with_soft_removed_exits():
...
# ── format_operation: all field branches ────────────────────────────────────


def testformat_operation_alter_column_no_optional_fields():
...
def testformat_operation_restore_column_text_type():
...
def testformat_operation_remove_column_text_type():
...
# ── format_columns: skip field with empty _column_type ──────────────────────
...
# ── diff_states: _removed_ prefix with incompatible type → SystemExit ────────


def testdiff_states_removed_prefix_incompatible_type_exits():
...
# ── format_operation: AddColumn with default value ──────────────────────────


def testformat_operation_add_column_with_default_includes_default():
...
def test_parse_operation_op_name_none_is_noop():
    """op_name is None (subscript call) → parse_operation returns early."""
    ...
    state: dict = {}
    parse_operation(call_node, state)
    assert state == {}
...
def test_parse_create_table_ast_constant_dict_column():
    ...
    state: dict = {}
    parse_create_table(call_node, state)
