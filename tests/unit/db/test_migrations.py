...
class TestDialectHelpers:
    def test_get_dialect_mapping(self):
        with patch("openviper.db.migrations.executor.settings") as mock_settings:
            ...
            get_dialect.cache_clear()
            assert get_dialect() == "mssql"

            mock_settings.DATABASE_URL = "oracle://..."
            get_dialect.cache_clear()
            assert get_dialect() == "oracle"
    ...
    def test_map_column_type(self):
        # Base cases
        assert map_column_type("VARCHAR(100)", "sqlite") == "VARCHAR(100)"
        assert map_column_type("BOOLEAN", "sqlite") == "INTEGER"

        # Postgres
        assert map_column_type("JSON", "postgresql") == "JSONB"
        with patch("openviper.db.migrations.executor.settings") as mock_settings:
            mock_settings.USE_TZ = True
            assert map_column_type("DATETIME", "postgresql") == "TIMESTAMP WITH TIME ZONE"

        # MySQL
        assert map_column_type("BINARY", "mysql") == "BLOB"

        # Oracle
        assert map_column_type("BOOLEAN", "oracle") == "NUMBER(1)"

    def test_quote_identifier(self):
        assert quote_identifier("table", "mysql") == "`table`"
        assert quote_identifier("table", "mssql") == "[table]"
...
class TestOperations:
    ...
    def test_create_table(self):
        ...
        with patch("openviper.db.migrations.executor.get_dialect", return_value="oracle"):
            sql = op.forward_sql()[0]
...
class TestMigrationDiscovery:
    ...
    def test_discover_migrations_legacy_path(self):
        with (
            ...
            with patch("openviper.db.migrations.executor.discover_app_migrations") as mock_discover:
...
class TestMigrationIntrospection:
    ...
    async def test_should_skip_forward_add_column(self):
        conn = MagicMock()
        # Mocking column_exists to return True
        with patch(
            "openviper.db.migrations.executor.column_exists",
            new_callable=AsyncMock,
            return_value=True,
        ):
            op = AddColumn(table_name="t", column_name="c", column_type="TEXT")
            assert await should_skip_forward(conn, op) is True

    @pytest.mark.asyncio
    async def test_should_skip_backward_add_column_not_exists(self):
        conn = MagicMock()
        # Mocking column_exists to return False
        with patch(
            "openviper.db.migrations.executor.column_exists",
            new_callable=AsyncMock,
            return_value=False,
            ...
            op = AddColumn(table_name="t", column_name="c", column_type="TEXT")
            assert await should_skip_backward(conn, op) is True
...
class TestMigrationValidation:
    ...
    async def test_validate_restore_column_type_mismatch(self):
        ...
        with patch(
            "openviper.db.migrations.executor.get_soft_removed_info", return_value=soft_info
        ):
    ...
    async def test_validate_restore_column_null_conflict(self):
        ...
        with (
            patch("openviper.db.migrations.executor.get_soft_removed_info", return_value=soft_info),
            patch("openviper.db.migrations.executor.count_null_values", return_value=5),
        ):
...
class TestShouldSkipForwardRemoveColumn:
    ...
    async def test_no_skip_remove_column_exists(self):
        ...
        with patch(
            ...
            op = RemoveColumn(table_name="t", column_name="c")
            assert await should_skip_forward(conn, op) is False
...
class TestNormalizeTypeAndTypesCompatible:
    ...
    def test_normalize_type_aliases(self):
        ...
        assert normalize_type("CHAR") == "VARCHAR"

    def test_types_compatible_same(self):
        assert types_compatible("VARCHAR(100)", "VARCHAR(255)") is True
        assert types_compatible("INT", "INTEGER") is True

    def test_types_incompatible(self):
        assert types_compatible("TEXT", "INTEGER") is False
        assert types_compatible("BOOLEAN", "VARCHAR(50)") is False
...
class TestMapColumnTypeTZBranch:
    def test_datetime_with_use_tz_postgres(self):
        ...
        with patch("openviper.db.migrations.executor.settings") as mock_settings:
            mock_settings.USE_TZ = True
            result = map_column_type("DATETIME", "postgresql")
...
class TestMigrationExecutorMigrate:
    ...
    async def test_migrate_applies_operations(self):
        ...
        with (
            ...
            patch("openviper.db.migrations.executor.get_migration_table"),
...
class TestValidateRestoreColumn:
    ...
    async def test_validate_no_soft_info(self):
        ...
        conn = AsyncMock()
        with patch(
            "openviper.db.migrations.executor.get_soft_removed_info",
            new_callable=AsyncMock,
            return_value=None,
        ):
            op = RestoreColumn(table_name="t", column_name="c")
    ...
    async def test_validate_null_values(self):
        ...
        with (
            ...
            patch(
                "openviper.db.migrations.executor.count_null_values",
                new_callable=AsyncMock,
                return_value=5,
            ),
            patch(
                "openviper.db.migrations.executor.count_total_rows",
                new_callable=AsyncMock,
                return_value=10,
            ),
        ):
...
class TestShouldSkipBackward:
    ...
    async def test_no_skip_backward_other_op(self):
        conn = AsyncMock()
        assert await should_skip_backward(conn, DropTable(table_name="t")) is False


# ── Coverage for get_soft_removed_info and _count helpers (L862-866, 905-926)
...
class TestSoftRemovedHelpers:
    ...
    async def test_get_soft_removed_info_returns_none_on_exception(self):
        conn = AsyncMock()
        with patch(
            "openviper.db.migrations.executor.get_soft_removed_table",
            side_effect=RuntimeError("boom"),
        ):
            result = await get_soft_removed_info(conn, "t", "c")
            assert result is None
    ...
    async def test_count_null_values_exception(self):
        ...
        result = await count_null_values(conn, "t", "c")
        assert result == 0
    ...
    async def test_count_total_rows_exception(self):
        conn = AsyncMock()
        conn.execute.side_effect = RuntimeError("boom")
        result = await count_total_rows(conn, "t")
        assert result == 0
    ...
    async def test_get_existing_columns_exception(self):
        ...
        with patch("openviper.db.migrations.executor.sa.inspect", side_effect=RuntimeError("boom")):
            result = get_existing_columns_sync(conn, "nonexistent")
            assert result == set()
...
class TestMigrationExecutorRollback:
    ...
    async def test_rollback_success(self):
        ...
        with (
            ...
            patch("openviper.db.migrations.executor.get_migration_table") as mock_table,
            patch("openviper.db.migrations.executor.sa"),
        ):
