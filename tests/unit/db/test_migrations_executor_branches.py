...
async def test_migrate_target_app_and_target_name_filtering():
    ...
    conn = AsyncMock()
    engine = MagicMock()
    engine.begin = lambda: begin_with(conn)
...
async def test_migrate_restore_validation_error_with_ignore_errors_continues():
    ...
    with (
        ...
        patch("openviper.db.migrations.executor.settings") as mock_settings,
        patch(
            "openviper.db.migrations.executor.should_skip_forward",
            new_callable=AsyncMock,
            return_value=False,
        ),
        patch(
...
def test_map_column_type_unknown_dialect_returns_unchanged():
    result = map_column_type("BOOLEAN", "unknown_dialect")
    assert result == "BOOLEAN"


def test_map_column_type_no_regex_match_returns_unchanged():
    result = map_column_type("123-INVALID!", "postgresql")
...
async def test_should_skip_forward_create_table_auth_users_custom_model():
    ...
    with patch("openviper.db.migrations.executor.settings") as s:
        ...
        result = await should_skip_forward(conn, op)
...
async def test_should_skip_forward_add_column_already_exists():
    ...
    with patch(
        "openviper.db.migrations.executor.column_exists", new_callable=AsyncMock, return_value=True
    ):
        result = await should_skip_forward(conn, op)
    assert result is True
...
async def test_should_skip_backward_returns_false_for_run_sql():
    conn = AsyncMock()
    op = RunSQL("SELECT 1")
    result = await should_skip_backward(conn, op)
    assert result is False
...
def test_quote_identifier_mssql():
    assert quote_identifier("my_table", "mssql") == "[my_table]"
...
async def test_rollback_executes_backward_sql():
    ...
    with (
        ...
        patch("openviper.db.migrations.executor.sa"),
        patch(
            "openviper.db.migrations.executor.should_skip_backward",
            new_callable=AsyncMock,
            return_value=False,
        ),
    ):
...
async def test_rollback_raises_when_migration_not_found():
    executor = MigrationExecutor()
    engine = MagicMock()
    engine.begin = lambda: begin_with(AsyncMock())
...
# ── get_existing_columns_sync and column_exists ────────────────────────────
...
def test_get_existing_columns_sync_returns_column_name_set():
    ...
    with patch("openviper.db.migrations.executor.sa") as mock_sa:
        ...
        result = get_existing_columns_sync(mock_conn, "users")
...
async def test_column_exists_when_present():
    ...
    result = await column_exists(mock_conn, "users", "email")
    assert result is True
...
async def test_column_exists_when_absent():
    ...
    result = await column_exists(mock_conn, "users", "missing_col")
    assert result is False


# ── get_soft_removed_info: row-found path ────────────────────────────────────
...
async def test_get_soft_removed_info_returns_dict_when_row_found():
    ...
    with (
        patch("openviper.db.migrations.executor.get_soft_removed_table"),
        patch("openviper.db.migrations.executor.sa"),
    ):
        result = await get_soft_removed_info(mock_conn, "users", "deleted_at")
...
# ── count_null_values: row-found path ───────────────────────────────────────
...
async def test_count_null_values_returns_row_count_when_row_found():
    ...
    with (
        patch("openviper.db.migrations.executor.get_dialect", return_value="postgresql"),
        patch(
            "openviper.db.migrations.executor.quote_identifier", side_effect=lambda n, _: f'"{n}"'
        ),
        patch("openviper.db.migrations.executor.sa") as mock_sa,
    ):
        mock_sa.text.return_value = MagicMock()
        result = await count_null_values(mock_conn, "items", "col")
...
# ── migrate: should_skip_forward returns True → continue ────────────────────
...
async def test_migrate_invalidates_soft_removed_cache_after_applying():
    ...
    with (
        ...
        patch("openviper.db.migrations.executor.settings") as mock_settings,
        patch("openviper.db.migrations.executor.timezone") as mock_timezone,
        patch("openviper.db.migrations.executor.invalidate_soft_removed_cache") as mock_invalidate,
    ):
        mock_settings.USE_TZ = False
        mock_settings.INSTALLED_APPS = []
        mock_timezone.now.return_value = MagicMock(replace=MagicMock(return_value="now"))
...
# ── rollback: should_skip_backward returns True → continue ──────────────────
