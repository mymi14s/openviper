...
class TestBulkOperations:
    ...
    async def test_bulk_create(self):
        ...
        with (
            ...
            patch("openviper.db.models.begin", return_value=mock_begin_ctx),
        ):
...
class TestCheckPermCached:
    ...
    async def test_ignore_permissions_returns_early(self):
        with patch(
            ...
            await check_perm_cached(Model, "read", ignore_permissions=True)
    ...
    async def test_no_request_context_falls_through(self):
        ...
        try:
            with patch(
                ...
                await check_perm_cached(Model, "read")
    ...
    async def test_cache_miss_calls_check_and_stores(self):
        ...
        try:
            with patch(
                ...
                await check_perm_cached(Model, "create")
...
class TestCallHook:
    ...
    async def test_sync_hook(self):
        ...
        result = await call_hook(sync_fn)
        assert result == 42
    ...
    async def test_async_hook(self):
        ...
        result = await call_hook(async_fn)
        assert result == 99
    ...
    async def test_sync_hook_with_args(self):
        ...
        result = await call_hook(sync_fn, 3, 4)
        assert result == 7
    ...
    async def test_async_hook_with_args(self):
        ...
        result = await call_hook(async_fn, 5)
        assert result == 10


class TLAuthor(Model):
    username = CharField()
...
class TLBlog(Model):
    title = CharField()
    author = ForeignKey(TLAuthor, on_delete="CASCADE")
...
class TestQuerySetOperations:
    ...
    async def test_exists_permission_denied_returns_false(self):
        with patch(
            "openviper.db.models.check_perm_cached",
            new_callable=AsyncMock,
            side_effect=ModelPermissionError("denied"),
        ):
            result = await self.Post.objects.all().exists()
...
class QSSRBook(Model):
    title = CharField()
    writer = ForeignKey(QSSRWriter, on_delete="CASCADE")
...
class QSPRProduct(Model):
    name = CharField()
    category = ForeignKey(QSPRCategory, on_delete="CASCADE")
...
class CFKPet(Model):
    name = CharField()
    owner = ForeignKey(CFKOwner, on_delete="CASCADE")
...
class TestModelValidate:
    ...
    async def test_validate_raises_on_invalid(self):
        ...
        with (
            patch("openviper.db.models.load_soft_removed_columns", new_callable=AsyncMock),
...
class TestModelInitUnknownKwargs:
    ...
    def test_private_kwarg_raises_type_error(self):
        ...
        with pytest.raises(TypeError, match="private keyword argument"):
            self.Profile(name="Alice", _internal="secret")
...
class TestCursorEncoding:
    """Tests for cursor_encode and cursor_decode helpers."""

    def test_encode_decode_roundtrip(self):
        values = {"id": 42, "name": "Alice"}
        encoded = cursor_encode(values)
        decoded = cursor_decode(encoded)
...
class TestBuildKeysetQ:
    """Tests for build_keyset_q keyset pagination helper."""

    def test_single_asc_field(self):
        result = build_keyset_q(["id"], {"id": 10})
        assert result is not None
        assert isinstance(result, Q)

    def test_single_desc_field(self):
        result = build_keyset_q(["-created_at"], {"created_at": "2025-01-01"})
        assert result is not None

    def test_missing_cursor_value_returns_none(self):
        result = build_keyset_q(["id"], {})
        assert result is None

    def test_empty_order_fields_returns_none(self):
        result = build_keyset_q([], {"id": 1})
        assert result is None


class TestIndexConditionValidation:
    """Tests for Index condition SQL injection guard."""

    def test_valid_condition_accepted(self):
        idx = Index(fields=["name"], condition="active = true")
        assert idx.condition == "active = true"

    def test_semicolon_in_condition_rejected(self):
        with pytest.raises(ValueError, match="disallowed SQL pattern"):
            Index(fields=["name"], condition="active = true; DROP TABLE users")

    def test_sql_comment_in_condition_rejected(self):
        with pytest.raises(ValueError, match="disallowed SQL pattern"):
            Index(fields=["name"], condition="active = true -- comment")

    def test_drop_keyword_in_condition_rejected(self):
        with pytest.raises(ValueError, match="disallowed SQL pattern"):
            Index(fields=["name"], condition="DROP TABLE users")
...
class TestModelComparisonOperators:
    """Tests for Model __lt__, __le__, __gt__, __ge__ comparison operators."""

    class Item(Model):
        name = CharField()

        class Meta:
            table_name = "comp_items"

    def test_lt_with_pks(self):
        a = self.Item(name="a")
        a.id = 1
        b = self.Item(name="b")
        b.id = 2
    ...
    def test_lt_returns_notimplemented_for_none_pk(self):
        a = self.Item(name="a")
        b = self.Item(name="b")
        assert a.__lt__(b) is NotImplemented

    def test_gt_returns_notimplemented_for_different_class(self):
        a = self.Item(name="a")
        a.id = 1
        assert a.__gt__(42) is NotImplemented

    def test_lt_returns_notimplemented_for_different_class(self):
        a = self.Item(name="a")
        a.id = 1
...
class TestTextChoice:
    """Tests for TextChoice enum with value + label."""

    def test_text_choice_value_and_label(self):
        class Status(TextChoice):
    ...
    def test_text_choice_choices(self):
        class Status(TextChoice):
            ACTIVE = "active", "Active"
            INACTIVE = "inactive", "Inactive"
        ...
        assert ("inactive", "Inactive") in choices

    def test_text_choice_values(self):
        class Status(TextChoice):
            ACTIVE = "active", "Active"
            INACTIVE = "inactive", "Inactive"

        assert Status.values == ["active", "inactive"]

    def test_text_choice_labels(self):
        class Status(TextChoice):
            ACTIVE = "active", "Active"
            INACTIVE = "inactive", "Inactive"

        assert Status.labels == ["Active", "Inactive"]

    def test_text_choice_names(self):
        class Status(TextChoice):
            ACTIVE = "active", "Active"
            INACTIVE = "inactive", "Inactive"

        assert Status.names == ["ACTIVE", "INACTIVE"]

    def test_text_choice_from_value(self):
        class Status(TextChoice):
            ACTIVE = "active", "Active"

        result = Status.from_value("active")
        assert result == Status.ACTIVE

    def test_text_choice_from_label(self):
        class Status(TextChoice):
            ACTIVE = "active", "Active"

        result = Status.from_label("Active")
        assert result == Status.ACTIVE

    def test_text_choice_from_label_invalid_raises(self):
        class Status(TextChoice):
            ACTIVE = "active", "Active"

        with pytest.raises(ValueError, match="not a valid label"):
            Status.from_label("Nonexistent")


class TestSelectForUpdateValidation:
    """Tests for select_for_update() parameter validation."""

    def test_nowait_and_skip_locked_raises(self):
        qs = QuerySet(Model)
        with pytest.raises(ValueError, match="cannot use both"):
            qs.select_for_update(nowait=True, skip_locked=True)


class TestModelAsDictAsJson:
    """Tests for Model.as_dict() and as_json() methods."""
    ...
    class Article(Model):
        ...
        class Meta:
            table_name = "articles_asdict"

    def test_as_dict_basic(self):
        a = self.Article(title="Hello", body="World")
        a.id = 1
        d = a.as_dict()
    ...
    def test_as_dict_with_relations(self):
        a = self.Article(title="Hello")
        a.id = 1
        a._relation_cache = {"author": self.Article(title="Inner")}
        a._relation_cache["author"].id = 2
        d = a.as_dict(include_relations=True)
        assert "author" in d
    ...
    def test_as_json_with_indent(self):
        a = self.Article(title="Hello")
        a.id = 1
        result = a.as_json(indent=2)
        assert "\n" in result
