...
from openviper.db.tools.backup.mariadb import (
    MariaDBBackupEngine,
    build_mysql_args,
    mysql_defaults_file,
)
...
class TestMariaDBBackupEngineDump:
    ...
    def test_defaults_file_rejects_control_characters(self) -> None:
        with pytest.raises(ValidationError, match="control characters"):
            with mysql_defaults_file("safe\nuser=attacker"):
                pass
...
class TestMariaDBBackupEngineRestore:
    ...
    async def test_calls_mysql_with_file(self, tmp_path: Path) -> None:
        ...
        async def fake_run(args, env=None, timeout=3600, stdin_data=None):
