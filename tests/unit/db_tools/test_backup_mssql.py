...
from openviper.db.tools.backup.mssql import (
    MSSQLBackupEngine,
    escape_mssql_identifier,
    parse_mssql_url,
)
...
class TestParseMssqlUrl:
    def test_parses_user_password_host_db(self) -> None:
        conn = parse_mssql_url("mssql://sa:pwd@sqlhost/mydb")
        assert conn["user"] == "sa"
    ...
    def test_default_port_1433(self) -> None:
        conn = parse_mssql_url("mssql://sa@sqlhost/mydb")
        assert conn["port"] == "1433"

    def test_pyodbc_driver_stripped(self) -> None:
        conn = parse_mssql_url("mssql+pyodbc://sa@host/db")
        assert conn["host"] == "host"

    def test_aioodbc_driver_stripped(self) -> None:
        conn = parse_mssql_url("mssql+aioodbc://sa@host/db")
        assert conn["host"] == "host"

    def test_escapes_bracket_delimited_identifier(self) -> None:
        assert escape_mssql_identifier("db]name") == "db]]name"
...
class TestMSSQLBackupEngineRestore:
    ...
    async def test_force_includes_replace_keyword(self, tmp_path: Path) -> None:
        ...
        async def fake_run(args, env=None, timeout=3600):
            captured.append(args)
            query_path = Path(args[args.index("-i") + 1])
            captured_sql.append(query_path.read_text(encoding="utf-8"))
            return (0, "", "")
        ...
        assert "REPLACE" in captured_sql[0]
