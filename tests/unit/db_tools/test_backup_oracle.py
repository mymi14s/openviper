...
from openviper.db.tools.backup.oracle import OracleBackupEngine, parse_oracle_url


class TestParseOracleUrl:
    def test_parses_user_password_host_service(self) -> None:
        conn = parse_oracle_url("oracle://admin:pwd@orahost:1521/ORCL")
    ...
    def test_default_port_1521(self) -> None:
        conn = parse_oracle_url("oracle://admin@orahost/ORCL")
        assert conn["port"] == "1521"

    def test_oracledb_driver_stripped(self) -> None:
        conn = parse_oracle_url("oracle+oracledb://admin@host/SVC")
