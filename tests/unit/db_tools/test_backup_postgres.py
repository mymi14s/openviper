...
from openviper.db.tools.backup.postgres import PostgresBackupEngine, build_pg_env


class TestBuildPgEnv:
    def test_parses_host_user_db(self) -> None:
        flags, env = build_pg_env("postgresql://user@host/mydb")
        assert "-h" in flags
    ...
    def test_password_goes_to_env_not_flags(self) -> None:
        flags, env = build_pg_env("postgresql://user:secret@host/db")
    ...
    def test_port_included_when_present(self) -> None:
        flags, _ = build_pg_env("postgresql://user@host:5434/db")
        assert "-p" in flags
        assert "5434" in flags

    def test_asyncpg_driver_stripped(self) -> None:
        flags, _ = build_pg_env("postgresql+asyncpg://user@host/db")
        assert "-h" in flags

    def test_no_password_pgpassword_absent(self) -> None:
        _, env = build_pg_env("postgresql://user@host/db")
