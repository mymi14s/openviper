...
class TestRequestBody:
    ...
    async def test_json_rejects_trailing_comma_with_http_400(self):
        payload = b'{"score": 1200,}'
        req = Request(
            make_scope(headers=[(b"content-type", b"application/json")]),
            receive=make_receive(payload),
        )
...
class TestFormElseBranch:
    ...
    @pytest.mark.asyncio
    async def test_multipart_without_boundary_returns_empty_form(self):
        """Multipart content-type without a boundary parameter returns an empty form."""
        scope = {
            "type": "http",
            "method": "POST",
            "path": "/",
    ...
    @pytest.mark.asyncio
    async def test_multipart_without_boundary_returns_cached_form(self):
        """Second call to form() after boundary-less multipart returns the cached form."""
        scope = {
            "type": "http",
            "method": "POST",
            "path": "/",
    ...
    @pytest.mark.asyncio
    async def test_multipart_import_error_raises(self):
        """ImportError is raised when python-multipart is not installed."""
        with (
            patch.object(_req_mod, "FormParser", None),
            patch.object(_req_mod, "parse_options_header", None),
        ):
            scope = {
                "type": "http",
                "method": "POST",
