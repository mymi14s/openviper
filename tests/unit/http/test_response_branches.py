...
class TestImportErrorHandling:
    ...
    def testget_jinja2_env_success_path(self):
        """Should return jinja2 environment when Environment is available."""
        get_jinja2_env.cache_clear()
        result = get_jinja2_env(("/tmp",))
        assert result is not None


class TestComputeTemplateSearchPaths:
    """Test compute_template_search_paths function."""
    ...
    def test_handles_import_error_for_app(self):
        ...
        result = compute_template_search_paths(base_dir, installed_apps)
    ...
    def test_handles_app_without_file_attribute(self):
        ...
        with patch("openviper.http.response.importlib.import_module", return_value=mock_module):
            result = compute_template_search_paths(base_dir, ("test.app",))
...
class TestCacheClear:
    ...
    def testcache_clear_clears_both_caches(self):
        """Should clear both get_jinja2_env and compute_template_search_paths caches."""
        with patch("openviper.http.response.get_jinja2_env") as mock_get_env:
            with patch("openviper.http.response.compute_template_search_paths") as mock_compute:
