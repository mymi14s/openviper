...
from openviper.auth.authentications import (
    get_user_cached as get_user_cached,
)
...
def bearer_scope(token):
    return http_scope([(b"authorization", f"Bearer {token}".encode("latin-1"))])


def cookie_scope(cookie_val):
...
class TestAuthMiddlewareJWT:
    ...
    async def test_valid_jwt_sets_user(self):
        ...
        with patch(
            ...
            with patch(
                ...
                await mw(bearer_scope(token), None, None)
    ...
    async def test_malformed_jwt_falls_to_anonymous(self):
        ...
        scope = http_scope([(b"authorization", b"Bearer not.a.valid.token")])
...
class TestAuthMiddlewareSession:
    ...
    async def test_valid_session_sets_user(self):
        ...
        scope = cookie_scope("sessionid=abc")
    ...
    async def test_inactive_session_user_becomes_anonymous(self):
        ...
        scope = cookie_scope("sessionid=fake")
        scope["user"] = inactive
    ...
    async def test_session_auth_exception_falls_to_anonymous(self):
        ...
        scope = cookie_scope("sessionid=bad")
    ...
    async def test_session_returns_none_falls_to_anonymous(self):
        ...
        scope = cookie_scope("sessionid=x")
...
class TestAuthMiddlewareContextVar:
    ...
    async def test_context_var_set_during_request(self):
        ...
        await AuthenticationMiddleware(app)(http_scope(), None, None)
    ...
    async def test_context_var_reset_after_request(self):
        ...
        try:
            await AuthenticationMiddleware(noop)(http_scope(), None, None)
...
class TestUserCache:
    ...
    async def test_cache_hit_avoids_db_call(self):
        ...
        with patch(
            ...
            u1 = await get_user_cached(100)
            u2 = await get_user_cached(100)
    ...
    async def test_cache_miss_calls_db(self):
        ...
        with patch(
            ...
            r1 = await get_user_cached(200)
            r2 = await get_user_cached(201)
            assert r1 is user_a
            assert r2 is user_b
    ...
    async def test_expired_entry_refetched(self):
        ...
        with patch(
            ...
            await get_user_cached(300)
            # Manually expire the entry
            USER_CACHE[300] = (USER_CACHE[300][0], time.monotonic() - 1)
            await get_user_cached(300)
            assert mock_get.await_count == 2
    ...
    async def test_eviction_when_cache_full(self):
        ...
        with patch(
            ...
            for i in range(4096):
                USER_CACHE[i] = (fake_user, time.monotonic() - 9999)
            # Next insertion must trigger eviction - cache size must not exceed maxsize+1
            await get_user_cached(99999)
            assert len(USER_CACHE) <= 4096
    ...
    async def test_concurrent_cache_access_safe(self):
        ...
        with patch("openviper.auth.authentications.get_user_by_id", new=fake_get_user):
            ...
            await get_user_cached(400)
            await asyncio.gather(*[get_user_cached(400) for _ in range(10)])
...
class TestUserCacheLock:
    ...
    def test_same_lock_returned(self):
        lock1 = get_user_cache_lock()
        lock2 = get_user_cache_lock()
    ...
    def test_thread_safe_initialization_guard_exists(self):
        """LOCK_INIT_GUARD must exist to prevent TOCTOU races at lock creation."""
        auth_module = sys.modules[get_user_cache_lock.__module__]
        assert hasattr(auth_module, "LOCK_INIT_GUARD")
...
class TestUserCacheLRUEviction:
    ...
    async def test_evicts_oldest_when_no_expired_entries(self):
        ...
        with patch.object(_auth_mod, "USER_CACHE_MAXSIZE", 2):
            USER_CACHE[1001] = (fake_user, future_exp)
            USER_CACHE[1002] = (fake_user, future_exp)
            ...
            with patch("openviper.auth.authentications.get_user_by_id", new=fake_get_user):
                ...
                await get_user_cached(1003)

        # 1001 (oldest) should have been evicted
        assert 1001 not in USER_CACHE
        # New user should be present
        assert 1003 in USER_CACHE
