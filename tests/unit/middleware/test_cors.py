...
def make_scope(method="GET", path="/", origin=None, headers=None):
    h = list(headers or [])
    if origin:
...
class TestOriginAllowed:
    ...
    async def test_allow_all(self):
        mw = CORSMiddleware(ok_app, allowed_origins=["*"])
        hd, _ = await run(mw, make_scope(origin="https://example.com"))
        assert b"access-control-allow-origin" in hd

    @pytest.mark.asyncio
    async def test_exact_match_allowed(self):
        mw = CORSMiddleware(ok_app, allowed_origins=["https://example.com"])
        hd, _ = await run(mw, make_scope(origin="https://example.com"))
        assert b"access-control-allow-origin" in hd
    ...
    async def test_exact_match_denied(self):
        ...
        await mw(make_scope(origin="https://other.com"), None, None)
        assert "app" in calls

    @pytest.mark.asyncio
    async def test_wildcard_pattern_allowed(self):
        mw = CORSMiddleware(ok_app, allowed_origins=["https://*.example.com"])
        hd, _ = await run(mw, make_scope(origin="https://sub.example.com"))
        assert b"access-control-allow-origin" in hd
    ...
    async def test_multiple_exact_origins(self):
        for origin in ("https://a.com", "https://b.com"):
            mw = CORSMiddleware(ok_app, allowed_origins=["https://a.com", "https://b.com"])
            hd, _ = await run(mw, make_scope(origin=origin))
            assert b"access-control-allow-origin" in hd
    ...
    async def test_exact_and_wildcard_combined(self):
        ...
        hd, _ = await run(mw, make_scope(origin="https://sub.wild.com"))
        assert b"access-control-allow-origin" in hd
...
class TestCORSPreflight:
    ...
    async def test_preflight_returns_204(self):
        mw = CORSMiddleware(noop_app, allowed_origins=["https://example.com"])
        _, status = await run(mw, make_scope(method="OPTIONS", origin="https://example.com"))
        assert status == 204

    @pytest.mark.asyncio
    async def test_preflight_allowed_has_origin_header(self):
        mw = CORSMiddleware(noop_app, allowed_origins=["https://example.com"])
        hd, _ = await run(mw, make_scope(method="OPTIONS", origin="https://example.com"))
        assert b"access-control-allow-origin" in hd

    @pytest.mark.asyncio
    async def test_preflight_disallowed_origin(self):
        mw = CORSMiddleware(noop_app, allowed_origins=["https://allowed.com"])
        hd, _ = await run(mw, make_scope(method="OPTIONS", origin="https://evil.com"))
    ...
    async def test_preflight_all_origins_echoes_origin(self):
        mw = CORSMiddleware(noop_app, allowed_origins=["*"])
        hd, _ = await run(mw, make_scope(method="OPTIONS", origin="https://foo.com"))
        assert hd[b"access-control-allow-origin"] == b"https://foo.com"

    @pytest.mark.asyncio
    async def test_preflight_contains_methods_header(self):
        mw = CORSMiddleware(noop_app, allowed_origins=["*"], allowed_methods=["GET", "POST"])
        hd, _ = await run(mw, make_scope(method="OPTIONS", origin="https://foo.com"))
    ...
    async def test_preflight_max_age(self):
        mw = CORSMiddleware(noop_app, allowed_origins=["*"], max_age=1200)
        hd, _ = await run(mw, make_scope(method="OPTIONS", origin="https://foo.com"))
    ...
    async def test_preflight_body_is_empty(self):
        ...
        mw = CORSMiddleware(noop_app, allowed_origins=["*"])
        await mw(make_scope(method="OPTIONS", origin="https://foo.com"), None, _send)
...
class TestCORSNonPreflight:
    ...
    async def test_adds_cors_headers_to_response(self):
        mw = CORSMiddleware(ok_app, allowed_origins=["https://example.com"])
        hd, _ = await run(mw, make_scope(method="GET", origin="https://example.com"))
        assert b"access-control-allow-origin" in hd
    ...
    async def test_disallowed_origin_passthrough(self):
        ...
        await mw(make_scope(method="GET", origin="https://evil.com"), None, None)
        assert "app" in calls

    @pytest.mark.asyncio
    async def test_credentials_header_added(self):
        mw = CORSMiddleware(ok_app, allowed_origins=["https://example.com"], allow_credentials=True)
        hd, _ = await run(mw, make_scope(origin="https://example.com"))
    ...
    async def test_no_credentials_header_when_disabled(self):
        mw = CORSMiddleware(
            ok_app, allowed_origins=["https://example.com"], allow_credentials=False
        )
        hd, _ = await run(mw, make_scope(origin="https://example.com"))
    ...
    async def test_post_with_allowed_origin(self):
        mw = CORSMiddleware(ok_app, allowed_origins=["https://example.com"])
        hd, status = await run(mw, make_scope(method="POST", origin="https://example.com"))
        assert status == 200
        assert b"access-control-allow-origin" in hd
...
class TestCORSVaryHeader:
    ...
    async def test_vary_origin_not_set_for_wildcard(self):
        mw = CORSMiddleware(ok_app, allowed_origins=["*"])
        hd, _ = await run(mw, make_scope(origin="https://anything.com"))
...
class TestCORSConfiguration:
    ...
    async def test_expose_headers_in_response(self):
        mw = CORSMiddleware(ok_app, allowed_origins=["*"], expose_headers=["X-Custom"])
        hd, _ = await run(mw, make_scope(origin="https://foo.com"))
    ...
    async def test_no_credentials_response(self):
        mw = CORSMiddleware(ok_app, allowed_origins=["*"], allow_credentials=False)
        hd, _ = await run(mw, make_scope(origin="https://foo.com"))
        assert b"access-control-allow-credentials" not in hd
    ...
    async def test_explicit_headers_in_preflight(self):
        mw = CORSMiddleware(noop_app, allowed_origins=["*"], allowed_headers=["Content-Type"])
        hd, _ = await run(mw, make_scope(method="OPTIONS", origin="https://foo.com"))
        assert b"content-type" in hd[b"access-control-allow-headers"]

    @pytest.mark.asyncio
    async def test_wildcard_methods_expands_to_all(self):
        mw = CORSMiddleware(noop_app, allowed_origins=["*"], allowed_methods=["*"])
        hd, _ = await run(mw, make_scope(method="OPTIONS", origin="https://foo.com"))
...
class TestCORSExposeHeaders:
    ...
    async def test_preflight_includes_expose_headers(self):
        mw = CORSMiddleware(noop_app, allowed_origins=["*"], expose_headers=["X-Custom"])
        hd, _ = await run(mw, make_scope(method="OPTIONS", origin="https://foo.com"))
        assert b"access-control-expose-headers" in hd
    ...
    async def test_non_preflight_includes_expose_headers(self):
        mw = CORSMiddleware(ok_app, allowed_origins=["*"], expose_headers=["X-Custom"])
        hd, _ = await run(mw, make_scope(method="GET", origin="https://foo.com"))
        assert b"access-control-expose-headers" in hd
...
class TestCORSOriginSpoofing:
    ...
    async def test_subpath_origin_not_matched_as_prefix(self):
        ...
        mw = CORSMiddleware(ok_app, allowed_origins=["https://example.com"])
        hd, _ = await run(mw, make_scope(origin="https://evil-example.com"))
        assert b"access-control-allow-origin" not in hd
    ...
    async def test_http_vs_https_not_matched(self):
        ...
        mw = CORSMiddleware(ok_app, allowed_origins=["https://example.com"])
        hd, _ = await run(mw, make_scope(origin="http://example.com"))
        assert b"access-control-allow-origin" not in hd
...
class TestCORSFromSettings:
    ...
    def _build_stack(self, **cors_overrides):
        ...
        return build_middleware_stack(ok_app, [(CORSMiddleware, cors_kwargs)])
    ...
    async def test_allow_credentials_true(self):
        ...
        hd, _ = await run(stack, make_scope(origin="https://frontend.example.com"))
        assert hd.get(b"access-control-allow-credentials") == b"true"
    ...
    async def test_allow_credentials_false(self):
        ...
        hd, _ = await run(stack, make_scope(origin="https://example.com"))
        assert b"access-control-allow-credentials" not in hd
    ...
    async def test_allowed_origins_restricts(self):
        ...
        hd, _ = await run(stack, make_scope(origin="https://trusted.example.com"))
        assert hd.get(b"access-control-allow-origin") == b"https://trusted.example.com"

        hd2, _ = await run(stack, make_scope(origin="https://evil.com"))
    ...
    async def test_empty_origins_falls_back_to_wildcard(self):
        ...
        hd, _ = await run(stack, make_scope(origin="https://any-origin.com"))
    ...
    async def test_max_age_in_preflight(self):
        ...
        scope = make_scope(method="OPTIONS", path="/api/", headers=preflight_headers)
        hd, _ = await run(stack, scope)
...
class TestCORSFromSettingsConfig:
    """CORSMiddleware reads CORS_* settings when constructor args are None."""

    @pytest.mark.asyncio
    async def test_allowed_origins_from_settings(self):
        """CORS_ALLOWED_ORIGINS setting populates allowed_origins."""
        with patch("openviper.middleware.cors.settings") as mock_settings:
            mock_settings.CORS_ALLOW_CREDENTIALS = False
            mock_settings.CORS_MAX_AGE = 600
            mock_settings.CORS_ALLOWED_ORIGINS = ("https://from-settings.com",)
            mock_settings.CORS_ALLOWED_METHODS = None
            mock_settings.CORS_ALLOWED_HEADERS = None
            mw = CORSMiddleware(ok_app)
            hd, _ = await run(mw, make_scope(origin="https://from-settings.com"))
            assert b"access-control-allow-origin" in hd

    @pytest.mark.asyncio
    async def test_expose_headers_in_preflight_response(self):
        """Expose-headers must appear in preflight (OPTIONS) responses."""
        mw = CORSMiddleware(
            noop_app,
        ...
        hd, _ = await run(mw, make_scope(method="OPTIONS", origin="https://example.com"))
        assert b"access-control-expose-headers" in hd
        assert b"X-Request-Id" in hd[b"access-control-expose-headers"]
