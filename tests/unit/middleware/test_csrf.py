...
async def collect_status(mw, scope):
    messages = []
...
class TestCSRFTokenMasking:
    def test_mask_produces_96_char_string(self):
        masked = mask_csrf_token(generate_csrf_token(), "secret")
    ...
    def test_different_salts_same_token(self):
        ...
        token = generate_csrf_token()
        m1 = mask_csrf_token(token, "secret")
        m2 = mask_csrf_token(token, "secret")
    ...
    def test_mask_with_empty_secret_still_produces_96_chars(self):
        ...
        masked = mask_csrf_token(generate_csrf_token(), "")
        assert len(masked) == 96  # 32 hex (16 bytes salt) + 64 hex (SHA256)
...
class TestCookieExtraction:
    def test_extracts_correct_cookie(self):
        assert (
            extract_cookie_value("session=abc; csrftoken=xyz123; other=val", "csrftoken")
            == "xyz123"
        )

    def test_missing_cookie(self):
        assert extract_cookie_value("session=abc; other=val", "csrftoken") == ""

    def test_empty_header(self):
        assert extract_cookie_value("", "csrftoken") == ""

    def test_first_cookie(self):
        assert extract_cookie_value("csrftoken=first; other=second", "csrftoken") == "first"

    def test_no_prefix_match(self):
        """'notcsrftoken=x' must not match 'csrftoken'."""
        assert extract_cookie_value("notcsrftoken=x", "csrftoken") == ""

    def test_cookie_with_spaces_around_semicolon(self):
        assert extract_cookie_value("a=1 ;  csrftoken=tok ; b=2", "csrftoken") == "tok"
...
class TestCSRFMiddlewareSafeMethods:
    ...
    async def test_get_passes_through(self):
        ...
        await mw(make_scope(method="GET"), None, None)
        assert "GET" in calls
    ...
    async def test_head_passes_through(self):
        ...
        mw = CSRFMiddleware(app, secret="s")
        await mw(make_scope(method="HEAD"), None, None)
        assert "app" in calls
    ...
    async def test_options_passes_through(self):
        ...
        mw = CSRFMiddleware(app, secret="s")
        await mw(make_scope(method="OPTIONS"), None, None)
        assert "app" in calls
    ...
    async def test_trace_requires_csrf(self):
        ...
        await mw(make_scope(method="TRACE"), None, capture_send)
...
class TestCSRFExemptPaths:
    ...
    async def test_exempt_path_skips_validation(self):
        ...
        await mw(make_scope(path="/api/webhook"), None, None)
        assert "app" in calls
...
class TestCSRFMiddlewareValidation:
    ...
    async def test_post_with_cookie_only_returns_403(self):
        token = generate_csrf_token()
        scope = make_scope(headers=[(b"cookie", f"csrftoken={token}".encode())])
        status = await collect_status(CSRFMiddleware(lambda *a: None, secret="s"), scope)
        assert status == 403

    @pytest.mark.asyncio
    async def test_post_with_header_only_returns_403(self):
        token = generate_csrf_token()
        masked = mask_csrf_token(token, "s")
        scope = make_scope(headers=[(b"x-csrftoken", masked.encode())])
        status = await collect_status(CSRFMiddleware(lambda *a: None, secret="s"), scope)
        assert status == 403
    ...
    async def test_wrong_secret_returns_403(self):
        token = generate_csrf_token()
        masked = mask_csrf_token(token, "other-secret")
        scope = make_scope(
            headers=[
                (b"cookie", f"csrftoken={token}".encode()),
                (b"x-csrftoken", masked.encode()),
