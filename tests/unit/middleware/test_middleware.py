...
class TestCORSMiddleware:
    ...
    def test_origin_allowed_exact_match(self):
        ...
        assert mw.origin_allowed("https://exact.com") is True
        assert mw.origin_allowed("https://other.com") is False

    def test_origin_allowed_wildcard_pattern(self):
        mw = CORSMiddleware(dummy_app, allowed_origins=["https://*.example.com"])
        assert mw.origin_allowed("https://sub.example.com") is True
        assert mw.origin_allowed("https://evil.io") is False
...
class TestCSRFHelpers:
    def testgenerate_csrf_token_length(self):
        token = generate_csrf_token()
        assert len(token) == 64  # secrets.token_hex(32)

    def testgenerate_csrf_token_is_hex(self):
        token = generate_csrf_token()
    ...
    def test_mask_and_verify_token(self):
        ...
        assert verify_csrf_token(token, masked, secret) is True
    ...
    def test_verify_wrong_token_fails(self):
        ...
        assert verify_csrf_token(other_token, masked, secret) is False

    def test_verify_too_short_fails(self):
        assert verify_csrf_token("cookie", "short", "secret") is False

    def testextract_cookie_value(self):
        header = "session=abc; csrftoken=xyz123; other=val"
        assert extract_cookie_value(header, "csrftoken") == "xyz123"

    def testextract_cookie_value_not_found(self):
        assert extract_cookie_value("a=1; b=2", "missing") == ""

    def testextract_cookie_value_empty_header(self):
        assert extract_cookie_value("", "name") == ""
