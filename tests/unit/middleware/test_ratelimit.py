...
from openviper.middleware.ratelimit import RateLimitMiddleware, SlidingWindowCounter, rate_limit


def make_scope(client=("127.0.0.1", 8000), headers=None):
    return {
        "type": "http",
        "method": "GET",
        "path": "/",
...
class TestSlidingWindowCounter:
    ...
    async def test_window_expiry_resets_count(self):
        ...
        counter = SlidingWindowCounter(max_requests=1, window_seconds=0.05)
...
class TestSlidingWindowEviction:
    ...
    async def test_stale_entries_evicted_via_short_window(self):
        ...
        counter = SlidingWindowCounter(max_requests=1, window_seconds=0.02, evict_interval=0.0)
...
class TestRateLimitMiddleware:
    ...
    async def test_allows_request_within_limit(self):
        ...
        msgs = await run_mw(mw, make_scope())
    ...
    async def test_blocks_request_over_limit(self):
        ...
        scope = make_scope(client=("1.2.3.4", 9000))
        await run_mw(mw, scope)  # 1st - allowed
        msgs = await run_mw(mw, scope)  # 2nd - blocked
    ...
    async def test_rate_limit_headers_added(self):
        ...
        msgs = await run_mw(mw, make_scope(client=("2.3.4.5", 9000)))
    ...
    async def test_429_includes_retry_after(self):
        ...
        scope = make_scope(client=("3.4.5.6", 9000))
    ...
    async def test_429_ratelimit_remaining_is_zero(self):
        ...
        mw = RateLimitMiddleware(app, max_requests=1, window_seconds=60)
        scope = make_scope(client=("4.5.6.7", 9000))
        await run_mw(mw, scope)
        msgs = await run_mw(mw, scope)
        hd = dict(msgs[0]["headers"])
    ...
    async def test_custom_key_func(self):
        ...
        scope_a = make_scope(client=("10.0.0.1", 9000))
        scope_b = make_scope(client=("10.0.0.2", 9000))
        await run_mw(mw, scope_a)
        msgs = await run_mw(mw, scope_b)  # different IP, same key → blocked
        assert msgs[0]["status"] == 429
    ...
    async def test_different_ips_tracked_independently(self):
        ...
        mw = RateLimitMiddleware(app, max_requests=1, window_seconds=60)
        await run_mw(mw, make_scope(client=("5.5.5.5", 9000)))  # exhaust 5.5.5.5
        msgs = await run_mw(mw, make_scope(client=("6.6.6.6", 9000)))  # 6.6.6.6 still fresh
        assert msgs[0]["status"] == 200
...
class TestRateLimitDefaultKey:
    ...
    async def test_client_tuple_used_as_key(self):
        ...
        mw = RateLimitMiddleware(app, max_requests=1, window_seconds=60)
        scope = make_scope(client=("7.7.7.7", 9000))
        await run_mw(mw, scope)
        msgs = await run_mw(mw, scope)
        assert msgs[0]["status"] == 429
    ...
    async def test_client_tuple_preferred_over_xff(self):
        ...
        msgs_xff = await run_mw(mw, scope_xff)
...
class TestRateLimitDecorator:
    ...
    async def test_decorator_allows_within_limit(self):
        ...
        assert await handler(make_request("20.0.0.1")) == "ok"
    ...
    async def test_decorator_blocks_over_limit(self):
        ...
        await handler(make_request("20.0.0.2"))
    ...
    async def test_decorator_includes_retry_after_in_exception(self):
        ...
        await handler(make_request("20.0.0.3"))
    ...
    async def test_decorator_uses_client_ip(self):
        ...
        await handler(make_request("20.0.0.4"))
        with pytest.raises(TooManyRequests):
            await handler(make_request("20.0.0.4"))  # same IP - blocked
        assert await handler(make_request("20.0.0.5")) == "ok"  # different IP - ok
