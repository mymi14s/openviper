...
def make_scope(method="GET", path="/", scheme="https", host="example.com", headers=None):
    h = list(headers or [])
    if host:
...
class TestHostAllowed:
    ...
    async def test_wildcard_allows_all(self):
        ...
        await mw(make_scope(host="anything.com"), None, send)
    ...
    async def test_exact_host_allowed(self):
        ...
        await mw(make_scope(host="example.com"), None, send)
        assert messages[0]["status"] == 200
    ...
    async def test_disallowed_host_returns_400(self):
        ...
        await mw(make_scope(host="evil.com"), None, send)
    ...
    async def test_wildcard_suffix_allowed(self):
        ...
        await mw(make_scope(host="sub.example.com"), None, send)
        assert messages[0]["status"] == 200
...
class TestSSLRedirect:
    ...
    async def test_redirects_http_to_https(self):
        ...
        await mw(make_scope(scheme="http", host="example.com", path="/test"), None, send)
    ...
    async def test_ssl_redirect_preserves_path(self):
        ...
        await mw(make_scope(scheme="http", host="example.com", path="/my/page"), None, send)
        location = dict(messages[0]["headers"]).get(b"location", b"").decode()
    ...
    async def test_ssl_redirect_with_query_string(self):
        ...
        scope = make_scope(scheme="http", host="example.com", path="/test")
    ...
    async def test_no_redirect_when_already_https(self):
        ...
        await mw(make_scope(scheme="https", host="example.com"), None, send)
        assert messages[0]["status"] == 200
...
class TestCRLFRejection:
    ...
    async def test_clean_host_ssl_redirect_succeeds(self):
        ...
        await mw(make_scope(scheme="http", host="example.com"), None, send)
        assert messages[0]["status"] == 301
...
class TestGetHostHelper:
    ...
    def test_get_host_from_header(self):
        ...
        assert mw.get_host(scope) == "example.com"
    ...
    def test_get_host_from_server_fallback(self):
        ...
        assert mw.get_host(scope) == "myserver.com"
...
class TestIsHostAllowedWildcardExact:
    def test_wildcard_matches_bare_domain(self):
        ...
        assert mw.is_host_allowed("example.com") is True
    ...
    def test_wildcard_matches_subdomain(self):
        ...
        assert mw.is_host_allowed("sub.example.com") is True
