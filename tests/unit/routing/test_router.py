...
from openviper.routing.router import Route, Router, compile_path, include, normalize_path
...
def test_normalize_path_double_slash():
    ...
    assert normalize_path("/api//v1//items") == "/api/v1/items"
    assert normalize_path("///deep") == "/deep"


def test_normalize_path_single_slash_unchanged():
    path = "/users/123"
    assert normalize_path(path) is path  # fast-path returns same object
...
@pytest.mark.asyncio
async def test_router_add_class_view_infers_registered_methods() -> None:
    ...
    router = Router()
    router.add("/scores", ScoreView.as_view(), namespace="scores")

    get_route, _ = router.resolve("GET", "/scores")
    post_route, _ = router.resolve("POST", "/scores")
    options_route, _ = router.resolve("OPTIONS", "/scores")

    assert get_route.name == "scores"
    assert post_route.name == "scores"
    assert options_route.name == "scores"
    assert get_route.methods == {"GET", "POST", "OPTIONS"}


@pytest.mark.asyncio
async def test_router_add_class_view_infers_parameterized_method_paths() -> None:
    class ScoreView(View):
        async def post(self, request, **kwargs):
            return Response(b"post")

        async def get(self, request, id: int, **kwargs):
            return Response(str(id).encode())
    ...
    post_route, _ = router.resolve("POST", "/scores")
    get_route, get_params = router.resolve("GET", "/scores/42")
    put_route, put_params = router.resolve("PUT", "/scores/42")

    assert post_route.path == "/scores"
    assert post_route.methods == {"POST", "OPTIONS"}
    assert get_route.path == "/scores/{id:int}"
    assert get_route.name == "scores_id"
    assert get_route.methods == {"GET", "PUT", "OPTIONS"}
    assert get_params == {"id": 42}
    assert put_params == {"id": 42}


def test_router_add_class_view_infers_multiple_parameter_segments() -> None:
    class NestedView(View):
        async def get(self, request, owner_id: int, slug: str, **kwargs):
            return Response(b"nested")

    router = Router()
    router.add("/scores", NestedView.as_view(), namespace="nested")

    route, params = router.resolve("GET", "/scores/7/winner")
    assert route.path == "/scores/{owner_id:int}/{slug:str}"
    assert params == {"owner_id": 7, "slug": "winner"}


def test_router_add_class_view_does_not_duplicate_existing_path_parameters() -> None:
    class DetailView(View):
        async def get(self, request, id: int, **kwargs):
            return Response(b"detail")

    router = Router()
    router.add("/scores/{id:int}", DetailView.as_view(), namespace="detail")

    route, _ = router.resolve("GET", "/scores/1")
    assert route.path == "/scores/{id:int}"


def test_router_add_function_handler_keeps_get_default() -> None:
    router = Router()

    async def handler(request):
        pass

    router.add("/plain", handler)

    route, _ = router.resolve("GET", "/plain")
...
async def test_compile_path_cache():
    p1, c1 = compile_path("/foo/{id:int}")
    p2, c2 = compile_path("/foo/{id:int}")
