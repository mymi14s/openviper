...
class TestSQLInjectionPrevention:
    ...
    def test_db001_safe_table_name_regex(self):
        ...
        assert SAFE_TABLE_NAME_RE.match("users")
        assert SAFE_TABLE_NAME_RE.match("auth_permissions")
        assert SAFE_TABLE_NAME_RE.match("my_table_123")

        # Unsafe table names must be rejected
        assert not SAFE_TABLE_NAME_RE.match("users; DROP TABLE users")
        assert not SAFE_TABLE_NAME_RE.match("users--")
        assert not SAFE_TABLE_NAME_RE.match("'; DROP TABLE users;--")
        assert not SAFE_TABLE_NAME_RE.match("users WHERE 1=1")
