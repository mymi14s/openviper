...
Covers:
- S1: path traversal prevention in persist_files
...
class Field:
    ...
    def __init__(
        ...
        *,
        ...
        self.default = Field.NOT_PROVIDED if use_sentinel else default


def make_field(**kw: object) -> Field:
    return Field(**kw)
...
class TestFieldIsOptional:
    def test_primary_key_is_optional(self):
        field = make_field(primary_key=True)
        assert field_is_optional(field) is True
    ...
    def test_field_with_sentinel_not_provided_is_required(self):
        ...
        field = make_field(use_sentinel=True)
...
class TestModelSerializerCreate:
    ...
    async def test_create_does_not_pass_none_for_unset_optional(self):
        ...
        class FakeModel:
            _fields = {
                "id": make_field(primary_key=True),
                "name": make_field(null=False, default=None),
                "bio": make_field(null=True),
            }
            objects = MagicMock()
    ...
    async def test_create_excludes_readonly_fields(self):
        class FakeModel:
            _fields = {
                "id": make_field(primary_key=True),
                "title": make_field(),
                "created_at": make_field(auto_now_add=True),
            }
            objects = MagicMock()
    ...
    @pytest.mark.asyncio
    async def test_create_rejects_required_readonly_fields_before_database_write(self):
        class FakeModel:
            _fields = {
                "id": make_field(primary_key=True),
                "user": make_field(use_sentinel=True),
                "title": make_field(use_sentinel=True),
            }
            objects = MagicMock()
        ...
        serializer = FakeSerializer.validate({"user": 1, "title": "hello"})
        ...
        assert exc_info.value.validation_errors == [
            {
                "field": "user",
                "message": "Required field is read-only and cannot be saved.",
                "type": "missing",
            }
        ]
        FakeModel.objects.create.assert_not_called()

    @pytest.mark.asyncio
    async def test_create_maps_unique_integrity_errors_to_field_validation_errors(self):
        ...
        class FakeModel:
            _fields = {
                "id": make_field(primary_key=True),
                "game_id": FakeField(use_sentinel=True),
            }
            objects = MagicMock()

        FakeModel.objects.create = AsyncMock(
            side_effect=SQLAlchemyIntegrityError(
                "INSERT",
                {},
                Exception("UNIQUE constraint failed: fake_model.game_id"),
            )
        )
        ...
        serializer = FakeSerializer.validate({"game_id": "abc"})
    ...
    async def test_traversal_sequence_stripped(self):
        ...
        with patch("openviper.serializers.base.default_storage") as mock_storage:
            ...
            with patch.object(
                ...
                await FakeSerializer.persist_files({"avatar": evil_file})

        assert saved_paths, "save should have been called"
        saved = saved_paths[0]
        assert ".." not in saved
        assert saved.startswith("uploads/evil_")
    ...
    async def test_windows_backslash_traversal_stripped(self):
        ...
        with patch("openviper.serializers.base.default_storage") as mock_storage:
            ...
            with patch.object(
                ...
                await FakeSerializer.persist_files({"doc": evil_file})
    ...
    async def test_integer_value_raises_type_error(self):
        ...
        with patch.object(
            ...
            with pytest.raises(TypeError, match="Unsupported file value type"):
                await FakeSerializer.persist_files({"avatar": 1_000_000})
