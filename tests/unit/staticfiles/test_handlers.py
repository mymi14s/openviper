...
def start_headers(send: MagicMock) -> dict:
...
class TestStaticFilesMiddlewareRouting:
    ...
    async def test_passes_through_non_static_path(self):
        ...
        await mw(make_scope(path="/api/users"), AsyncMock(), AsyncMock())
    ...
    async def test_rejects_post_with_405(self):
        mw = StaticFilesMiddleware(AsyncMock())
        send = AsyncMock()
        await mw(make_scope(method="POST"), AsyncMock(), send)
    ...
    async def test_rejects_delete_with_405(self):
        mw = StaticFilesMiddleware(AsyncMock())
        send = AsyncMock()
        await mw(make_scope(method="DELETE"), AsyncMock(), send)
        assert send.call_args_list[0][0][0]["status"] == 405
...
class TestPathTraversalRejection:
    ...
    async def test_rejects_dotdot_component(self):
        mw = StaticFilesMiddleware(AsyncMock())
        send = AsyncMock()
        await mw(make_scope(path="/static/../etc/passwd"), AsyncMock(), send)
    ...
    async def test_rejects_dotdot_in_subdirectory(self):
        mw = StaticFilesMiddleware(AsyncMock())
        send = AsyncMock()
        await mw(make_scope(path="/static/a/../../etc/passwd"), AsyncMock(), send)
        assert send.call_args_list[0][0][0]["status"] == 400
    ...
    async def test_rejects_deep_traversal(self):
        mw = StaticFilesMiddleware(AsyncMock())
        send = AsyncMock()
        await mw(make_scope(path="/static/js/../../../etc/shadow"), AsyncMock(), send)
        assert send.call_args_list[0][0][0]["status"] == 400
...
class TestFindFile:
    ...
    async def test_returns_404_for_missing_file(self):
        ...
        send = AsyncMock()
        await mw(make_scope(path="/static/missing.js"), AsyncMock(), send)
    ...
    async def test_finds_existing_file(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            send = AsyncMock()
            await mw(make_scope(path="/static/app.js"), AsyncMock(), send)
    ...
    async def test_directory_not_served_as_file(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            mw = StaticFilesMiddleware(AsyncMock(), directories=[tmpdir])
            send = AsyncMock()
            await mw(make_scope(path="/static/subdir"), AsyncMock(), send)
        assert send.call_args_list[0][0][0]["status"] == 404
    ...
    async def test_traversal_blocked_by_resolve_guard(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            send = AsyncMock()
            scope = make_scope(path="/static/safe.js")
...
class TestServeFile:
    ...
    async def test_serves_200_with_content(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            mw = StaticFilesMiddleware(AsyncMock(), directories=[tmpdir])
            send = AsyncMock()
            await mw(make_scope(path="/static/hello.txt"), AsyncMock(), send)
    ...
    async def test_head_sends_no_body_content(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            mw = StaticFilesMiddleware(AsyncMock(), directories=[tmpdir])
            send = AsyncMock()
            await mw(make_scope(path="/static/file.js", method="HEAD"), AsyncMock(), send)
    ...
    async def test_etag_header_present(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            mw = StaticFilesMiddleware(AsyncMock(), directories=[tmpdir])
            send = AsyncMock()
            await mw(make_scope(path="/static/file.js"), AsyncMock(), send)

        etag = start_headers(send).get(b"etag", b"")
    ...
    async def test_if_none_match_returns_304(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            send1 = AsyncMock()
            await mw(make_scope(path="/static/file.js"), AsyncMock(), send1)
            etag = start_headers(send1)[b"etag"]

            send2 = AsyncMock()
            scope = make_scope(path="/static/file.js", headers=[[b"if-none-match", etag]])
    ...
    async def test_content_type_for_js(self):
        ...
        assert b"javascript" in start_headers(send)[b"content-type"]
    ...
    async def test_content_length_matches_file_size(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            mw = StaticFilesMiddleware(AsyncMock(), directories=[tmpdir])
            send = AsyncMock()
            await mw(make_scope(path="/static/data.bin"), AsyncMock(), send)

        assert start_headers(send)[b"content-length"] == b"512"
    ...
    async def test_accept_ranges_header(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            mw = StaticFilesMiddleware(AsyncMock(), directories=[tmpdir])
            send = AsyncMock()
            await mw(make_scope(path="/static/f.bin"), AsyncMock(), send)
...
class TestDiscoverAppStaticDirs:
    def test_returns_tuple(self):
        with patch("openviper.conf.settings") as ms:
            ms.INSTALLED_APPS = []
            discover_app_static_dirs.cache_clear()
            result = discover_app_static_dirs()
            discover_app_static_dirs.cache_clear()
    ...
    def test_caches_result(self):
        with patch("openviper.conf.settings") as ms:
            ms.INSTALLED_APPS = []
            discover_app_static_dirs.cache_clear()
            r1 = discover_app_static_dirs()
            r2 = discover_app_static_dirs()
            discover_app_static_dirs.cache_clear()
        assert r1 is r2
...
class TestServeMIMEFallback:
    ...
    async def test_unknown_extension_uses_octet_stream(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            mw = StaticFilesMiddleware(AsyncMock(), directories=[tmpdir])
            send = AsyncMock()
            await mw(make_scope(path="/static/datafile.unknownxyz"), AsyncMock(), send)
    ...
    async def test_gz_file_has_content_encoding_header(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            mw = StaticFilesMiddleware(AsyncMock(), directories=[tmpdir])
            send = AsyncMock()
            await mw(make_scope(path="/static/bundle.gz"), AsyncMock(), send)
...
class TestCollectStaticCopyTreeSkipsDirs:
    def test_copy_tree_skips_non_file_items(self, tmp_path):
        ...
        with patch("openviper.staticfiles.handlers.discover_app_static_dirs", return_value=()):
...
class TestParseRange:
    def test_explicit_range_returns_offsets(self) -> None:
        assert parse_range(b"bytes=0-499", 1000) == (0, 499)

    def test_explicit_range_clamps_end_to_file_size(self) -> None:
        assert parse_range(b"bytes=0-9999", 100) == (0, 99)

    def test_open_ended_range(self) -> None:
        assert parse_range(b"bytes=500-", 1000) == (500, 999)

    def test_suffix_range(self) -> None:
        assert parse_range(b"bytes=-200", 1000) == (800, 999)

    def test_suffix_larger_than_file_returns_full(self) -> None:
        assert parse_range(b"bytes=-9999", 100) == (0, 99)

    def test_multi_range_returns_ignore(self) -> None:
        assert parse_range(b"bytes=0-99,200-299", 1000) == "ignore"

    def test_non_bytes_unit_returns_ignore(self) -> None:
        assert parse_range(b"items=0-99", 1000) == "ignore"

    def test_start_beyond_eof_is_unsatisfiable(self) -> None:
        assert parse_range(b"bytes=1000-1999", 1000) == "unsatisfiable"

    def test_start_greater_than_end_is_unsatisfiable(self) -> None:
        assert parse_range(b"bytes=500-100", 1000) == "unsatisfiable"

    def test_empty_file_suffix_range_is_unsatisfiable(self) -> None:
        assert parse_range(b"bytes=-100", 0) == "unsatisfiable"

    def test_open_ended_at_eof_is_unsatisfiable(self) -> None:
        assert parse_range(b"bytes=1000-", 1000) == "unsatisfiable"

    def test_garbage_value_returns_ignore(self) -> None:
        assert parse_range(b"not-a-range", 1000) == "ignore"

    def test_invalid_ascii_returns_ignore(self) -> None:
        assert parse_range(b"\xff\xfe", 1000) == "ignore"
...
class TestRangeRequests:
    ...
    async def test_range_content_length_is_partial(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            headers = [[b"range", b"bytes=0-4"]]
            await mw(make_scope(path="/static/data.bin", headers=headers), AsyncMock(), send)
        assert start_headers(send)[b"content-length"] == b"5"
...
class TestLastModifiedHeader:
    ...
    async def test_last_modified_present_in_200_response(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            mw = StaticFilesMiddleware(AsyncMock(), directories=[tmpdir])
            send = AsyncMock()
            await mw(make_scope(path="/static/f.js"), AsyncMock(), send)
        assert b"last-modified" in start_headers(send)
    ...
    async def test_last_modified_present_in_206_response(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            send = AsyncMock()
            headers = [[b"range", b"bytes=0-4"]]
            await mw(make_scope(path="/static/f.bin", headers=headers), AsyncMock(), send)
        assert b"last-modified" in start_headers(send)
    ...
    async def test_last_modified_format_is_rfc_1123(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            send = AsyncMock()
            await mw(make_scope(path="/static/f.js"), AsyncMock(), send)
        raw = start_headers(send)[b"last-modified"].decode()
