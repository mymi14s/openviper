...
- DefaultStorage lazy proxy isolation
"""
...
class TestStorageAbstract:
    ...
    def test_generate_unique_name_different_each_call(self):
        names = {generate_unique_name("photo.jpg") for _ in range(50)}
        assert len(names) == 50

    def test_generate_unique_name_preserves_extension(self):
        result = generate_unique_name("archive.tar.gz")
    ...
    def test_generate_unique_name_full_uuid(self):
        ...
        result = generate_unique_name("file.txt")
...
class TestReadSizeLimit:
    ...
    async def test_read_small_file(self, tmp_path):
        fs = make_fs(tmp_path)
        await fs.save("small.txt", b"hello")
        data = await fs.read("small.txt")
        assert data == b"hello"

    @pytest.mark.asyncio
    async def test_read_rejects_oversized_file(self, tmp_path):
        """read() must raise ValueError for files exceeding MAX_READ_SIZE."""
        fs = make_fs(tmp_path, chunk_size=1024)
...
class TestDefaultStorage:
    ...
    def test_instances_are_independent(self):
        """configure() on one DefaultStorage must not pollute another."""
        ds1 = DefaultStorage()
        ds2 = DefaultStorage()
