...
def make_message(message_id: str = "msg-001", actor: str = "my_actor", queue: str = "default"):
    msg = MagicMock()
...
class TestEventBuffer:
    def test_push_non_terminal_no_flush(self) -> None:
        buf = EventBuffer(flush_threshold=5)
        with patch.object(buf._executor, "submit") as mock_submit:
            buf.push(TrackingEvent("id1", {"status": "running"}, terminal=False))
    ...
    def test_push_terminal_triggers_flush(self) -> None:
        ...
        with patch.object(
            ...
            buf.push(TrackingEvent("id1", {"status": "success"}, terminal=True))
        assert len(submitted) == 1

    def test_threshold_triggers_flush(self) -> None:
        buf = EventBuffer(flush_threshold=3)
        submitted = []
        with patch.object(
            buf._executor, "submit", side_effect=lambda fn, evts: submitted.append(evts)
        ):
            for i in range(3):
                buf.push(TrackingEvent(f"id{i}", {"status": "running"}, terminal=False))
        assert len(submitted) == 1
        assert len(submitted[0]) == 3

    def test_queue_cleared_after_flush(self) -> None:
        buf = EventBuffer(flush_threshold=2)
        with patch.object(buf._executor, "submit"):
            buf.push(TrackingEvent("id1", {}, terminal=False))
            buf.push(TrackingEvent("id2", {}, terminal=True))
        assert len(buf._queue) == 0

    def test_flush_calls_batch_upsert(self) -> None:
        buf = EventBuffer(flush_threshold=10)
        events = [TrackingEvent("id1", {"status": "success"}, terminal=True)]
    ...
    def test_flush_handles_exception(self) -> None:
        buf = EventBuffer(flush_threshold=10)
        events = [TrackingEvent("id1", {}, terminal=True)]
        with patch(
...
class TestResetTrackingBuffer:
    def test_clears_queue(self) -> None:
        mw_module._event_buffer._queue.append(
            TrackingEvent("id1", {"status": "running"}, terminal=False)
        )
        reset_tracking_buffer()
