...
class TestPeriodicDecorator:
    ...
    def test_periodic_with_every(self) -> None:
        ...
        registry = Registry()
        assert health_check_periodic.periodic_config["name"] in registry.periodic_jobs
    ...
    def test_periodic_with_cron(self) -> None:
        ...
        registry = Registry()
        assert morning_report_periodic.periodic_config["name"] in registry.periodic_jobs
    ...
    def test_periodic_config_attached(self) -> None:
        @periodic(every="5m", startup=True)
        async def config_task() -> None:
            pass

        assert config_task.periodic_config["startup"] is True

    def test_periodic_cron_stores_schedule(self) -> None:
        """Cron expressions should be stored in the periodic config."""

        @periodic(cron="0 8 * * *")
        async def cron_task() -> None:
            pass

        assert cron_task.periodic_config["cron"] == "0 8 * * *"

    def test_periodic_every_stores_interval(self) -> None:
        """Interval schedules should store the every value in the periodic config."""

        @periodic(every="5m")
        async def interval_task() -> None:
            pass

        assert interval_task.periodic_config["every"] == "5m"
    ...
    def test_periodic_dedup_is_automatic(self) -> None:
        ...
        assert "singleton" not in auto_dedup_task.periodic_config
