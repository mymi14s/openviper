...
def make_actor(name: str = "my_actor") -> MagicMock:
    actor = MagicMock()
...
def make_schedule(due: bool = True) -> MagicMock:
    sched = MagicMock()
...
class TestScheduleEntry:
    def test_is_due_enabled(self) -> None:
        sched = make_schedule(due=True)
        entry = ScheduleEntry(name="t", actor=make_actor(), schedule=sched)
        assert entry.is_due() is True

    def test_is_due_disabled(self) -> None:
        sched = make_schedule(due=True)
        entry = ScheduleEntry(name="t", actor=make_actor(), schedule=sched, enabled=False)
        assert entry.is_due() is False

    def test_is_due_not_due(self) -> None:
        sched = make_schedule(due=False)
        entry = ScheduleEntry(name="t", actor=make_actor(), schedule=sched)
        assert entry.is_due() is False
    ...
    def test_defaults(self) -> None:
        sched = make_schedule()
        entry = ScheduleEntry(name="x", actor=make_actor(), schedule=sched)
        assert entry.args == ()
        assert entry.kwargs == {}
...
class TestScheduleRegistry:
    ...
    def test_register_kwargs_defaults_to_empty(self) -> None:
        reg = ScheduleRegistry()
        entry = reg.register("t", make_actor(), IntervalSchedule(60))
        assert entry.kwargs == {}

    def test_register_with_args_kwargs(self) -> None:
        reg = ScheduleRegistry()
        entry = reg.register(
            "t", make_actor(), IntervalSchedule(60), args=(1, 2), kwargs={"k": "v"}
        )
    ...
    def test_register_disabled(self) -> None:
        reg = ScheduleRegistry()
        entry = reg.register("t", make_actor(), IntervalSchedule(60), enabled=False)
        assert entry.enabled is False

    def test_unregister_existing(self) -> None:
        reg = ScheduleRegistry()
        reg.register("t", make_actor(), IntervalSchedule(60))
        reg.unregister("t")
        assert len(reg) == 0
    ...
    def test_all_entries(self) -> None:
        reg = ScheduleRegistry()
        reg.register("a", make_actor(), IntervalSchedule(60))
        reg.register("b", make_actor(), IntervalSchedule(120))
        entries = reg.all_entries()
        assert len(entries) == 2

    def test_all_due(self) -> None:
        reg = ScheduleRegistry()
        due_sched = make_schedule(due=True)
        not_due_sched = make_schedule(due=False)
        reg.register("due_task", make_actor(), due_sched)
        reg.register("not_due_task", make_actor(), not_due_sched)
        now = datetime(2024, 1, 1, tzinfo=UTC)
        due = reg.all_due(now)
        assert len(due) == 1
    ...
    def test_all_due_defaults_now(self) -> None:
        reg = ScheduleRegistry()
        sched = make_schedule(due=True)
        reg.register("t", make_actor(), sched)
        due = reg.all_due()
        assert len(due) == 1
    ...
    def test_clear(self) -> None:
        reg = ScheduleRegistry()
        reg.register("a", make_actor(), IntervalSchedule(60))
        reg.register("b", make_actor(), IntervalSchedule(60))
        reg.clear()
        assert len(reg) == 0
