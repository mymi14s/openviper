...
class TestToSyncUrl:
    def test_sqlite_aiosqlite(self) -> None:
        result = to_sync_url("sqlite+aiosqlite:///test.db")
        assert result == "sqlite:///test.db"

    def test_mysql_aiomysql(self) -> None:
        result = to_sync_url("mysql+aiomysql://user:pass@localhost/db")
    ...
    def test_postgresql_asyncpg_with_psycopg2(self) -> None:
        with patch("importlib.import_module") as mock_import:
            ...
            result = to_sync_url("postgresql+asyncpg://user:pass@localhost/db")
        assert result.startswith("postgresql+psycopg2://")

    def test_postgres_asyncpg_alias(self) -> None:
        with patch("importlib.import_module") as mock_import:
            mock_import.return_value = MagicMock()
            result = to_sync_url("postgres+asyncpg://user:pass@localhost/db")
    ...
    def test_postgresql_no_sync_driver_raises(self) -> None:
        with patch("importlib.import_module", side_effect=ModuleNotFoundError("no driver")):
            with pytest.raises(RuntimeError, match="No synchronous PostgreSQL driver"):
                to_sync_url("postgresql+asyncpg://localhost/db")
...
class TestGetEngine:
    def test_creates_engine(self) -> None:
        ...
        results_module._upsert_fn = build_upsert_fn("sqlite")
        result = get_engine()
...
# ---------------------------------------------------------------------------
# build_upsert_fn - dialect coverage
# ---------------------------------------------------------------------------
...
class TestBuildUpsertFn:
    ...
    def test_generic_fallback_insert(self) -> None:
        engine = sa.create_engine("sqlite:///:memory:")
        results_module._metadata.create_all(engine)
        fn = build_upsert_fn("generic_db")  # unknown dialect → fallback
        with engine.begin() as conn:
...
# ---------------------------------------------------------------------------
# get_engine with non-memory URL (pool size branch)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# row_to_dict - JSON and datetime handling
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# build_upsert_fn - sqlite conflict do nothing (no update fields)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# get_engine - second check in lock (line 154)
# ---------------------------------------------------------------------------
...
class TestGetEngineLock:
    def test_cached_engine_inside_lock(self) -> None:
        ...
        # Call get_engine while _engine is set → fast path (line 150)
        result = get_engine()
        assert result is engine


# ---------------------------------------------------------------------------
# get_engine - double-checked lock inner check (line 154)
# ---------------------------------------------------------------------------
