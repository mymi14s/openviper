...
class TestExpandField:
    def test_star_wildcard(self) -> None:
        result = expand_cron_field("*", 0, 5)
        assert result == {0, 1, 2, 3, 4, 5}

    def test_single_value(self) -> None:
        assert expand_cron_field("3", 0, 59) == {3}

    def test_range(self) -> None:
        assert expand_cron_field("1-3", 0, 59) == {1, 2, 3}

    def test_step_with_star(self) -> None:
        assert expand_cron_field("*/2", 0, 6) == {0, 2, 4, 6}

    def test_step_with_range(self) -> None:
        assert expand_cron_field("0-6/2", 0, 6) == {0, 2, 4, 6}

    def test_step_with_value(self) -> None:
        result = expand_cron_field("1/2", 0, 5)
        assert result == {1, 3, 5}

    def test_comma_separated(self) -> None:
        assert expand_cron_field("1,3,5", 0, 59) == {1, 3, 5}

    def test_comma_with_range(self) -> None:
        assert expand_cron_field("1-3,7", 0, 59) == {1, 2, 3, 7}

    def test_invalid_step_zero(self) -> None:
        with pytest.raises(ValueError, match="step must be >= 1"):
            expand_cron_field("*/0", 0, 59)

    def test_invalid_step_negative(self) -> None:
        with pytest.raises(ValueError, match="step must be >= 1"):
            expand_cron_field("*/-1", 0, 59)
...
class TestTryImportCroniter:
    def test_returns_bool(self) -> None:
        result = croniter_available()
        assert isinstance(result, bool)

    def test_returns_true_when_available(self) -> None:
        with patch("openviper.tasks.schedule._HAS_CRONITER", True):
            assert croniter_available() is True

    def test_returns_false_when_unavailable(self) -> None:
        with patch("openviper.tasks.schedule._HAS_CRONITER", False):
            assert croniter_available() is False
