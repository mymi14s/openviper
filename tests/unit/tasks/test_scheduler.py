...
class TestStartScheduler:
    ...
    def testenqueue_nows_run_on_start_tasks(self):
        ...
        with patch("openviper.tasks.scheduler.Scheduler"):
            with patch("openviper.tasks.scheduler.threading.Thread"):
                with patch("openviper.tasks.scheduler.enqueue_now") as mockenqueue_now:
                    start_scheduler()

                    mockenqueue_now.assert_called_once_with(
...
class TestTickLoop:
    """Test tick_loop internal function."""
...
class TestTickLoopBranches:
    def testtick_loop_breaks_when_scheduler_is_none(self):
        """tick_loop breaks out of the while loop when _scheduler is None."""
        ...
        with patch.object(_sched_mod, "_scheduler", None):
            with patch.object(_sched_mod, "_stop_event") as mock_event:
                ...
                tick_loop()  # must not raise or loop forever
                mock_event.wait.assert_called_once()

    def testtick_loop_logs_fired_entries(self):
        """tick_loop logs each fired task name when tick() returns non-empty."""
