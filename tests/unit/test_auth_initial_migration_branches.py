...
def test_resolve_user_dependency_empty_for_default() -> None:
    mod = migration_module()
    with patch.object(
        mod,
        "openviper_settings",
        SimpleNamespace(USER_MODEL=None, AUTH_USER_MODEL=mod.AUTH_USER),
    ):
        assert mod.resolve_user_dependency() == []
...
def test_resolve_user_dependency_for_custom_user() -> None:
    ...
    with patch.object(
        ...
        assert mod.resolve_user_dependency() == [("accounts", "0001_initial")]
...
def test_resolve_user_table_reads_table_name() -> None:
    ...
    with (
        ...
        assert mod.resolve_user_table() == "accounts_users"
...
def test_resolve_user_table_falls_back_on_import_error() -> None:
    ...
    with patch.object(
        ...
        assert mod.resolve_user_table() == "auth_users"
