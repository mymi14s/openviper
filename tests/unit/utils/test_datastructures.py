...
class TestCheckNoCRLF:
    def test_raises_on_cr(self):
        """check_no_crlf raises ValueError on carriage-return."""

        with pytest.raises(ValueError, match="CR or LF"):
            check_no_crlf("bad\rvalue")

    def test_raises_on_lf(self):
        """check_no_crlf raises ValueError on line-feed."""

        with pytest.raises(ValueError, match="CR or LF"):
            check_no_crlf("bad\nvalue")

    def test_passes_clean_value(self):
        """check_no_crlf does not raise for a clean string."""

        check_no_crlf("safe-value")  # should not raise
