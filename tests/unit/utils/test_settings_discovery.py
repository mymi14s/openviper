...
class TestDiscoverSettingsModule:
    ...
    def test_dotted_module_settings_found(self, tmp_path: Path) -> None:
        module_dir = tmp_path / "apps" / "proj"
        module_dir.mkdir(parents=True)
        (module_dir / "settings.py").write_text("x = 1\n")
        assert discover_settings_module("apps.proj", cwd=tmp_path) == "apps.proj.settings"

    def test_module_settings_package_found(self, tmp_path: Path) -> None:
        settings_dir = tmp_path / "proj" / "settings"
        settings_dir.mkdir(parents=True)
        (settings_dir / "__init__.py").write_text("x = 1\n")
        assert discover_settings_module("proj", cwd=tmp_path) == "proj.settings"
    ...
    def test_root_settings_package_found(self, tmp_path: Path) -> None:
