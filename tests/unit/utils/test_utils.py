...
class TestResolveTarget:
    ...
    def test_dotted_module_target(self, tmp_path):
        app_dir = tmp_path / "apps" / "shop"
        app_dir.mkdir(parents=True)
        (app_dir / "models.py").touch()
        result = resolve_target("apps.shop", cwd=tmp_path)
        assert result.app_label == "apps.shop"
        assert result.app_path == app_dir
        assert result.models_module == "apps.shop.models"

    def test_dotted_module_rejects_empty_part(self, tmp_path):
        app_dir = tmp_path / "apps" / "shop"
        app_dir.mkdir(parents=True)
        (app_dir / "models.py").touch()
        with pytest.raises(click.ClickException):
            resolve_target("apps..shop", cwd=tmp_path)
